...
// Audit 3c: POST /shifts/:id/assign must verify the target is a real onboarded staff user
// before inserting a shift_request. An existing-but-ineligible user (wrong role / not
// onboarded) otherwise gets an orphan request whose downstream SMS/email silently no-op.

const NONCE = `${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
let server, baseUrl, adminId, adminToken, eligibleId, ineligibleId, shiftId;
...
after(async () => {
  ...
  await pool.query('DELETE FROM users WHERE id = ANY($1::int[])', [[adminId, eligibleId, ineligibleId].filter(Boolean)]);
  await pool.end();
});
