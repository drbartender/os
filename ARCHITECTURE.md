...
| PATCH | `/shifts/:id/out-of-area` (api root, staffing-guarded) | Admin/Manager | The Out-of-Area Bonus knob (spec §6): integer cents 0<x<=25000 (DB CHECK backstop), null clears an UNLOCKED bonus, reduce/clear while locked 409s; lock stamps at approval on every path (approve, auto-assign, cover claim incl. the email one-click cascade) and releases on drop, keeping the amount. (`server/routes/shifts.js` + `serviceArea.js`) |
| GET/POST | `/proposals/:id/remote-staffing-check` / `/proposals/:id/remote-fee-prompt-answered` | Admin | Send-time Remote Staffing Fee popup data: on-demand venue geocode (street-gated, cached in `proposals.venue_lat/lng`), staff-within-40 vs uncounted (NULL-coord staff never count as far), server-derived suggested fee; prompt-answered stamps once per proposal. (`server/routes/proposals/remoteStaffing.js`) |
