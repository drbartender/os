...
- **Helper modules**: `server/utils/telegram.js` (Bot API, raw fetch, no SDK), `server/utils/pendingCall.js` (claim-then-call DB helpers + audit + prune), `server/utils/usPhone.js` (`toUsE164`/`isUsE164`), `server/utils/xmlEscape.js` (shared TwiML escape), `server/utils/vaCallingScheduler.js` (prune + line-aware undelivered-voicemail redelivery sweep + webhook heartbeat), `server/utils/voicemail.js` (voicemail ledger, per-line delivery, line-aware `alertOperator`, constructed-URL Twilio media fetch/delete), `server/utils/voicemailLine.js` (per-line routing policy: `resolveLine`, `E164_RE`, escalation targets, quiet windows, ring clamp, interception canary), `server/utils/voicemailTwiml.js` (shared TwiML fragments so the two emitting routers cannot drift), `server/utils/voicemailEscalation.js` (escalation claim/cap/acceptance/outcome ledger), `server/utils/sms.js#placeBridgedCall` (Twilio `calls.create`, gated identically to `sendSMS`).
