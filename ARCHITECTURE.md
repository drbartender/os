...
| POST | `/inbound/missed` | Twilio signature, **fail-closed in every env** | The `<Dial>` action callback. On a `DialCallStatus` of `no-answer`/`busy`/`failed`/`canceled` it checks `VM_DAILY_CAP`, claims a `voicemail_delivery` row (the ping dedup), returns a spoken greeting + `<Record>`, then pings Zul on Telegram AFTER the response. Any other status, or `VOICEMAIL_ENABLED` off, returns a bare `<Hangup/>`. |
...
- **Inbound flow**: client dials the 224 → `POST /api/voice/inbound` returns `<Dial>` forwarding to `VA_CELL` with the client's number as caller ID, carrying an `action` URL. Carrier voicemail is OFF on `VA_CELL` (owner-confirmed 2026-07-22), which is what makes the miss detectable: the dial rings out, ends `no-answer`, and Twilio requests `POST /api/voice/inbound/missed`. NOT YET VERIFIED: whether Twilio also requests that action URL when the CALLER hangs up mid-ring. Twilio's docs do not say, and one line ("if the initial caller hangs up, the Twilio session ends") leans against it, so an abandoned-while-ringing call may produce no ping and no ledger row. That is the same as pre-feature behavior, not a regression. Settle it with a live test call before relying on it.
