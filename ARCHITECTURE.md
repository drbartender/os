...
- **Webhook events**: `payment_intent.succeeded` (deposit, full, balance, drink-plan-extras), `checkout.session.completed` (deposit OR full via Payment Link, read from `metadata.payment_type`, AND bartender tips, branched on `metadata.kind`)
