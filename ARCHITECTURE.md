...
| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/eligibility/:shiftId` | Staff (`auth` + assigned/approved/not-dropped predicate on `shift_requests`) | Request-window check (event start through contracted end + 15 min grace) + 30-minute end-time step labels (max +3 hours) + pending flag + hosted/class flags. Times and durations only, never a price. |
...
| POST | `/:id/override` | Admin/Manager | Grant the time without payment (required `reason`, 3-500 chars): settle as `overridden`, VOID the invoice and cancel open intents (no receivable survives an override), apply payroll hours, greenlight staff, `finalizeExtension`, audit-logged via `logAdminAction`. |
