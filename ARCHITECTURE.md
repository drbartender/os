...
- `pricing_snapshot` (JSONB — full pricing breakdown at time of creation; also carries `gratuity {rate, tip_jar, staff_count, hours, staff_noun, total}`, `staff_noun`, `display_labels`, and `_version` — stamped by `calculateProposal` (`PRICING_SNAPSHOT_VERSION`); server readers route through `server/utils/pricingSnapshot.js` `readSnapshot()`, tolerating legacy unversioned snapshots as v1 and hard-failing only on unknown future versions)
- **Checkout gratuity** (spec 2026-06-05; election-at-payment 2026-08-03): `tip_jar` (BOOLEAN, default true), `gratuity_rate` (NUMERIC(10,4), default 0 — per-staff-per-hour pre-paid gratuity), `gratuity_rate_change_origin` (TEXT: `'staffing' | 'admin' | null`). CHECK `(tip_jar = true OR gratuity_rate >= 50)`. The dollar gratuity is computed (`rate × staffCount × hours`), appended to the breakdown as a `"Gratuity"` line, and folded into `total_price` on top of any override. **The CLIENT election is written by the `payment_intent.succeeded` webhook, not at checkout**: `create-intent` computes it in memory and stamps `tip_jar`/`gratuity_rate` into the PaymentIntent metadata, and the webhook applies it (before the Balance invoice, `FOR UPDATE`, payable-status guard) so an unpaid proposal never carries a gratuity; that apply sets `origin` to NULL (a client election). The other writer is `server/utils/lineItemCancel.js` — the cancel-line-item gratuity removal/lowering, which is admin's only path to change the gratuity and stamps `origin = 'admin'`. That `'admin'` stamp is live logic, not an archival label: `PATCH /:id` (`crud.js`) reads it and skips the `'staffing'` relabel, so an admin-set gratuity keeps its `'admin'` provenance and fires no staffing-gratuity client notice on a later staffing change (the dollar still rescales at the same rate, as it does for every origin). `'staffing'` is stamped by that PATCH rescale path. One-off cleanup of pre-election rows: `scripts/reset-unpaid-gratuity.js`.
