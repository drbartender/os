...
- Excess derivation (2026-07-26): the portion of any refund that exceeds the contract (`amount_paid - total_price` at refund time) never lowers `total_price`, for every caller. Money paid above the contract is attached to no contract line, so returning it corrects only what is held. Without this, refunding an overpayment lowered total AND paid by the same amount, leaving the proposal overpaid by exactly the same figure forever while the contract silently shrank on each attempt.
