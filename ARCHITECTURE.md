...
- `total_scope` (TEXT NOT NULL DEFAULT 'contract', CHECK contract|overpayment) — the total_price rule for THIS refund. 'contract' = Approach A (refund lowers the total); 'overpayment' (cancel-line) = the fold already corrected total_price before the refund fired, so reconciliation leaves total_price alone. Durable on the row because webhook/sweeper adoption runs later with no caller memory; `refundExecute` adopts its own row BY ID so a stranded same-amount pending row can never rewrite the rule. Invoice `amount_due` is a separate question: it is left alone ONLY for invoices whose demand the CANCEL transaction already rebuilt before the refund fired, i.e. UNLOCKED invoices with a total-tracking label (`proposalMoneyShared.TOTAL_TRACKING_INVOICE_LABELS` = Balance / Full Payment). Every other invoice — locked, or unlocked Deposit / Additional Services / Enhancement Lab / manual — drops both figures, or a settled invoice flips to a client-visible `partially_paid` phantom balance on a still-live pay link. **Do not widen that list to match the refresh.** Since 2026-07-28 `refreshUnlockedInvoices` derives EVERY unlocked label, which makes the two look like they should agree; they must not. The refresh runs BEFORE the refund and floors `amount_due` at `amount_paid`, so it is a deliberate no-op on a settled invoice and cannot stand in for the refund's own correction. Widening the list re-opens exactly the phantom balance it was created to close (`refundHelpers.scope.test.js`, RC1).
...
- **Invoice invariant + monitor**: every open invoice is a derived view of its proposal, never an independent authority for what a client owes. `refreshUnlockedInvoices` (`server/utils/invoiceLifecycle.js`) enforces `Σ(open invoice remaining) ≤ owed` where `owed = total_price − amount_paid`: partial bills (`proposalMoneyShared.PARTIAL_BILL_LABELS` = Deposit / Drink Plan Extras) are capped and never raised, every other label is a remainder bill and takes what the partials leave, and `amount_due` floors at `amount_paid` so a settled invoice stays settled. `server/utils/balanceInvoiceMonitor.js` watches the same invariant hourly in both directions and is **alert-only**: it reads `proposals`/`invoices`, writes only `proposal_activity_log`, and never creates, edits, or voids an invoice (a bug there would be a wrong number in front of a client; a bug here is a spurious email). Gated by `RUN_BALANCE_INVOICE_MONITOR`.
