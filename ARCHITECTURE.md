...
| GET/POST/PATCH | `/staff-reviews`, `/staff-reviews/:id`, `/:id/confirm`, `/:id/dismiss` | Admin | Review log (spec 2026-08-06 §7): TT-webhook-ingested + manual google rows, credit tagging, confirm materializes the $10 `review_bounty` per credited 5-star staffer (and clears system removals so dismiss-then-reconfirm re-pays), dismiss refuses while a bounty is paid. All mutations audit-log. (`server/routes/admin/staffReviews.js`) |
