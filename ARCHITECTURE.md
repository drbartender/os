...
- **PARTIALLY ENFORCED.** Two gates exist and only one honors this column. The audience resolver's `MAILABLE_SQL` (lane mkt-c) excludes the contact from every new campaign. The LIVE gate for the automated touches — the drip, the retention nudge, the New Year touch — is `scheduledMessageDispatcher.js`'s marketing-category check, which consults `communication_preferences.marketing_enabled` and nothing else. **Lane `mkt-f-compliance` owns patching it**, deliberately deferred out of mkt-a and mkt-c so a comms-critical file is opened by the lane that already touches that file family and already carries `security-review`. Until then an excluded client is excluded from new campaigns and still receives the automated ones. Interim safety: no UI writes the flag before lane mkt-d.
