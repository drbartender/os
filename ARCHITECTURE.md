...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/opt-in` | **public, none** (`publicLimiter`) | Submit for the standalone `/sms` opt-in page (`server/routes/smsOptIn.js`, mounted ahead of `sms.js`). Requires name + email + phone + a ticked `sms_consent`; records via `recordSmsConsent` with `source_form='sms_page'`. Rolls the whole submit back rather than leave a created client SMS-on without a recorded opt-in. Same generic 200 for every consent outcome (no client/opt-out enumeration); 503 `SMS_CONSENT_VERSION_SKEW` on an unknown consent version. |
