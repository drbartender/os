...
| GET | `/t/:token/lab` | Public | Enhancement Lab shelves (`drinkPlans/lab.js`): submitted drinks with dossier enhancements + housemade-syrup pricing (server-computed via `calculateSyrupCost`, unpriceable ids never offered), active addon price list (Jack pair excluded), proposal balance state, stored lab additions. `state`: `not_ready` (pre-submit) / `open` / `locked` (list approved or plan finalized) |
| PUT | `/t/:token/lab` | Public | Reconcile the client's lab additions (full desired state, idempotent). 409 outside the open window. Transactional: merges `labAdded` addOns + `labSyrupSelections` into selections (planner-owned addOns never touched), funds a dedicated `'Enhancement Lab'` invoice at the lab-only `computeExtrasBreakdown` total (NEVER the pay-now-coupled `'Drink Plan Extras'` invoice; settled lab invoices subtract so pay-then-add mints a delta invoice). Post-commit: regenerates the shopping list (admin_set holds preserved), strips lab-covered syrup lines, stages `pending_review` (never touches an approved list) |
