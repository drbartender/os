...
- **RECORDED ONLY, NOT YET ENFORCED. Nothing reads this column today.** It is written by `PUT /api/marketing/contacts/:id/do-not-contact` and returned by the clients list, and that is all. Two gates will eventually honor it, neither of which exists yet in shipped code: the audience resolver's `MAILABLE_SQL`, which excludes the contact from new campaigns and lands with **lane mkt-c**; and `scheduledMessageDispatcher.js`'s marketing-category check, the LIVE gate for the drip, the retention nudge, and the New Year touch, which today consults `communication_preferences.marketing_enabled` and nothing else. **Lane `mkt-f-compliance` owns patching that dispatcher gate**, deliberately deferred out of mkt-a and mkt-c so a comms-critical file is opened by the lane that already touches that file family and already carries `security-review`. Interim safety: no UI writes the flag before lane mkt-d, so nothing can be marked excluded and then silently emailed in the gap.
