...
- **Checkout gratuity** (spec 2026-06-05): `tip_jar` (BOOLEAN, default true), `gratuity_rate` (NUMERIC(10,4), default 0 — per-staff-per-hour pre-paid gratuity), `gratuity_rate_change_origin` (TEXT: `'staffing' | 'admin' | null`). CHECK `(tip_jar = true OR gratuity_rate >= 50)`. The dollar gratuity is computed (`rate × staffCount × hours`), appended to the breakdown as a `"Gratuity"` line, and folded into `total_price` on top of any override.
...
- **Alternative**: Admin generates a reusable Payment Link via `POST /api/stripe/payment-link/:id`. Amount follows the same booking-window policy as the client flow — full total inside the 14-day window, otherwise the $100 deposit — and tags the link with `metadata.payment_type` so the webhook settles it correctly.
