...
| POST | `/inbound/missed` | Twilio signature, **fail-closed in every env** | The shared `<Dial>` action callback, keyed on the HMAC-covered `?line=`. On a missed `DialCallStatus` it checks `VM_DAILY_CAP`, claims a `voicemail_delivery` row (the ping dedup, stamping `line`), returns the PER-LINE greeting (+ a press-1 `<Gather>` wrapping it when `VM_ESCALATION_ENABLED`) + `<Record>`, then pings Zul AFTER the response on ZUL-LINE misses only (a primary miss shows natively on the GV dial target). Any other status, or `VOICEMAIL_ENABLED` off, returns a bare `<Hangup/>`. An instant-answer on the primary line is logged by the interception canary (LOG-ONLY since 2026-08-14 — the per-call `dialSec` console line; the Sentry page was removed as inverted, see voice.js's canary block). |
