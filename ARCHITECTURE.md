...
| POST | `/payroll/payouts/:id/toggle-draw` | Admin | Flip a payout `pending` ⇄ `no_draw` (owner rows: tracked, never owed — spec 2026-08-07). Row-locked like mark-paid; refuses paid payouts and paid periods. Parking the last pending payout of a `processing` period finalizes it (same check as mark-paid). Audit-logged as `payout_toggle_draw`; audit runs after client release (one-connection rule). Returns `{ payout, period_status }`. |
...
| GET | `/payouts/:periodId/paystub` | Yes | Lazy-generates a PDF paystub on first download for a PAID period, uploads it to R2 (`paystubs/<contractor_id>/<period_id>.pdf`), caches the key on `payouts.paystub_storage_key`, and returns `{ url }` (a 15-minute signed URL). 404 if not the staffer's payout, 409 if the period is not paid. Renders via `paystubPdf.js` from `paystubData.js` (incl. YTD); never touches `mark-paid`. `payment_handle` is intentionally not on the document (PII). |
