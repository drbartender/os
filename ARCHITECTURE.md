...
- **Server:** `@sentry/node` initialized at the top of `server/index.js`. Gated on `SENTRY_DSN_SERVER` env var (silent in dev). Redaction lives in `server/utils/sentryScrub.js`, NOT inline in `index.js`, so it can be unit-tested (index.js starts the app on require). It exports two hooks because Sentry has two pipelines that do not share one: `scrubErrorEvent` (`beforeSend`, ERROR events) and `scrubTransactionEvent` (`beforeSendTransaction`, PERFORMANCE events). Both drop request headers/cookies, redact the request body, and run every URL through `scrubUrl` (public token segments: `/t/`, `/tip/`, `/unsubscribe/`, `/reset-password/`, `/api/voice/vm/`, plus `?token=`). Both hooks also scrub the transaction NAME, breadcrumbs, and the OpenTelemetry span attributes in `contexts.trace.data` (the transaction hook additionally walks `spans[].data` and span descriptions, which error events do not carry), and delete any captured `http.request.header.*` / `http.response.header.*` attribute (which re-carries what the request-block scrub removes). That span scrub works **by VALUE, not by key name**: every string attribute is run through `scrubUrl`, which is a no-op on anything lacking a public-token path shape. This is deliberate and load-bearing. A live listen token reached Sentry three times during the 2026-08-11 review. The first was a missing path shape plus an error-only hook; the last two were the same mistake twice — the defense was a list of attribute keys, and the SDK had one more. The third came through a bare `url` key that Sentry's own OTel bridge derives from `url.full` (`getRequestSpanData` in `@sentry/opentelemetry`), together with `http.query` and `http.fragment`; no key allowlist would have covered it. `SPAN_URL_KEYS` still exists but is documentation, pinned by a test asserting each listed key is genuinely scrubbed. `http.query` additionally gets a prefix-free `token=` rule because it is `url.search`, which carries the leading `?` in some SDK versions and not others. **When you add a public-token route, add its path shape to `scrubUrl`** — that one function is what both pipelines, every span attribute, and every breadcrumb go through. `scrubUrl` also redacts the Twilio Account SID, which is the basic-auth username on our media fetches and rides in outgoing-request URLs.

The scrub fails CLOSED. Walking attributes we did not author means a frozen object or a throwing getter is reachable, and both must not end in a leak: the module is `'use strict'` (in sloppy mode a failed assignment is a SILENT no-op, which would ship the token with no symptom) and the walk is wrapped, because an exception escaping a Sentry hook makes the SDK DISCARD the whole event. An attribute bag that cannot be safely walked is therefore replaced wholesale with `{'sentry.scrub_failed': true}` and the event is tagged `scrub_failed=true`. **If you see that marker in Sentry, the span's attributes were dropped deliberately** — timing, `op`, and the span tree are intact; only the attribute bag is gone.
...
- `listUncertifiedStaffable()` — the admin compliance alert. **Certification only**, and only across `STAFFABLE_STATUSES` (`submitted`/`reviewed`/`approved`), which mirrors the assignment gate at `server/routes/shifts.approval.js:233`. A LATERAL join attaches each worker's soonest upcoming shift so a booked worker outranks a merely-eligible one. Served by `GET /api/admin/hiring/uncertified` and rendered as one named row per person in the Needs-attention strip. The client applies the Staffing tab's 14-day horizon to that escalation (2026-08-17): a booking further out reverts to the standing "eligible" row rather than dropping, because a certification takes lead time to obtain and the person must never fall off the tab.
...
(`CURRENT_VERSION = 'contractor-agreement-v3'`; v2 stays in the `VERSIONS` map for prior signers). The React page at `/agreement` fetches the current version
...
| GET | `/active-staff` | Staffing | Paginated list of onboarded staff. `?include_stubs=true` widens the status list to include `deactivated`. `onboarding_progress` is a LEFT JOIN and the `onboarding_completed` check is skipped for deactivated rows (2026-08-19, staff-hub spec): the imported placeholders were seeded with no progress row at all and a staffer who quit mid-onboarding has an incomplete one, so the old inner JOIN hid rows the hub's `deactivated_count` had already counted. The active set is byte-identical (approved/reviewed/submitted still require a completed progress row). |
| GET | `/staff-hub/summary` | Admin/Manager | The ONE read behind the Staff hub chrome (spec 2026-08-19): roster counts (`active_count`, `deactivated_count`, `former_staff_count`, `imported_count`, sharing the roster feed's predicate family so count and list agree), admin decision counts (`new_applications`, `pending_reviews`), and `open_period`. The open pay run is DERIVED from the Chicago date by `server/utils/staffHubSummary.js` (`summarizeOpenPeriod`, pure) and LEFT JOINed to `pay_periods` on its UNIQUE `start_date`, because rows are minted lazily by `ensurePayPeriod` when the week's first shift accrues: a row lookup alone goes blank mid-week. Read-only by construction, it never INSERTs a `pay_periods` row (the `findOpenPeriodForDate` INSERT-fallback in `payrollLateTip.js` / `payrollClawback.js` is deliberately not copied). Managers get `null` for the three admin fields; a manager without `can_staff` gets `null` for the roster counts too (same gate as `/active-staff`). (`server/routes/admin/staffHub.js`) |
...
| GET | `/:id/shopping-list` | Admin | Fetch persisted shopping list + `shopping_list_status` + `finalized_at` (the modal opens read-only while the BEO is finalized) |
| PUT | `/:id/shopping-list` | Admin | Save shopping list edits (auto-saved by the modal while admin edits) |
| PATCH | `/:id/shopping-list/approve` | Admin | **Deprecated**, kept mounted for API compatibility: delegates to the `shopping_list_approve` comms action. Runs the action's idempotent side effects (atomic flip of `shopping_list_status` to `'approved'` + approved-snapshot write, then the derived BEO finalize) then sends the default unedited email on the default channels; the response carries `side_effects: { beo }`. New admin sends go through the compose-first `POST /api/comms/send`. |
...
| PATCH | `/:id/status` | Admin (`drinkPlanWriteLimiter`) | Update plan status. A flip to `reviewed` runs the derived BEO finalize (`autoFinalizeIfEligible`) and the response carries `beo: { finalized, reason?, unpaid_extras_cents? }` for that transition only. |
...
| GET | `/t/:token` | Public | Fetch questionnaire by token (JOINs proposal for guest_count, num_bartenders, pricing_snapshot). Returns a locked payload `{ locked: true, proposalToken }` when the linked proposal is pre-deposit, so a stale emailed `/plan/:token` link renders a lock screen instead of the wizard (`isDrinkPlanPreBooking` allowlist, fails safe). Planner v2: payload adds `planner_version`, package slot config, and for v2 hosted plans a `hosted_coverage` per-drink two-tier block (`has_contents:false` = contents not entered, legacy hosted flow renders) |
| POST | `/t/:token/lab-cta` | Public | Enhancement Lab CTA click marker: write-once `lab_cta_clicked_at` (attach-rate funnel) |
| GET | `/t/:token/lab` | Public | Enhancement Lab shelves (`drinkPlans/lab.js`): submitted drinks with dossier enhancements + housemade-syrup pricing (server-computed via `calculateSyrupCost`, unpriceable ids never offered), addon price list (Jack pair + package-covered slugs excluded), proposal balance state, stored lab additions + their server-priced `lab_breakdown`. `state`: `not_ready` (pre-submit or planner v1) / `open` / `locked` (list approved or plan finalized) |
| PUT | `/t/:token/lab` | Public | Reconcile the client's lab additions (full desired state, idempotent; offered-surface allowlist + dossier syrup pairing + drift-drop). 409 outside the open window. Transactional: merges `labAdded` addOns + `labSyrupSelections` into selections (planner-owned addOns never touched), then FOLDS additions into the proposal — lab-owned `proposal_addons` rows + contract-safe reprice via `utils/proposalExtrasFold.js` (override moves by catalog delta; paid-in-full demoted when new money is owed) — and `refreshUnlockedInvoices` absorbs the new total into the open Balance/Full Payment invoice with each lab addon as its own line. A standing open `'Enhancement Lab'` invoice zeroes while a Balance absorbs (pre-fold migration + coexistence guard). Only when nothing absorbs and locked invoices exist (fully paid) does ONE open itemized `'Enhancement Lab'` invoice carry the UNINVOICED remainder (total − external − locked − other open contract invoices, so a standing unpaid Additional Services invoice is never double-billed; FOR UPDATE + guarded against a concurrent webhook pay+lock). Fails closed (400) when the proposal can't reprice. Response returns the folded `balance`. Post-commit: regenerates the shopping list (admin_set holds preserved), strips lab-covered syrup lines, stages `pending_review` (never touches an approved list) |
| PUT | `/t/:token` | Public | Save draft or submit selections (on submit: processes addOns into proposal_addons, recalculates pricing, sends admin email, auto-generates pending_review shopping list). Planner v2: allow-list adds `crowd`/`barPlacement`/`powerAtBar` (normalized) + the guestPreferences data-loss fix; hosted fence addons validate through coverageEngine and the mocktail Jack rule is re-derived server-side from RESOLVED picks (1 = pre-batched, 2+ = mocktail-bar), never trusted from the client; a v2 submit with `logistics.parking = 'paid'` server-derives the `parking-fee` add-on attach (per_staff, engine-priced, attach-only — removal is admin cancel-line-item); confirmation email appends the full-selections echo |
...
| POST | `/:id/finalize` | Admin | Manual finalize. **Since 2026-09-11 finalize is derived:** a plan finalizes on its own the moment it is `reviewed` AND its shopping list is `approved` (a hosted package, `service_packages.category='hosted'` and `bar_type<>'class'`, needs no list), never over unpaid extras. Mark reviewed and the shopping-list approve both call `autoFinalizeIfEligible`; whichever lands last fires it, and `proposal_activity_log.details.trigger` records `manual` / `reviewed` / `shopping_list_approved`. This route stays as the unpaid-extras override (409 `unpaid_extras` unless `overrideUnpaidExtras: true`) and the way back after Unfinalize. Stamps `finalized_at`/`finalized_by` (only when status=`reviewed`, selections non-empty, proposal not archived), schedules T-3 staff nudges for every approved staffer on every non-cancelled shift, writes `beo_finalized` activity log. Locks every mutation route on the plan until Unfinalize. Every refusal is a `ConflictError` with a code (`not_linked`, `archived`, `no_selections`, `already_finalized`, `not_reviewed`, `list_not_approved`, `unpaid_extras`). Implemented in `server/utils/beoFinalize.js`, mounted into the drink-plans router. |
...
| POST | `/send` | Admin/Manager (`adminWriteLimiter`) | Run the action's idempotent side effects, then dispatch the admin's (possibly edited) message on the selected channels. Returns per-channel truth (`sent`/`failed`/`skipped` + skip reasons); each attempt writes a `message_log` row carrying `sent_by` + `body_edited`. Validates non-empty subject/body per selected channel and the 640-char SMS cap; an empty channel list is accepted only when the action genuinely has no channel to offer (side-effects-only confirm). An optional `silent: true` (with `channels: []`) applies the action's side effects and sends nothing, honored only for actions that declare `allowSilent` (currently `shopping_list_approve`, for the editor's quiet re-publish); it is validated before `ensureSideEffects`, so a rejected silent request never flips a side effect. The response also carries `side_effects_applied` and an allow-listed `side_effects` report (today only `shopping_list_approve`'s `beo: { finalized, reason?, unpaid_extras_cents? }`, meaning "the BEO is finalized now", so a Retry never clobbers it). |

### BEO — `/api/beo`
Route paths and the `beo_*` identifiers are frozen; the 2026-07-22 staff event-details spec renamed only the words staff read ("BEO" is gone from the staff UI). Payload construction is shared with the shift-keyed surface below via `server/utils/eventDetailsPayload.js`. All three routes require `auth` + `requireOnboarded`.

| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/:proposalId` | Auth + onboarded (admin/manager always; any onboarded staffer when the proposal has a non-cancelled shift) | Event-details payload — proposal/client/package, drink plan (without `token`), addons, shift_requests with `beo_acknowledged_at`, `shifts[]` (every non-cancelled shift with roles/`approved_by_role`/equipment/supply-run/cover flags + the viewer's own request), `menu_print` tri-state, viewer flags (`is_admin`, `is_assigned`, `is_acknowledged`), and `team_roster[]`. **Redaction:** `client.phone` and teammate phones are null unless the viewer is approved+active on the proposal (or admin/manager) — the anti-harvest gate that survives the read ungating. 404 (never 403) when the proposal is missing or has no staffable shift, so staff cannot enumerate proposal ids. Rate-limited per `req.user.id` via `beoReadLimiter` (60/15min). |
...
| POST | `/:proposalId/acknowledge` | Auth | **Write, still assigned-only** (the read ungating did not extend to it; the UPDATE predicates are the enforcement). Staff: stamps `shift_requests.beo_acknowledged_at = NOW()` on every approved active shift_request the staffer holds on this proposal (gated on `drink_plans.finalized_at IS NOT NULL`; 409 if not finalized). A caller who is not on the event gets the assignment 409, checked before the finalize gate so the message names the real reason. Admin/manager: returns `{acknowledged:false}` (no-op). |

### Staff event details (shift-keyed) — `/api/shifts`
Mounted from `server/routes/eventDetails.js`, BEFORE `staffShiftActions.js` (which applies `auth` router-wide, so falling through it would run `auth` twice). This is what the staff portal page calls; keying by shiftId removes the old three-feed proposalId hunt and makes deep links work cold. Both routes require `auth` + `requireOnboarded` and share the `beoReadLimiter` budget (60/15min per user).

| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/:shiftId/event-details` | Auth + onboarded (any onboarded staffer) | Same payload as `GET /api/beo/:proposalId` plus `shift_id`. A cancelled or unknown shift 404s. A legacy shift with no `proposal_id` returns a shift-only brief (`proposal: null`, `menu_print: {status:'not_required'}` so consumers never branch on null, one row in `shifts[]`) so a requestable manual shift still renders. |
| GET | `/:shiftId/menu-print` | Auth + onboarded (admin/manager, or staff approved+active on the proposal) | Bar-menu print-file download. R2 proxy mirroring the logo route (signed URL, 8 s timeout), served `Content-Disposition: attachment`. The key must match `menu-print/<thisProposalId>/` and contain no traversal segment, so one proposal's file can never be served under another. 403 for a browsing staffer: downloading is an assigned-staff action. 404 when no file is posted. |
...
| POST | `/:id/request-aliases` | Admin | Remember a client-typed custom request (`{alias}`) as this drink (shopping-list Match existing). Appends to `request_aliases` in a transaction; no-op on the drink's own name or an alias it already carries; 400 at the 20-alias cap or over 200 chars; 409 naming the drink when the text already equals the name or an alias of another drink IN THE MATCHER'S CANDIDATE POOL (recipe-carrying, on- or off-menu, either table); recipe-less rows such as an abandoned Add-recipe draft never block a match |
...
| POST | `/:id/request-aliases` | Admin | Same alias-append contract as cocktails (shared `appendRequestAlias` in `routes/potions.js`) |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/` | Admin | List proposals with filters. Base filters: `status` (single value or CSV, whitelisted), `search`, `source`, `view`. Money-board filters (mb-a lane): `from`/`to` (YYYY-MM-DD, validated server-side; malformed values ignored), `axis` (`event`\|`sent`, selects the date column), `event_type`, `balance=open` (accepted rows where `total_price > amount_paid`), and `cohort` (`quoted`\|`won`\|`lost`) which mirrors the dashboard-stats `qSent`/`qAccepted`/`qLostValue` predicates exactly (and its own date column) and supersedes `status`/`view` when present. `X-Total-Count` carries the unpaginated total (the reconciliation surface for the Overview drill-outs). Default excludes paid + archived (those appear in Events / archive) |
| GET | `/financials` | Admin | Aggregated revenue + payments for the Overview money board (Band 2 — the former FinancialsDashboard). Accepts `?from=&to=&basis=` (booked\|scheduled\|paid) for date-range + lens filtering. `summary.leadSpend` reports Thumbtack acquisition cost (charged leads only, cents, attributed-to-proposal vs not, cash basis by lead created_at). |
...
| GET | `/dashboard-stats` | Admin | Server-side aggregations (booked / collected / outstanding / funnel / pipeline / monthly revenue series) for the Overview money board (Dashboard + Financials merged into one surface at `/dashboard`; `/financials` and `/financials?tab=payouts` now redirect there). Accepts `?from=&to=&basis=` for date-range + lens filtering; prior-period deltas included. |
| GET | `/metrics-split` | Admin/Manager | Funnel sent/accepted math split by `by=source\|event_type` (required, whitelisted; anything else 400s) with optional `?from=&to=` (YYYY-MM-DD, half-open via the shared `dateClause`; malformed ignored). Native-era only (no `basis`/`include_cc` — the frozen CC ledger keeps no type/source detail, so this endpoint never reads the cc tables). Query-time vocabulary normalization merges the twin event-type vocabularies (native slug + Thumbtack human string) and folds NULL/empty to the `__untyped` sentinel; source folds NULL to `direct`. Returns `{ by, filters, segments: [{ key, sent:{count,value}, won:{count,value}, closeRatePct, pending }], truncated }` (keys only; labels resolve client-side). `sent`/`won`/`closeRatePct` mirror `qSent`/`qAccepted`/`qWinRate` per segment so the sum of segment `won.count` reconciles with the funnel `qAccepted` native leg. Segments capped at 12 (ordered by sent desc); the remainder rolls into one `__other` row and `truncated` reports it. Sibling of `dashboard-stats`; the two LAW endpoints stay byte-frozen. |
| POST | `/` | Admin | Create proposal (auto-calculates pricing, creates client if needed). Rules are re-validated server-side via `validateProposalRules`. `send_now: true` creates the proposal as `sent`, creates its invoice in-transaction, and emails the client; otherwise it lands as a `draft`. `send_now` is fail-safe: anything but an explicit `true` is a draft. The `send_now: true` path stays for API compatibility, but the admin creation cockpit no longer uses it: it always creates a `draft`, then composes the initial send through `POST /api/comms/send` (action `proposal_send`: a row-locked draft→sent flip + invoice in one transaction, with drip enrollment post-commit). A past-2:00-AM duration is refused 400 (`fieldErrors.past_curfew`) unless the body carries `acknowledge_past_curfew: true`, which books it and audit-logs `proposal_booked_past_service_curfew`. Rate-limited per admin via `adminWriteLimiter` (10/min). |
| POST | `/calculate` | Admin | Preview pricing without saving |
| GET | `/packages` | Admin | List service packages |
| GET | `/addons` | Admin | List add-ons |
| GET | `/:id` | Admin/Manager (`auth, requireAdminOrManager`) | Get single proposal with addons + activity log + `lead_call` (newest lead-call bridge outcome for TT-drafted proposals, null otherwise). Also carries the refund-side derived money (spec 2026-09-15): `overpayment_cents` (the NETTED overpayment, same figure and key as the cancel-line preview), `off_contract_paid_cents` (the netting term, so the editor can net a hypothetical new total without a round trip), `max_refundable_cents` (largest single Stripe charge still refundable) and `max_overpayment_refundable_cents` (largest UNCREDITED headroom on one charge, i.e. how much of the overpayment can actually come back through Stripe). |
| PATCH | `/:id` | Admin | Update event details, recalculate pricing, and re-sync the linked event shift (date/time/location/client) when the proposal has been converted. A draft→sent transition creates the proposal's invoice in the same DB transaction. Accepts an optional `change_request_id` in the body that links the edit to a pending `proposal_change_requests` row: the request is stamped `approved` in the same transaction as the edit, the standard admin edit email is suppressed (the client gets the change-request decision email instead), and the regular money + status reconciliation runs. **Notify-client contract (2026-07-22): the reschedule notice is opt-in** — the request names it via `notify: [{ type: 'event_details_changed', channels, email: {subject, body_text}, sms: {body} }]` (reviewed text composed at preflight; absent/empty list = nothing sends; a requested notice the save does not trigger is a 400 that rolls the edit back). Re-anchoring + balance-due recompute always run regardless. The staffing-gratuity disclosure stays automatic (billing disclosure) behind the suppression gate. When the save touches `event_date` / `event_start_time` / `event_duration_hours` and the result runs past the 2:00 AM curfew, it refuses 400 (`fieldErrors.past_curfew`) unless the body carries `acknowledge_past_curfew: true` (booked + audit-logged `proposal_booked_past_service_curfew`). Response carries `notifications: [{type, email, sms, email_error, sms_error, skip_reasons}]`. |
...
| POST | `/:id/resend` | Admin | **Deprecated**, kept mounted for API compatibility: delegates to the `proposal_resend` comms action. Re-sends the proposalSent email + initial-proposal SMS without changing status (validate-only RESENDABLE-status guard, no state side effect); dispatch owns its `message_log` writes. New admin sends go through the compose-first `POST /api/comms/send`. Rate-limited via `adminWriteLimiter`. |
| POST | `/:id/portal-invite` | Admin | **Deprecated**, kept mounted for API compatibility: delegates to the `portal_invite` comms action. Emails the client their portal link (plain invite; the portal sits behind the OTP login, so no token rides in the email, and nothing is minted); no state side effect. Still logs `portal_invite_sent`. New admin sends go through the compose-first `POST /api/comms/send`. Rate-limited via `adminWriteLimiter`. |
| PATCH | `/:id/notes` | Admin | Update admin notes |
| DELETE | `/:id` | Admin | Delete a proposal |
| GET | `/t/:token` | Public | Fetch proposal by token (tracks views + geolocation). Payload built by the shared `buildPublicProposalPayload` (also the switch endpoint's response, so the two shapes cannot drift). Response includes `client_phone_prefill` (blank when the stored phone matches a known Thumbtack proxy, so the sign form never displays the proxy back to the customer) and `options_available` (whether the "See other packages" drawer entry link has anywhere to go: status sent/viewed, no `total_price_override`, unsigned, not grouped-undecided, `amount_paid = 0`, >= 2 comparable catalog packages; the gate columns themselves never leak). **An `archived` proposal is NOT served here — 404, same door-close as a voided invoice link**, so a stale emailed link cannot render a live page with working Sign & Pay. Note the asymmetry with `/t/:token/resolve` below, which is deliberately status-blind. Limiters: `requireUuidToken` runs first, then `publicTokenIpLimiter` (300 per 15 min, keyed by IP) as a ceiling on the client-supplied token key, then the token-keyed `proposalCheckoutLimiter` (60 per 15 min), which this route was moved onto off the shared IP-keyed `publicLimiter` on 2026-08-28 so that browsing can never spend the budget paying depends on. |
| POST | `/t/:token/sign` | Public | Client signature + acceptance. Body accepts an optional `client_phone`; a valid value overwrites `clients.phone` (replacing a Thumbtack proxy with the customer's real number) and resets `phone_status` to `'ok'`. Body requires `acknowledged_total` (400 without it, since 2026-08-28; the switch route has always required it): the UPDATE lands only if it still matches the row total, else 409 `TOTAL_CHANGED` (409 `ALREADY_ACCEPTED` when the row is no longer signable). Every 409 writes a `sign_failed` activity row (`code`, `acknowledged_total`, the proxy-validated `ip`, so a run of them reads as one client retrying or a leaked token being probed) and pages Sentry `proposal_sign_failed` (warning for `TOTAL_CHANGED`, info for a benign `ALREADY_ACCEPTED` replay), and the `signed` breadcrumb carries `acknowledged_total` too, so a blocked signature is never silent. |
| PATCH | `/:id/balance-due-date` | Admin | Override balance due date for a proposal |
| POST | `/:id/notify-preflight` | Admin/Manager | **Read-only.** Which client notices a pending edit would trigger, plus the drafted event-details message (composed old-vs-new while the old values still exist; money-free except the projected balance-due/auto-charge date, computed by the same function the save uses). Zero notices when `change_request_id` is present. Writes nothing. |
| POST | `/:id/send-reminder` | Admin | **Deprecated**, kept mounted for API compatibility: delegates to the `payment_reminder` comms action. Emails (and can text) the client a balance-due reminder (balance = `total_price - amount_paid`, computed exactly as before, no money written; `ensureSideEffects` only enforces the NO_BALANCE_DUE guard). New admin sends go through the compose-first `POST /api/comms/send`. |
| POST | `/:id/record-payment` | Admin | Record an outside payment (cash, Venmo, etc.) — triggers shift creation. Client receipt is **opt-in** via `notify_client: true` (absent/false = no receipt; suppression + `.invalid` placeholder gates apply even to a yes); the admin routine_finance notice always fires. Response carries `notifications` (`payment_receipt` entry when attempted). |
...
| POST | `/:id/send-group` | Admin/Manager | **Deprecated**, kept mounted for API compatibility: delegates to the `proposal_send_group` comms action. Sends the whole comparison as ONE `proposalOptionsSent` email (one `/compare/:token` link), transitioning every draft option to `sent`. Defers per-option invoicing (to the winner's settle) and suppresses the per-option `proposalSent` email/SMS. Idempotent: a resend with no new draft options sends nothing. `PATCH /:id/status → sent` on a grouped option is blocked (409 `USE_GROUP_SEND`) so the solo path can't double-invoice/email it. New admin sends go through the compose-first `POST /api/comms/send`. |
| POST | `/:id/archive` | Admin/Manager | Archive this proposal (scope `one`) or this plus every other open, unpaid proposal for the same client (scope `set` — covers formal comparisons and loose multi-proposal sets). Paid/converted targets 409 (`NOT_ARCHIVABLE`). Voids unpaid invoices in the same transaction; best-effort marketing/change-request reaps run post-commit. Note: on a client's FIRST settled payment (deposit/full/record-payment), the same sweep runs automatically via `sweepClientAlternatives` — their other open unpaid proposals archive as `option_not_chosen`; later balance/extras payments never sweep. |
| POST | `/:id/cancel/preview` | Admin/Manager | Server-computed consequence preview for cancelling a BOOKED event (`server/routes/proposals/cancel.js`): `days_out`, `mode`, `refund_cents`, `refund_breakdown {gratuity_cents, excess_cents, fee_cents}` (all cents; math in `server/utils/cancellationMath.js`), affected `staff[]`, `comms_halted[]`, `email_preview`, and `blocking[]` (already_archived / completed / not_booked / autopay_in_progress). |
| POST | `/:id/cancel` | Admin/Manager | Execute the cancellation. Body `{ mode: 'client'\|'drb', note, suppress_client_email, suppress_staff_notifications, confirm_last_name }`. Server-validates the typed last name (422 `LAST_NAME_MISMATCH`); requires a booked status (409 `ALREADY_ARCHIVED`/`ALREADY_COMPLETED`/`NOT_CANCELLABLE`); 409 `AUTOPAY_IN_PROGRESS` when a balance charge is mid-flight. One transaction: proposal → `archived` + `archive_reason` (`client_cancelled`/`we_cancelled`) + `cancelled_at`/`cancelled_by`/`cancellation_note`; linked shifts cancelled (shift_requests denied, BEO/shift comms suppressed); pending proposal-level `scheduled_messages` deleted; unpaid invoices voided; audit note + activity log. Post-commit tail: idempotent tip clawback (`clawbackTipsForCancelledProposal`, marker-coordinated with the `charge.refunded` webhook), marketing/change-request reaps, client cancellation email + staff shift-cancel notices (both suppressible; email over SMS). |
| POST | `/:id/cancel/refund` | Admin ONLY | Issue the agreement refund after a cancellation. Recomputes the target from `cancellationMath` (mode from `cancelled_by`, daysOut fixed at `cancelled_at`), caps at the refundable remainder, loops `server/utils/refundExecute.js` per refundable charge largest-first (deposit + balance) until met, attributing the gratuity portion via `proposal_refunds.gratuity_cents`. Idempotent (already-refunded caps the target; Stripe idempotency key per charge). Body `{ idempotency_key }`.  Honors the cancel dialog's `suppress_client_email` for the refund notice too (one checkbox, whole flow). |
| GET | `/:id/cancel-line/targets` | Admin/Manager | Enumerate cancellable pricing lines as typed targets (`addon:<slug>`, `bar`, `syrup:<id>`, `extra-bartender`, `adjustment:<idx>`, `gratuity`; `package` listed non-cancellable → cancel-event flow). `server/routes/proposals/cancelLineItem.js` + `server/utils/lineItemCancel.js`. |
| POST | `/:id/cancel-line/preview` | Admin ONLY | Server-computed removal preview: runs the SAME core as execute inside a rolled-back tx (old/new totals, overpayment cents, refund split incl. `manual_return_cents` for external money, locked-invoice notes, staffing warning, gratuity jar/notice flags, `fingerprint`). |
| POST | `/:id/cancel-line` | Admin ONLY | Execute: fingerprint-guarded (409 `STALE_PREVIEW`), commits the removal (fold-based contract-safe reprice, invoice refresh + delta-invoice reconcile, shift sync, lab-selection cleanup), THEN fires sequential per-charge refunds via `refundExecute` with `total_scope='overpayment'` (never re-lowers the folded total). Body `{ target, quantity?, new_rate?, reason, idempotency_key, notify_client, fingerprint }`. |
| GET | `/t/:token/resolve` | Public | Non-mutating: whether this proposal is one option in a comparison group + whether the group is decided, so `ProposalView` can redirect to `/compare/:token` without bumping `view_count` or flipping `sent->viewed` (which the full `/t/:token` GET does). **Deliberately status-blind, unlike `/t/:token`:** a GROUPED losing option is archived `option_not_chosen` with `chosen_proposal_id` set in the same transaction, and this is what redirects that client to the option they actually chose. Adding an archived filter here would strand exactly the clients the redirect exists to rescue. Note the scope: `sweepClientAlternatives` writes the same `option_not_chosen` reason on a client's other UNGROUPED quotes at first payment, and those correctly fall through to the not-found page. Pinned by `publicToken.archived.test.js`. Limiters: `requireUuidToken` runs first, then `publicTokenIpLimiter` (300 per 15 min, keyed by IP) as a ceiling on the client-supplied token key, then the token-keyed `proposalCheckoutLimiter` (60 per 15 min), which this route was moved onto off the shared IP-keyed `publicLimiter` on 2026-08-28 so that browsing can never spend the budget paying depends on. |
| GET | `/t/:token/payment-state` | Public | Non-mutating: the post-checkout settle poll's target. Returns `{ status, amount_paid, total_price, payment_type }` in DOLLARS and touches nothing, no `view_count` bump, no `last_viewed_at`, no `sent->viewed` flip, no activity row, so a thirteen-attempt poll cannot record thirteen views of a page the client is staring at once (the full `/t/:token` GET above does all four on every call). 404 on an unknown token and on an `archived` proposal, stated in its own SQL rather than inherited; `/t/:token/resolve` above stays deliberately status-blind. Limiters: `requireUuidToken` runs first, then `publicTokenIpLimiter` (300 per 15 min, keyed by IP), then the token-keyed `proposalPollLimiter` (40 per 15 min), its own bucket so a settle can never spend the checkout's budget. |
| GET | `/group/:token` | Public | Compare-page data for an option group: shared event header + per-option public-safe fields (`package_id`/name/slug/category/pricing_type/total/deposit + `floor_reason`/`billed_guests`/`floor_applied` extracted from the STORED pricing_snapshot). The compare matrix renders the stored totals and floor fields as-is, never a live reprice (live `POST /public/calculate` pricing belongs to the quote wizard; the proposal page's own comparison prices through `POST /t/:token/options` instead). On `publicReadLimiter` (100/15min; read-only, shares the client's IP budget with proposal loads). 404 while every option is an unsent draft. Positive-allowlist projection (no admin_notes/stripe/signature/PII). |
...
| POST | `/t/:token/options` | Public | Priced option set for the proposal's "see other options" panel (`proposals/publicOptions.js`). Body carries only a SELECTION (`tier_addon_id`, `extra_addon_ids`); every pricing input — guests, hours, bars, adjustments, gratuity floor — is read off the proposal row, never the request. Prices each active non-class package (plus the proposal's own, even if retired) through `priceProposedState` with a pre-fetched catalog, so the quote equals what a commit would compute; a rule violation is a per-option `available:false` + reason rather than a failed request. Returns `comparable:false` with no options for a `total_price_override` proposal (never re-price a negotiated number) or an already-signed one. Add-ons are gated by `visibleAddonsFor` per option, so a byob-only extra is never priced onto a hosted column. Read-only despite the verb, but on its OWN token-keyed `optionsQuoteLimiter` (120/15min) rather than the shared IP-keyed `publicReadLimiter` — browsing re-quotes on every extras/tier toggle, and exhausting the shared bucket would make the next page load report the proposal as expired and stop the Stripe payment form mounting. |
| POST | `/t/:token/switch` | Public | The drawer's "Switch to this package" (`proposals/publicSwitch.js`): the ONE pre-signature write behind the options panel. Body carries a configuration (`package_id`, `tier_addon_id`, `extra_addon_ids`) plus `acknowledged_total` (dollars, compared in integer cents); every pricing input is read off the FOR UPDATE row. Guards on the locked row: status whitelist `sent`/`viewed`, no `total_price_override`, unsigned, `amount_paid = 0`, not grouped-undecided; pending Stripe intents are canceled server-side first and a `processing`/`succeeded` intent refuses with `PAYMENT_IN_FLIGHT`. On an engine total that differs from the acknowledged cents: 409 `TOTAL_CHANGED` carrying a fresh quote (built by the SAME `buildOptionsQuote` that serves `/options`), nothing written. On match, in one transaction on one pooled connection: proposal row (package/total/snapshot, `num_bartenders` synced to priced staffing; never tip_jar/gratuity fields/deposit_amount), addon rows replaced from engine output, payment-status reconcile, `refreshUnlockedInvoices`, and a `package_switched` activity-log row (old/new package + totals + dropped add-ons + IP). Responds with the same public payload as GET `/t/:token`. `requireUuidToken` runs first, then `publicTokenIpLimiter` (300 per 15 min, keyed by IP) as a ceiling on the client-supplied token key, then the token-keyed `switchLimiter` (20/15min). Pricing identity shared with the quote via `proposals/optionsPricingShared.js`. |

### Bar menu print file (admin) — `/api/proposals`
Mounted from `server/routes/proposals/menuPrint.js`. Staff print the bar menu and bring it framed; this is where admin posts the file or declares that no printed menu is needed. All three return `{ menu_print: { status } }` with the derived tri-state.

| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/:id/menu-print` | Admin/Manager (`adminWriteLimiter`) | Upload or replace the print file (multipart field `file`). Accepts PDF / PNG / JPEG **by magic bytes** (the declared filename and content-type are not trusted); the sniffed type also chooses the stored extension. Size capped by `MAX_FILE_SIZE` (`abortOnLimit`). Stored at `menu-print/<proposalId>/<uuid>.<ext>`; the previously stored object is orphaned (no delete in `storage.js`, mirroring the drink-plan logo). Clears `menu_not_required` so the tri-state cannot contradict itself. |
| PATCH | `/:id/menu-print` | Admin/Manager (`adminWriteLimiter`) | Body `{ not_required: boolean }`. Setting it true while a file is posted returns 409 (remove the file first) — enforced inside the UPDATE's WHERE clause, not read-then-check, so concurrent admins cannot land the contradictory state. |
| DELETE | `/:id/menu-print` | Admin/Manager (`adminWriteLimiter`) | Clears `menu_print_key` (R2 object orphaned); status is reported from the `RETURNING` row. |

### Remote Staffing Fee (admin send surfaces) — `/api/proposals`
Mounted from `server/routes/proposals/remoteStaffing.js` (carved out of `crud.js` at the size ratchet, mounted before the greedy `/:id` in `getOne.js`). Answers "is this venue far enough out, and thin enough on nearby staff, that the client should be charged a Remote Staffing Fee?" for the two admin send surfaces. Read-only plus a one-flag stamp; the fee itself lands as an ordinary `adjustments` surcharge through the existing editor PATCH, never a new money path.

| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/:id/remote-staffing-check` | Admin/Manager | Venue distance from `HOME_BASE`, active-staff counts, and the server-derived `suggested_fee_cents`. Geocodes the venue ON DEMAND when `venue_lat` is NULL and a street address exists (throttled; street-less venues are never geocoded, so `venue_distance_miles` stays null and the popup is disabled). Returns `{venue_distance_miles, staff_within_40, staff_uncounted, suggested_fee_cents, prompted, accepted, should_prompt}`. `should_prompt` owns the whole rule server-side (coords present, >= 40 mi out, fewer than 3 active staff within 40 mi, not already answered, proposal not yet accepted) so the client renders a decision instead of making one — the suggestion bands live only in `server/utils/serviceArea.js`. |
| POST | `/:id/remote-fee-prompt-answered` | Admin/Manager | Stamps `proposals.remote_fee_prompted_at`. Every choice (add suggested, add custom, send without) stamps it, so a repeat send never re-asks a decision the admin already made. |
...
| GET | `/:id/change-requests` | Admin/Manager | Per-proposal change-request history (pending, approved, declined, cancelled) — drives the `ProposalChangeRequestCard` on Proposal Detail. Also returns `package_names` (`{id: name}` for every package_id appearing in a diff, inactive included) so the card renders package names, not raw ids. |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/create-intent/:token` | Public | Create Stripe PaymentIntent (deposit or full amount, with optional autopay). `server/routes/stripeCreateIntent.js`. **Writes no gratuity to the proposal** (election-at-payment 2026-08-03): the client's tip-jar election is computed in memory, charged, and stamped into the intent's `tip_jar`/`gratuity_rate` metadata for the webhook to apply on success. It does still stamp payment preferences (`payment_type`, `autopay_enrolled`, and a default `balance_due_date`) plus the `stripe_sessions` row. Intent identity is (amount, election metadata), so an election-bearing pending intent is never reused and a stale one is cancelled. Limiters: `requireUuidToken` runs first, then `publicTokenIpLimiter` (300 per 15 min, keyed by IP) as a ceiling on the client-supplied token key, then the token-keyed `proposalCheckoutLimiter` (60 per 15 min), which this route was moved onto off the shared IP-keyed `publicLimiter` on 2026-08-28 so that browsing can never spend the budget paying depends on. |
...
| POST | `/refund/:id` | Admin (`auth, adminOnly`) | Issue a partial refund — auto-targets the largest refundable charge; no cross-charge spanning. Body `total_scope`: `'contract'` (default; Approach A, the refund corrects the total) or `'overpayment'` (returns money held beyond the contract and moves neither `total_price` nor `total_price_override`). An overpayment refund is capped at the netted overpayment MINUS any pending overpayment refund, asserted under the proposals row lock in the same transaction that writes the pending row (409-free: 400 `REFUND_EXCEEDS_OVERPAYMENT`), and must land on a charge carrying enough UNCREDITED headroom (400 `OVERPAYMENT_NOT_ON_A_CHARGE`) so it cannot reverse invoice credits while the contract stands. Unknown scope is 400 `INVALID_SCOPE`. Client refund notice is **opt-in** via `notify_client: true` (suppression + `.invalid` placeholder gates apply; the archived-status gate deliberately does not); response carries `notifications`. |
| GET | `/refunds/:id` | Admin/Manager (`auth, requireAdminOrManager`) | Refund history for a proposal, including each row's `total_scope` |
| POST | `/webhook` | Stripe | Handle `payment_intent.succeeded`, `refund.created` (refund reconciliation, spec 2026-09-15: the Refund object is the payload, so it survives the account API version that stripped the refunds list off the Charge; scope comes from the named pending row, else Stripe's `duplicate` reason or the netted overpayment), `payment_intent.processing` (bank debit in flight, spec 2026-09-14: records `processing` on `stripe_sessions`, one activity row, client and admin emails), `payment_intent.payment_failed`, `checkout.session.completed`, `charge.refunded` (tip clawback only), dispute events, and `payout.paid`/`payout.failed` (read-side payout mirror, livemode-gated, catch-and-ack). Structure: `stripeWebhook.js` is signature-verify + dispatch only; per-event handler bodies live in `server/routes/stripeWebhookHandlers/`. `paymentIntentSucceeded` discriminates a service-extension payment by the `service_extensions.invoice_id` lookup: it skips the `amount_paid` roll-up (off-ledger label) and, post-commit and post-release, runs the settle tail (`settleExtension` then payroll then staff greenlight then `finalizeExtension`); a payment landing after the request already closed alerts admins for a refund instead of extending the event. |
| POST | `/create-intent-for-invoice/:token` | Public | Create a Stripe PaymentIntent for an open invoice (balance / Additional Services / Service Extension), used by the public token-gated InvoicePage (handler lives in `server/routes/stripe.js`). Extension invoices are gated server-side: 409 unless the linked `service_extensions` row is still `pending`, unexpired, and carries a recorded terms acceptance (`EXTENSION_TERMS_NOT_ACCEPTED`), so a client who routes around the terms page cannot pay with no acceptance artifact. Carries no gratuity metadata, so an invoice payment never writes the proposal's gratuity. |
...
| PATCH | `/:id/out-of-area` | Staffing | Attach, raise, lower, or clear the Out-of-Area Bonus. Body `{ amount_cents: int\|null }`, integer cents, `0 < x <= 25000` (the DB CHECK carries the same bound); null clears an unlocked bonus. Once the bonus is LOCKED (a staffer was approved while it was attached) the amount may only go UP: any reduce or clear is a 409. Attaching to an already-staffed shift auto-locks when there is exactly one approved non-dropped worker; with 2+ it stays unlocked and every shift payload carries `unlocked_warning`. A same-value save that cannot stamp a lock is a true no-op: 200 with the current payload, no UPDATE, no audit row, no re-derivation. Every real mutation (amount change, or a same-value save that stamps the lock) audit-logs (`shift_out_of_area_set`) and re-derives duty lines post-commit. |
...
**Shift visibility asks "has it finished", never "what day is it"** (spec `docs/superpowers/specs/2026-08-14-shift-lifecycle-design.md`). A shift's END INSTANT is defined exactly once, in `server/utils/shiftEndInstant.js`, as a SQL fragment every consumer imports:

- `end_time` parseable → `(event_date + end_time::time) AT TIME ZONE tz`, rolled to the next day on an overnight wrap (an 8pm-1am booking really does end the following morning). The wrap is decided against `start_time` when it parses, and against `WRAP_CUTOFF_HOUR` when it does not — no shift legitimately *ends* in the small hours of its own event date without having started the evening before;
- only `start_time` parseable (`end_time` NULL or malformed) → `start_time + the proposal's event_duration_hours (or ASSUMED_HOURS) + OVERRUN_GRACE_HOURS`. The end is **assumed, not deferred to midnight**. Deferring it was the lane's own P1 and a regression against the code it replaced: a 10am brunch with no `end_time` stayed "not finished" until midnight, outranked tomorrow in `findNearestApprovedShift`, and a CANT text meant for tomorrow denied and re-opened the shift the staffer had already worked. The grace is what keeps a shift running long from vanishing mid-service;
- neither parseable → end of that calendar day in `tz`. This last rung is the **fail-safe-toward-VISIBLE** one: hiding a shift too early is the bug, so a malformed time falls back rather than raising;
- `tz` = the proposal's `event_timezone`, `America/Chicago` when the shift has no proposal.

`shiftNotFinishedSql()` (`> NOW()`) replaces every `event_date >= CURRENT_DATE` in the family: `STAFF_OPEN_SHIFTS_SQL`, `GET /shifts/unstaffed-upcoming`, `badge-counts.unstaffed_events`, all five shift queries in `GET /api/me/staff-home` (next shift, pending, cover broadcasts, open-shifts teaser, "All (N)" count), `GET /api/messages/shifts`, `outstandingDocuments.listUncertifiedStaffable`, and `smsInbound.findNearestApprovedShift`. **A COUNT and the LIST it counts must import the same fragment** — the badge/modal pair and the teaser/count/Available-tab trio drifted apart twice, each side carrying a comment claiming it mirrored the other, so the pairing is asserted by test (`shifts.visibility.endInstant.test.js`, `staffPortal.staffHome.endInstant.test.js`). Two things stay OUT: `autoAssignScheduler.js` keeps `CURRENT_DATE` because it is an ACTION trigger, not a visibility predicate (converting it moved staff auto-approval SMS to just after midnight Chicago with no quiet-hours gate), and marketing/payroll/calendar windows measured in months or ±days, where a one-day shift is immaterial — the payroll member being `payrollAccrual.js`'s 21-day proposal-recency guard (`maybeReaccrueForDuty`), which is a lookback window and not a "which period is now" question.

**The pay-period LOOKUP was a separate case, and it cost a real defect (2026-08-19).** A pay-period lookup is a DATE boundary, not a shift-visibility one, so no end-instant applies — but it is not immaterial: `GET /api/me/staff-home` selected the current period with `CURRENT_DATE`, the UTC day, so from 19:00 Chicago on a period's LAST evening it resolved the NEXT period while accrual was still writing the current one, and — because `pay_periods` rows are minted lazily by accrual 0 to 4 days INTO their own week — tomorrow's row did not exist, so it matched **zero rows** and the card fell back to the new-hire empty state (*"No payouts yet. Your first payout will appear here after your first shift."*). A months-tenured contractor was told they had never been paid, five hours every period (six in winter). It now passes `chicagoTodayYmd()` as a parameter, matching the rest of the payroll "which period is now" family (`payrollProcessing.findOpenPeriodForDate` via `dutyLines.js`, `admin/payroll.js`). Pinned by `staffPortal.payPeriod.boundary.test.js`, which feeds the shipped predicate the Chicago day and the UTC day and requires them to select different periods — a clock-derived test cannot discriminate here, because the two agree for ~19 hours a day. The Staff hub's subtitle joins that family from the READ side (2026-08-19): `server/utils/staffHubSummary.js` (`summarizeOpenPeriod`, pure) derives the window from `chicagoTodayYmd()` through `payPeriodForDate` and only then LEFT JOINs the `pay_periods` row, so a mid-week request whose row has not been minted yet still names the right dates instead of going blank or falling back to last week.

**The rest of the family: the remaining bare `CURRENT_DATE` reads (2026-08-25).** The end instant answered every SHIFT-visibility question, and the pay-period lookup answered the payroll one, but the sweep stopped there — nine live sites still compared a date column to bare `CURRENT_DATE`, which resolves in the GMT session zone. Each now binds `chicagoTodayYmd()` as a parameter, the same shape the payroll paths use:

- **`contractor_profiles.hire_date`**, stamped in three places (`utils/contractorSeed.js` twice, `routes/admin/users.js` once for the rare no-application direct hire). This one WRITES, and nothing recomputes it: a contractor hired on a weekday evening carried a hire date one day in the future forever, and `GET /users/:id/seniority` derives tenure from it, which orders the roster.
- **The client portal's upcoming/archive split** (`routes/clientPortal.js`, three compares). A client opening their portal on the evening OF their event saw it gone from the focus card and sitting in the archive. The three now share ONE bound value, because they are complements and independent evaluation can drop or duplicate a row. Note the deliberate divergence from the shift rule above: a calendar day IS right here. The end-instant argument is about a write path — a CANT text re-opening a shift somebody already worked — and a client's own portal has none.
- **The autopay claim** (`utils/balanceScheduler.js`). `balance_due_date <= CURRENT_DATE` claimed a balance due TOMORROW during the evening window and charged the card, up to five hours before the date the client agreed to, unattended.
- **`utils/autoAssignScheduler.js`** (the window opened a day early), **`utils/autoAssign.js` + `GET /users/:id/seniority`** (tonight's shift counted as already worked, inflating a ranking — the two are documented mirrors), **`utils/payrollAccrual.js`** (the 21-day re-accrual guard), **`utils/retentionEligibility.js`** (a client booked TONIGHT read as having no upcoming event, so the retention nudge could fire at them), and **`routes/calcom.js`** (tomorrow's date in an auto-created client's note text).

Three sites are deliberately LEFT on `CURRENT_DATE`, and this is a judgement, not an oversight: `routes/calendar.js` (a ±30 and ±365 day window), `utils/marketingAudience.js` (11-to-13-month lapsed windows), and `scripts/staffPaymentImport/generateSeniorityMapping.js` (a one-off import already run). A one-day shift is immaterial at those scales.

Pinned by `contractorSeed.chicagoDay.test.js`, `clientPortal.chicagoDay.test.js` and `balanceScheduler.chicagoDay.test.js`. All three inject the day through a `_deps.today` seam rather than deriving it from the clock, for the reason the pay-period suite already documents: the UTC and Chicago days agree for ~19 hours out of 24, so a clock-derived fixture is green for most of the day against the very bug it exists to catch. The autopay suite seeds `amount_paid == total_price` so `balanceCents <= 0` returns before any Stripe call — this box talks to live Stripe — and its first test is an explicit non-vacuity guard, because `processAutopayCharges` returns early with no Stripe client and every "not charged" assertion would otherwise pass without the SQL running.

**`shifts.status = 'cancelled'` means the EVENT was cancelled — closure must never write it.** The value is overloaded: `client/src/components/adminos/shifts.js` `isCancelledEvent` renders a Cancelled chip off it on the admin `GET /shifts` feed, and `server/routes/calendar.js` turns it into `STATUS:CANCELLED` with a bumped `SEQUENCE`, which strikes the event off the owner's subscribed Google Calendar. Its intended producers are the genuine event-cancel paths (P6 cancel, `shiftReap.js`, `cancel-or-unassign` mode='cancel'). There **was** a fourth, unintended writer: `PUT /shifts/:id` (`server/routes/shifts.handlers.js`) destructured `status` off `req.body` with no allowlist — unlike `equipment_required` / `supply_run` validated a few lines below — and wrote `status = COALESCE($9, status)`. **CLOSED 2026-08-19: the handler now rejects any `status` in the body with a 400.** Two corrections to how this was recorded. (1) "an arbitrary status" overstated it once `shifts_status_check` was re-armed: an out-of-list value raises 23514 and 500s. The real exposure was narrower and worse — a PUT could stamp a **legal** `'cancelled'` on a delivered event, which is exactly the overload described above, so the Events chip and the iCal `STATUS:CANCELLED` would both fire on a shift nobody cancelled. (2) It was rejected outright rather than allowlisted to the four legal values, because nothing sends it: the only two client callers of this route send `equipment_required` and `supply_run`, and every legitimate transition already has an owner (`shiftReap.js` and cancel-or-unassign for `cancelled`, `staffShiftActions.js`/`smsInbound.js` for `open`, the closure sweep for `completed`). Pinned by `shifts.statusGuard.test.js`, whose fourth test asserts cancel-or-unassign still works — a guard that also disarmed the real cancel path would be worse than the bug.

**Closure is a SWEEP, not a completion hook.** `server/utils/shiftClosureSweep.js` runs hourly (`RUN_SHIFT_CLOSURE_SWEEP_SCHEDULER`) and closes a shift when the SHIFT has finished **and** its proposal is `completed`. It is deliberately not hung off the two proposal-completion doors (`balanceScheduler.processEventCompletions`, `PATCH /api/proposals/:id/status`): `shifts.event_date` is independent of `proposals.event_date`, so such a hook needs a date gate, and those doors never re-select an already-completed proposal — a shift the gate skipped was skipped forever. The sweep has no such hole. Terminal status is **always `completed`**, worked or not: a shift nobody was rostered on is an unrecorded ROSTER, a data gap, not a cancellation, and prod holds real paid-in-full events where the owner worked the bar himself. `server/scripts/backfillShiftClosures.js` runs the same query as a dry-run-default report and names those as ROSTER GAPs. `shiftClosureSweep.test.js` pins all of it.

**Losing quotes close out too.** `server/utils/staleProposalSweep.js` runs hourly and archives a proposal whose event date passed more than 48 hours ago while it was still in `draft`/`sent`/`viewed`/`modified`, writing `archive_reason = 'event_passed'` (a value the client portal deliberately does NOT surface, unlike `event_completed`). It reuses `voidUnpaidProposalInvoice`, `reapShiftsForProposal`, the pending-message delete and the post-commit tail from `POST /proposals/:id/archive` so the two archive doors cannot drift. Its date expression uses `AT TIME ZONE event_timezone` and never reads the free-text `event_start_time`.

**`accepted` is exempt, and that exemption is the money guard.** `reconcileProposalPaymentStatus` demotes a fully refunded proposal to `accepted`, so such a row has `amount_paid = 0` and a real payment history; sweeping it would stamp a refunded booking as a lead that never booked. Past-dated `accepted` proposals are emailed to an admin instead, once each, sending before marking so a Resend quota failure cannot silence the notice permanently.

Unlike every sibling, the scheduler is **opt-in** (`RUN_STALE_PROPOSAL_SWEEP_SCHEDULER=true` exactly, not the default-on `enabled()` helper): its first run voids about 105 invoices and makes about 220 live Stripe calls, and `RUN_SCHEDULERS=true` is a documented local pattern. `STALE_PROPOSAL_SWEEP_DRY_RUN=true` logs both candidate lists and writes nothing. A runaway bound (`MAX_ARCHIVES_PER_RUN = 200`) archives nothing and emails an admin if a tick ever selects an implausible number, and the run rethrows on any per-row failure so `wrapScheduler` records `failed` rather than reporting green through a systemic break.
...
| POST | `/payroll/payouts/:id/mark-paid` | Admin | Record an external payment: `{ payment_method, payment_handle, payment_reference?, expected_total_cents? }`. `zelle` is an allowed method. `expected_total_cents` (non-negative integer) is the drift guard: compared against `total_cents` under the row lock, 409 on mismatch so a stale generated QR/link can never be recorded. 409s a `no_draw` payout (convert to pending first). |
| POST | `/payroll/payouts/:id/toggle-draw` | Admin | Flip a payout `pending` ⇄ `no_draw` (owner rows: tracked, never owed — spec 2026-08-07). Row-locked like mark-paid; refuses paid payouts, paid periods, and PARKING anyone whose `contractor_profiles.takes_draw` is true (the flag is the server-side authority; un-parking is never gated). Parking the LAST pending payout of a `processing` period is an API-irreversible close, so it 409s unless the body carries `{ confirm_finalize: true }`; with it, the park finalizes the period (same check as mark-paid). Audit-logged as `payout_toggle_draw`; audit runs after client release (one-connection rule). Returns `{ payout, period_status }`. |
| POST | `/payroll/duty-lines` | Admin | Create a manual duty line `{ payout_id, kind, amount_cents, shift_id?, note? }` (`origin='admin'`, admin-owned; EVENT kinds only — review money flows exclusively from the reviews surface so its idempotency keys exist; nonzero integer cents within +/-100000; shift must sit in the payout's period; NULL-shift duplicates explicitly refused; frozen when payout/period paid or processing). Every duty mutation audit-logs and recomputes the payout total via the single writer. (`server/routes/admin/payrollDuty.js`) |
| PATCH | `/payroll/duty-lines/:id` | Admin | Edit amount/note; an AMOUNT edit stamps `admin_owned` (derivation never overwrites) and may set $0 (distinct from removal); any PATCH confirms a held line; note-only edits leave `admin_owned` alone. |
| POST | `/payroll/duty-lines/:id/remove` / `.../restore` | Admin | The pay-time checkbox. An admin removal is FINAL to derivation (never resurrected); restore returns the stored amount. |
| PUT | `/payroll/duty-attributions` | Admin | Set/move who gets an attributed duty (`bar_rental` / `equipment_supplies` / `hosted_supplies` / `menu_print`). Validates against the current approved roster, then re-derives via accrual so a first-time attribution materializes its line and a move re-homes the money with both totals recomputed. |
| GET | `/payroll/periods/:id/unattributed-duties` | Admin | What blocks Process: missing attributions plus stale ones (attributed user off the roster), with display names + proposal labels for the attribution modal. The process endpoint 409s (plain message) while this list is non-empty. |
| GET/POST/PATCH | `/staff-reviews`, `/staff-reviews/:id`, `/:id/confirm`, `/:id/dismiss` | Admin | Review log (spec 2026-08-06 §7): TT-webhook-ingested + manual google rows, credit tagging, confirm materializes the $10 `review_bounty` per credited 5-star staffer (and clears system removals so dismiss-then-reconfirm re-pays), dismiss refuses while a bounty is paid. All mutations audit-log. The LIST envelope also carries `bounty_cents` (the exported `REVIEW_BOUNTY_CENTS`, so no client holds a money literal), `bounties_paid_cents` (all-time sum of `review_bounty` duty lines with `removed_at IS NULL`, the payable filter) and `total_logged` (all-time row count; the list itself is capped). (`server/routes/admin/staffReviews.js`) |
| GET/POST | `/staff-reviews/leaderboard?quarter=` / `/staff-reviews/contest-award` | Admin | Quarterly contest: rate = confirmed named 5-stars over events worked (approved + not dropped, event date in quarter, bounded at today), floor 4 events + 2 reviews, server-computed winners/shares; award splits the $100 pot, idempotent via `UNIQUE(contest_quarter, contractor_id)`, refuses an in-progress quarter without `force`. (`server/routes/admin/staffReviewsContest.js`, mounted before the log router) |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/opt-in` | **public, none** (`publicLimiter`) | Submit for the standalone `/sms` opt-in page (`server/routes/smsOptIn.js`, mounted ahead of `sms.js`). Requires name + email. `sms_consent` is OPTIONAL and the phone is required only when it is ticked (Twilio forced-consent rule, 2026-08-03). Ticked: records via `recordSmsConsent` with `source_form='sms_page'`, and rolls the whole submit back rather than leave a created client SMS-on without a recorded opt-in. Ticked also 503s `SMS_CONSENT_VERSION_SKEW` on an unknown consent version. Un-ticked: upserts `email_leads` only, never touches `clients`, ignores any number given, and can never 503 (it resolves no version). Same generic 200 for every consent outcome (no client/opt-out enumeration). |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/inbound` | Twilio signature, **fail-closed in every env, signature BEFORE the limiter** | Client dials the 0082 → returns TwiML `<Dial timeout="20" action="…/inbound/missed?line=zul" callerId="<caller>"><Number>VA_CELL</Number></Dial>` forwarding to Zul's cell (caller = `req.body.From`, constrained to bare +digits or `VOICE_CALLER_ID`). |
| POST | `/inbound/primary` | Twilio signature, **fail-closed in every env, signature BEFORE the limiter** | Client dials the 1922 (the primary business number) → `<Dial timeout="VM_PRIMARY_RING_SEC" action="…/inbound/missed?line=primary">` to `VM_PRIMARY_DIAL_TARGET` (env-only, strict E.164; unset → apology TwiML + throttled Sentry page, never an empty `<Dial>`). Own limiter bucket so a storm on one line cannot silence the other. |
| POST | `/inbound/missed` | Twilio signature, **fail-closed in every env** | The shared `<Dial>` action callback, keyed on the HMAC-covered `?line=`. On a missed `DialCallStatus` it checks `VM_DAILY_CAP`, claims a `voicemail_delivery` row (the ping dedup, stamping `line`), returns the PER-LINE greeting (wrapped in a press-1 `<Gather>` whenever a digit could arrive, which is a DAY-ONLY question: `!night && (escalation is on OR the day greeting itself says "press one")`, and both 2026-08-19 day recordings do. NIGHT never gathers, switch on or off, and nothing validates a night-greeting override — a `VM_NIGHT_GREETING_URL*` pointed at a recording that mentions press 1 would reproduce the trap this closed for day — the `<Gather>` follows the GREETING, not the switch, so flipping `VM_ESCALATION_ENABLED` off can never leave callers pressing a key nobody catches) + `<Record>`, then pings Zul AFTER the response on ZUL-LINE misses only (a primary miss shows natively on the GV dial target). Any other status, or `VOICEMAIL_ENABLED` off, returns a bare `<Hangup/>`. An instant-answer on the primary line is logged by the interception canary (LOG-ONLY since 2026-08-14 — the per-call `dialSec` console line; the Sentry page was removed as inverted, see voice.js's canary block). |
| POST | `/inbound/voicemail` | Twilio signature, **fail-closed in every env** | The `<Record>` `recordingStatusCallback` (not `action`: Twilio skips that when the caller hangs up). Claims delivery (reading back `line`), then answers 204 and works detached: zul → fetch mp3 (constructed URL, never the body's `RecordingUrl`) + Telegram upload + delete on `ok === true`; primary → SMS the caller's number/duration **plus a listen link** (`/api/voice/vm/<listen_token>`, omitted when `VM_LISTEN_LINK_ENABLED=false`) to `VM_TEXT_DESTINATION`. The recording is RETAINED: the link streams from Twilio, so deleting would break every link already sent as well as destroying the only copy until 1b's R2 mirror. Failure alerts go to the LINE'S owner via `alertOperator`. |
...
| GET | `/audio/:name` | **public, none** | Static voicemail audio for Twilio `<Play>` (`voiceAssets.js`, mounted before `/api/voice`). `:name` is a KEY into a frozen allowlist, never a path, so no value can traverse out of `server/assets/`; an unknown key is a 404 that tells a prober nothing. Serves Dallas's four 2026-08-19 recordings plus Zul's 2026-07-24 greeting (also reachable at its original `/greeting.mp3` path). Rate limited 300/min per IP, because Twilio fetches these mid-call and a saturated origin means a live caller hears silence. |
| GET | `/greeting.mp3` | **public, none** | Serves the recorded voicemail greeting mp3 (bundled `server/assets/voicemail-greeting.mp3`) that `/inbound/missed` `<Play>`s. Unauthenticated by necessity — Twilio fetches it with no credentials or signature. Static asset, no DB, no caller input. Overridable / kill-switchable via `VM_GREETING_URL`. |

### Voicemail listen link — `/api/voice/vm` (mounted before `/api/voice`)
| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/:token` | **public, token-gated** | Streams the retained primary-line recording as `audio/mpeg` so the alert SMS link plays on a phone. The UUID token is the auth; the RECORDING SID is read from the row and never accepted from the request, so this cannot become an open proxy into the Twilio account. `requireUuidToken` runs first (a non-UUID 404s before the DB, no 22P02). Supports `Range` with `206`/`416` because iOS Safari will not play without it. One byte-identical `text/plain` 404 for every miss (unknown token, non-primary line, recording gone, switch off) so nothing can be enumerated; `502` for a DB or Twilio failure, never a 404. `no-store`, `noindex`, `Cross-Origin-Resource-Policy: same-origin`. Links expire at 30 days via the route's OWN SQL predicate, deliberately not borrowed from the row prune (which retains `recorded`/`failed`/`skipped` rows past retention, because their audio is still undelivered at Twilio, so a token on one of those rows would otherwise never expire). Kill switch `VM_LISTEN_LINK_ENABLED` (default ON) is TWO-SIDED: it closes this route and drops the link from the SMS. Revoke ONE link by rotating `listen_token`. |

### Press-1 Escalation — Twilio Voice — `/api/voice/escalate` (mounted before `/api/voice`)
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/` | Twilio signature (fail-closed in EVERY env) | The missed-greeting `<Gather action>`, reached only on a real digit. `1` + `VM_ESCALATION_ENABLED` + a shape-valid `CallSid` → env-only target for the OTHER person → quiet-window check → `claimEscalationUnderCap` (advisory-lock serialized cap+claim, a HARD `VM_ESCALATION_DAILY_CAP` bound; a capped/lost claim records and never dials) → billed `<Dial>` with a `<Number url=…/whisper>` keypress screen. Anything else → straight to `<Record>`. Skip outcomes (`skipped_no_target`/`skipped_quiet`/`skipped_cap`) are written AFTER the response. |
| POST | `/whisper` | Twilio signature (fail-closed in EVERY env) | TwiML played to the ANSWERING party only: `<Gather numDigits="1" finishOnKey="" action=…/accept?sid=PARENT>` + `<Hangup/>`. The keypress screen is what stops an unattended carrier voicemail from ever bridging; a malformed parent sid hangs up outright. |
| POST | `/accept` | Twilio signature (fail-closed in EVERY env) | The whisper's action: fires only on a real keypress, marks the PARENT call accepted (`escalation_accepted_at`, first-writer-wins, requires a claimed row), returns an empty `<Response>` which bridges the legs. Runs on the CHILD leg, hence the `?sid=` round-trip (HMAC-covered, `isCallSid`-gated both ways). |
| POST | `/done` | Twilio signature (fail-closed in EVERY env) | The outer `<Dial action>`. Branches on ACCEPTANCE STATE, never `DialCallStatus` (a screened-out machine still reports `completed`); not-accepted → back to `<Record>`, the outcome write runs after the response. Fails closed toward the caller: any doubt offers voicemail. |
...
### Consult Call Bridge — Twilio Voice — `/api/voice/consult`
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/answer` | Twilio signature (fail-closed in EVERY env) | Agent leg answered (Dallas/Zul). Returns `<Gather numDigits="1" timeout="10">` wrapping the spoken consult briefing (`buildConsultBriefing`, xml-escaped: booker name, slot time, ring wording, and the linked proposal's event date + guest count), one automatic repeat, then `<Hangup/>`. A row that is not waiting on THIS leg and ring gets the apology, never a 500 (`legMatches`, ruling R5). Query: `attempt`, `leg` (admin\|va), `ring` (0 for the Zul leg), `play`. |
| POST | `/digit` | Twilio signature (fail-closed in EVERY env) | Gather action. `1`: validate the dial target, re-check that the consult is still scheduled (a cancelled one speaks the cancellation and does NOT claim), then the guarded claim to `connected` (binding `admin_ring` on the admin leg, ruling R9) and `<Dial answerOnBridge="true" callerId="CONSULT_CALLER_ID">` (the 1922 for Dallas; the 0082 for Zul) `<Number statusCallback=…>toUsE164(booker_phone)</Number></Dial>`. Validate-before-claim is the point: a claim that then failed to dial would strand the row `connected`, which is terminal and never reaped. `9`: replay via `<Redirect>` (max 3 plays). Other: `<Hangup/>`, and the leg's status callback moves the chain on. |
| POST | `/dialend` | Twilio signature (fail-closed in EVERY env) | The `<Dial>` action: how the bridged call to the booker ended. `no-answer`/`busy`/`failed`/`canceled` (an allowlist, not a denylist) latch `client_no_answer_at = NOW()` once `WHERE client_no_answer_at IS NULL`, and only the latch winner texts Dallas. Either way the TwiML reads the booker's number back to the agent still on the line, re-derived through `toUsE164` so it speaks a number we would actually dial. Everything else, `completed` included, is a bare `<Hangup/>`. The latch is its own column rather than a value in `detail` (ruling R14): `placeLeg`'s catch writes a Twilio error code into `detail` earlier in the same chain. |
| POST | `/status` | Twilio signature (fail-closed in EVERY env) | Status callback for every leg, at-least-once tolerant. A status outside the five terminal Twilio values is a complete no-op BEFORE any write (ruling R25 gate 1; `onLegTerminal` carries its own copy as gate 2, those five plus the internal `create_failed` that `placeLeg` reports, so the billed-effect boundary is self-defending rather than trusting its only caller). Client leg: records `bridge_duration_sec`, digits-only parsed and `COALESCE`d so a redelivery cannot erase a good value. Agent legs: hand off to `onLegTerminal`, which owns the ring guard, the claim, the kill-switch and too-late re-checks, and every non-initial transition. Answers 200 with TwiML on any error, because Twilio retries a 5xx. |
...
- Corporate is **never inferred**. Measured across every client with a proposal: 14 of the 30 who booked corporate work used a personal address, and 10 of the 26 on company domains were booking their own weddings. `marketingSuggestions.js` therefore surfaces a suggestion with its reasoning for a human to accept, and never writes a tag.
- `GET /contacts` computes counts over **two scopes**: the rows and the held-back panel honor every filter, while the quick-filter chips exclude the active filter so they can answer "what would each filter give". `total` comes from the aggregate, not a window function, which would report 0 on a page past the end.
...
| POST | `/proposals/:token/change-requests` | Client (`clientPortalWriteLimiter`) | Create a pending change request: snapshots the requested fields, the baseline, the computed `edit_window` (`pre_booking` / `before_t14` / `inside_t14`), the price preview, the client's acknowledged total, request IP and user agent. A requested timing that runs past the 2:00 AM curfew is refused 400 outright (curfew sentence in the message; no client-side override). Enforced one-pending-per-proposal via the partial-unique index; fires the admin alert email + SMS via `notifyAdminCategory`. |
...
| GET | `/pending-first-replies` | Agent secret | Auto first-reply offer queue: leased `pending` reply jobs (lease stamps `first_reply_attempted_at` AND bumps `first_reply_attempts`; cap flips to `failed` un-offered; day rows downgrade to `night` while `LEAD_CALL_ENABLED='false'`; night rows created in the Chicago dead-hours window (`FIRST_REPLY_NIGHT_JITTER_START_HOUR`/`_END_HOUR`, default 2-8am, may wrap midnight) withheld on a 2-14 min per-row jitter, all other night rows offer immediately; rows past the call freshness bound never offered). `[]` unless `TT_AUTOREPLY_ENABLED='true'`. Returns negotiation_id, customer_name, first_reply_template, created_at. |
| POST | `/first-reply-sent` | Agent secret | Reply confirmation: guarded `pending`→`sent` flip (RETURNING the trigger inputs); a `day` flip younger than `FIRST_REPLY_CALL_MAX_AGE_MINUTES` fires `triggerLeadCall` with `skipWindowCheck` after `FIRST_REPLY_CALL_DELAY_SECONDS` (default 60s breathing room; sweep backstops restarts) (respond-then-ring); staler inserts a `failed`/`reply_confirmed_late` fault row. DB template is the source of truth, never the posted body. |
| POST | `/first-reply-failed` | Agent secret | Definitive reply failure (`template_not_found`/`lead_not_found`/`quick_reply_unavailable`/`send_unverified`/`ambiguous_lead`/`already_replied`/`response_cta_not_found`/`ai_draft_clear_failed`): guarded `pending`→`failed`; reason logged + Sentry-tagged (except `already_replied`, a benign manual-reply race), not stored; no email. A `day` flip inside the freshness bound still fires the promised call, after the same `FIRST_REPLY_CALL_DELAY_SECONDS` breathing room. |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/t/:token` | No (token-gated) | Fetch invoice by shareable token — line items, payments, refunds (succeeded, informational), client/event info. When a `service_extensions` row references the invoice, the response carries an `invoice.extension` block (status, rendered terms for the row's `terms_version`, `accepted_at`, `expires_at`, end times, `requires_payment`/`requires_acceptance`) that drives the InvoicePage terms gate; null on ordinary invoices, and it lives INSIDE the invoice object because InvoicePage stores `data.invoice` only. |
...
### Service Extensions — `/api/service-extensions`
On-site service extension (spec 2026-07-25): an assigned bartender requests added bar time mid-event, the client accepts versioned coverage terms and pays on the invoice page, and the event's duration plus payroll hours move. Routes live in `server/routes/serviceExtensions/` (composition router `index.js`; auth is applied inside each file, never at router level, because the public accept shares the mount). Extension money is side money on its own invoice (label `Service Extension`, in `OFF_LEDGER_INVOICE_LABELS`): `total_price` / `amount_paid` / `pricing_snapshot` never move. Staff-facing responses never contain `amount_cents`, `gratuity_cents`, or any dollar figure.

**The 2:00 AM service curfew is an insurance warranty, not a preference.** The liquor liability application (PK810225-GLLL230810) warrants "Do not have operations going past 2:00 AM", and breaching a policy warranty can void liquor liability coverage for the claim, so bar service is refused past that hour on every path that can set a duration. The primitives (`SERVICE_CURFEW_HOUR`, `maxDurationHoursBeforeCurfew`) live in `server/utils/eventEndInstant.js`; `server/utils/serviceCurfew.js` composes them into the proposal-facing checks (`checkCurfewForStart`, `curfewGateForSave`, `curfewMessage` — the one client-facing sentence); `serviceExtensionPricing.allowedAdditionalHours` applies the same primitives to extensions (alongside the 3-hour mis-scroll cap, and the staff picker is built from the same number so it can never offer a step the validator would reject). The curfew is the first 2:00 AM at or *after* the event's local start, computed in wall-clock time (the warranty is about the hour on the clock, which also makes the spring-forward night behave), and a booking ending exactly at 2:00 AM is legal. Enforcement differs by who is acting: the public quote builder and client change requests refuse outright; the admin proposal POST and PATCH refuse by default but proceed on an explicit `acknowledge_past_curfew` and write a `proposal_booked_past_service_curfew` row to the admin audit log; `settleExtension` refuses with no override (a webhook or the sweep is driving by then) and does so *before* claiming the row, so a refusal leaves it pending rather than settled on an event whose duration never moved. The admin PATCH checks only when the save touches `event_date` / `event_start_time` / `event_duration_hours`, so a legacy past-curfew booking does not demand an acknowledgement for a guest-count edit.

**Deliberately still unguarded, and why:** `server/routes/shifts.js` (admin mints a proposal from a shift), `server/utils/thumbtackProposalDraft.js` (duration comes from third-party lead data), `server/utils/proposalGroups.js` (clones an existing proposal's duration), and the seed/import scripts. These are admin- or system-driven rather than client-facing, and none of them is a booking a client can self-serve. A past-curfew value arriving through one of them is caught the next time anyone edits the timing. Closing them is fix-list work, not silent coverage.

**Also unguarded by design:** an event whose start time is itself after 2:00 AM (say a 3:00 AM start) measures its curfew against the *following* 2:00 AM, so it is not refused. Such a booking is already outside the warranty before any guard runs, and refusing to edit it would not cure that.

| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/eligibility/:shiftId` | Staff (`auth` + assigned/approved/not-dropped predicate on `shift_requests`) | Request-window check (event start through contracted end + 15 min grace) + 30-minute end-time step labels (max = the lesser of +3 hours and the room left before the 2:00 AM curfew; zero room = no offerable steps) + pending flag + hosted/class flags. Times and durations only, never a price. |
| POST | `/` | Staff (same predicate; `serviceExtensionLimiter`, 5/hour per user) | Create the request. Reprices inside the transaction under `SELECT ... FOR UPDATE` on the proposal (a stale pre-flight baseline after a prior settle 409s as `EXTENSION_BASELINE_MOVED`), mints the `Service Extension` invoice (`status='sent'`, line item `source_type='fee'`), inserts the `service_extensions` row (the partial unique index turns a concurrent second request into a 409 `EXTENSION_ALREADY_PENDING`), then post-commit texts/emails the client the invoice link and alerts admins. Response: id, status, requested end, `clientNotified`. No price. |
| POST | `/t/:token/accept` | Public (invoice-token gated: `requireUuidToken` + `publicLimiter`) | Record the client's terms acceptance: stamps `client_accepted_at` / `client_accept_ip` / `client_accept_ua` once (COALESCE keeps the first acceptance; idempotent re-accept returns the original timestamp). A priced extension returns `requiresPayment: true` and the webhook settles on payment; a zero-delta extension settles right here (duration + payroll + staff greenlight + `finalizeExtension`). A zero-delta settle refused on the 2:00 AM curfew 409s `EXTENSION_PAST_CURFEW` (never a false "confirmed"), alerts admins, and leaves the row pending. |
| GET | `/proposal/:proposalId` | Admin/Manager | Every extension for an event, money included (this is the admin surface), requester/override names via `COALESCE(contractor_profiles.preferred_name, users.email)`, plus a `payrollWarning` when the event's accrued lines were hand-edited. |
| POST | `/:id/override` | Admin/Manager | Grant the time without payment (required `reason`, 3-500 chars): settle as `overridden`, VOID the invoice and cancel open intents (no receivable survives an override), apply payroll hours, greenlight staff, `finalizeExtension`, audit-logged via `logAdminAction`. A settle refused on the 2:00 AM curfew (event times moved under the request) 409s `EXTENSION_PAST_CURFEW` with the curfew sentence, leaving the row pending. |
| POST | `/:id/cancel` | Admin/Manager | Decline a pending request: close as `cancelled`, void the invoice, send the staff decline notice, audit-logged. |
...
| GET | `/payouts/:periodId/paystub` | Yes | Lazy-generates a PDF paystub on first download for a PAID period, uploads it to R2 (`paystubs/<contractor_id>/<period_id>.pdf`), caches the key on `payouts.paystub_storage_key`, and returns `{ url }` (a 15-minute signed URL). 404 if not the staffer's payout, 409 unless the PAYOUT itself is `paid` (a `no_draw` payout earns no paystub even inside a paid period). Renders via `paystubPdf.js` from `paystubData.js` (incl. YTD); never touches `mark-paid`. `payment_handle` is intentionally not on the document (PII). |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| GET | `/tips` | Admin | Paginated list of all successful tips across bartenders for the admin tips ledger (filter by bartender, date range; cursor-paginated). Rows also project the Status columns `shift_id`, `deferred_at`, `rolled_forward_at`, `refunded_amount_cents` (COALESCEd to 0) and `dispute_won_at` (2026-08-19, staff-hub spec), so the ledger derives each row's status without a second read. |
| GET | `/tip-feedback` | Admin | List tip-page feedback submissions for admin triage. `?status=` is `unreviewed` (default), `reviewed`, or `all`; `?target_user_id=<int>` narrows to one bartender for the staffer profile's feedback card (a non-integer or non-positive value is a 400). |
...
- `communication_preferences` JSONB — `{sms_enabled, marketing_enabled}` (defaults true). Channel-level on/off shared by admin alerts and staff notifications. `sms_enabled` is the only channel kill switch; the STOP/START keyword flow writes it. Rows also carry an inert `email_enabled` key: it never had a product writer, nothing reads it as of 2026-08-14, and it was deliberately left in place rather than rewritten out. NOTE the column DEFAULT in `schema.sql` still includes it, so NEW rows get it too; dropping it from the default and backfilling existing rows is a separate migration decision, not something the 2026-08-14 removal did.
...
- `nudge_suppressed` BOOLEAN NOT NULL DEFAULT false — durable T-21 nudge suppression (cc-transfer 2026-07-07): set by `createDrinkPlan({skipNudge:true})`, respected inside `scheduleDrinkPlanNudge` so automatic re-enqueues (any proposal PATCH → `schedulePreEventReminders`) stay silent, cleared by the admin `reenroll-drink-plan-nudge` endpoint
...
- `lab_cta_clicked_at` TIMESTAMPTZ — write-once celebration-screen Enhancement Lab CTA click (attach-rate funnel)
...
- `finalized_at` TIMESTAMPTZ — BEO finalize stamp. Set by the derived finalize (reviewed + list approved, or hosted) from Mark reviewed / the shopping-list approve, or by the manual Finalize button (unpaid-extras override, post-Unfinalize); non-NULL locks every mutation route on the plan (status, notes, shopping list, logo, consult, source flip, delete; the list PUT and the approve flip carry `finalized_at IS NULL` inside their UPDATEs too) and gates the staff-side acknowledge + the T-3 nudge. One deliberate edge: re-confirming an already-approved list on a finalized plan is an idempotent no-op rather than a 409, so a send Retry still works. Cleared by Unfinalize.
...
- `group_id` (nullable INTEGER FK → proposal_groups, ON DELETE SET NULL) — set when this proposal is one "option" in a side-by-side comparison; NULL = a solo proposal (every existing row). Surfaced in the `GET /` list and `GET /clients/:id` payloads for the dashboard rollup.
- `setup_minutes_before` (nullable INTEGER) — crew arrival/setup lead time before service start. **Back-of-house only — never exposed on public token/proposal/invoice surfaces** (the `/t/:token` allowlist deliberately omits it). NULL ⇒ derive the default at read time: **90 for hosted (per-guest) packages, 60 otherwise** (`server/utils/setupTime.js`, mirrored client-side). Admin-adjustable via `PATCH /:id` (undefined/null sentinel like `total_price_override` — explicit `null` resets to the package default; never COALESCEd). Synced into `shifts.setup_minutes_before` by `createEventShifts` / `syncShiftsFromProposal`. A manual override **persists across package changes**; set it to NULL to fall back to the new package's default (90 hosted / 60 else). `GET /:id` also returns a server-derived `setup_time_display` (service start − effective minutes, 24-hour "HH:MM" — admin event detail is 24h throughout; staff-facing SMS/email keep 12-hour via the default `subtractMinutesFromTime`). Validation: integer 0–600 inclusive.
- `transferred_from_cc_id` (nullable TEXT, partial-unique) + `external_paid` (NUMERIC(10,2) NOT NULL DEFAULT 0, whole DOLLARS per the proposals money exception) — CC event transfer (2026-07-07, `scripts/cc-transfer-events.js`): events moved from CheckCherry before its shutdown. `external_paid` is money collected in CC, folded into `amount_paid` at transfer with ZERO `proposal_payments` rows (the CC-era ledger holds those payments; native rows would double-count blended metrics), so balance math (`total_price - amount_paid`) self-corrects dunning and invoice derivation. Transferred proposals are born `confirmed` + `accepted_at` set with `total_price_override` = the CC contracted total, drink plan created nudge-suppressed, balance/pre-event reminders enrolled (past-dated rungs purged). The payment panel shows an "Off-platform" line when `external_paid > 0`. `proposals.cc_id` stays a v1 relic (NULL) so the ledger loader's guard and the metrics tri-state keep their meaning.
- `pricing_snapshot` (JSONB — full pricing breakdown at time of creation; also carries `gratuity {rate, tip_jar, staff_count, hours, staff_noun, total, floor_rate}`, `staff_noun`, `display_labels`, and `_version` — stamped by `calculateProposal` (`PRICING_SNAPSHOT_VERSION`); server readers route through `server/utils/pricingSnapshot.js` `readSnapshot()`, tolerating legacy unversioned snapshots as v1 and hard-failing only on unknown future versions)
- **Checkout gratuity** (spec 2026-06-05; election-at-payment 2026-08-03; admin mandate 2026-08-10): `tip_jar` (BOOLEAN, default true), `gratuity_rate` (NUMERIC(10,4), default 0 — per-staff-per-hour pre-paid gratuity), `gratuity_rate_change_origin` (TEXT: `'staffing' | 'admin' | null`), `gratuity_floor_rate` (NUMERIC(10,4), nullable — admin-mandated floor rate; a mandate exists iff > 0, enforced by `proposals_gratuity_floor_positive_check`; mirrored into `pricing_snapshot.gratuity.floor_rate` by every persisting snapshot writer). CHECK `(tip_jar = true OR gratuity_rate >= 50 OR (gratuity_floor_rate IS NOT NULL AND gratuity_rate >= gratuity_floor_rate))` — the third disjunct lets a sub-$50 mandate coexist with a no-jar answer; the mandate REPLACES the no-jar floor at every enforcement layer, never stacks. Note the disjuncts are OR'd, so the CHECK is not itself a mandate-floor backstop (a jar-yes row can hold rate < floor as far as the DB cares); mandate-floor enforcement is application-level (`deriveGratuityRate` + the webhook's post-`FOR UPDATE` check), which every current writer satisfies. The dollar gratuity is computed (`rate × staffCount × hours`), appended to the breakdown as a `"Gratuity"` line, and folded into `total_price` on top of any override. **The CLIENT election is written by the `payment_intent.succeeded` webhook, not at checkout**: `create-intent` computes it in memory and stamps `tip_jar`/`gratuity_rate` into the PaymentIntent metadata, and the webhook applies it (before the Balance invoice, `FOR UPDATE`, payable-status guard) so an unpaid proposal never carries a client-elected gratuity (an admin MANDATE is the one pre-payment gratuity); that apply sets `origin` to NULL (a client election). The other writers: `server/utils/lineItemCancel.js` — the cancel-line-item gratuity removal/lowering, admin's only POST-PAYMENT path, stamps `origin = 'admin'` and clears `gratuity_floor_rate` — and the admin PATCH's mandate field `gratuity_mandate_total` (`server/utils/gratuityMandate.js`, spec 2026-08-10), the only PRE-payment/PRE-signature path, which writes `gratuity_rate` + `gratuity_floor_rate` together and never touches `origin`. That `'admin'` stamp is live logic, not an archival label: `PATCH /:id` (`crud.js`) reads it and skips the `'staffing'` relabel, so an admin-set gratuity keeps its `'admin'` provenance and fires no staffing-gratuity client notice on a later staffing change (the dollar still rescales at the same rate, as it does for every origin). `'staffing'` is stamped by that PATCH rescale path. One-off cleanup of pre-election rows: `scripts/reset-unpaid-gratuity.js` (skips mandated rows — `gratuity_floor_rate IS NULL` guard in both the SELECT and the under-lock re-check).
...
- **Bar menu print file** (staff event-details spec 2026-07-22): `menu_print_key` (TEXT, nullable — R2 object key under `menu-print/<proposalId>/`; replacing or removing a file orphans the old object, matching the drink-plan logo pattern since `storage.js` has no delete) and `menu_not_required` (BOOLEAN NOT NULL DEFAULT false — admin's explicit "no printed menu for this event"). The staff-facing tri-state is derived, never stored: key present = `ready`, else `menu_not_required` = `not_required`, else `pending`. Written only by `/api/proposals/:id/menu-print` (admin/manager); read by the staff event-details payload and the `/api/shifts/:shiftId/menu-print` download proxy.
- `total_price`, `status`: draft | sent | viewed | modified | accepted | deposit_paid | balance_paid | confirmed | completed | archived
- `archive_reason` (TEXT, nullable) — only set when `status = 'archived'`. CHECK-constrained to `no_hire | client_cancelled | we_cancelled | event_completed | other | option_not_chosen`. The legacy `cancelled` status was migrated to `archived` with `archive_reason = 'client_cancelled'` in Task 2 of the automated-communication rollout; the enum no longer accepts `cancelled`. A cancelled BOOKED event (P6/fix #7) archives with `client_cancelled` (client cancelled) or `we_cancelled` (Dr. Bartender cancelled).
- `cancelled_at` (TIMESTAMPTZ, nullable), `cancelled_by` (TEXT, CHECK IN `client` | `admin`, mirroring `proposal_change_requests.cancelled_by`), `cancellation_note` (TEXT) — set by `POST /api/proposals/:id/cancel` when a booked event is cancelled. Distinct from generic archival: these are only populated on the cancel-event path.
...
- Autopay claim: `autopay_status` (NULL | 'in_progress' | 'failed') and `autopay_attempted_at` — atomic row-claim used by both the scheduler and the admin manual charge so concurrent runs can't double-charge. Cleared to NULL when `payment_intent.succeeded` flips status to `balance_paid`. Stuck `'in_progress'` claims expire after 72h (F1: 24h equalled Stripe's idempotency-key lifetime, so a >24h webhook outage re-fired with an expired key and double-charged; a re-charge is now additionally guarded against Stripe's actual intent status via `server/utils/autopayDurableCharge.js`).
...
- `gratuity_cents` (INTEGER, nullable) — the gratuity portion attributed to this refund (P6/cancel). Label-based attribution has no gratuity concept, so the cancellation-refund path records how much of each refund row is the returned gratuity, keeping it auditable and out of contract-revenue adjustments. NULL for admin partial refunds that don't attribute gratuity.
...
- Excess derivation: ATTEMPTED AND PULLED (2026-07-26, revert 29316b10). The idea was that the portion of a refund exceeding the contract never lowers `total_price`. It was reverted because deriving the excess as `amount_paid - total_price` counts off-contract invoice money (syrups, manual fees) that lives in `amount_paid` but never in `total_price`, so it under-refunded. Do NOT re-attempt without netting that out first; see the do-not-reattempt note in `refundHelpers.js` and the invoice-provenance entry in the fix list. The `total_scope` column below is what actually ships this behavior, per refund row.
- `total_scope` (TEXT NOT NULL DEFAULT 'contract', CHECK contract|overpayment) — the total_price rule for THIS refund. 'contract' = Approach A (refund lowers the total); 'overpayment' = the refund returns money held beyond the contract, so reconciliation leaves total_price alone. THREE writers set it (2026-09-15): cancel-line, where the fold already corrected total_price before the refund fired; the admin payment panel, where `total_scope` on `POST /api/stripe/refund/:id` carries the admin's choice, capped at the netted overpayment under the proposals row lock; and the `refund.created` webhook, which derives it for a dashboard refund from Stripe's `duplicate` reason or the netted overpayment. Under overpayment scope reconciliation first consumes the payment's UNCREDITED headroom (an invoice credit is capped at that invoice's remaining due while the whole intent rolls into `amount_paid`), because those cents are on no invoice and refunding them must reverse none. Durable on the row because webhook/sweeper adoption runs later with no caller memory; `refundExecute` adopts its own row BY ID so a stranded same-amount pending row can never rewrite the rule. Invoice `amount_due` is a separate question: it is left alone ONLY for invoices whose demand something else already rebuilt, i.e. UNLOCKED invoices with a total-tracking label (`proposalMoneyShared.TOTAL_TRACKING_INVOICE_LABELS` = Balance / Full Payment). Every other invoice — locked, or unlocked Deposit / Additional Services / Enhancement Lab / manual, none of which `refreshUnlockedInvoices` recomputes from the total — drops both figures, or a settled invoice flips to a client-visible `partially_paid` phantom balance on a still-live pay link.
- Partial unique index on `stripe_refund_id` (WHERE NOT NULL) — idempotency anchor shared by the synchronous route, the `refund.created` webhook backstop and the stale-pending sweeper

**proposal_groups** — Option groups behind the "compare your options" feature. One row bundles two or three sibling proposals (each an "option") behind one public `/compare/:token` link. `token` UUID (the public compare link), `client_id` FK, `chosen_proposal_id` FK → proposals (set first-writer-wins on the winning option's first settled payment; NULL until decided). A proposal joins a group via `proposals.group_id`. Created / dissolved by `server/utils/proposalGroups.js`.
...
- A full payment on a deposit-terms proposal UPGRADES that first invoice rather than stranding it. `createInvoiceOnSend` picks the label from `payment_type` at SEND time (column default `deposit`), so a client who then chooses pay-in-full at checkout arrives with a $100 Deposit invoice open while a full-total capture lands, and the label-blind link credited $100 and dropped the rest against `linkPaymentToInvoice`'s cap. `upgradeDepositInvoiceToFull` (spec 2026-08-28; implementation in `invoiceLifecycle.js`, exported through the facade) re-derives that row FROM the proposal, inside the caller's transaction, after the proposal-side credit and BEFORE the invoice link: label `Full Payment`, `amount_due = total_price − external_paid`, `due_date = balance_due_date`, line items regenerated, invoice number unchanged. It locks the proposal row first (the global order is proposals then invoices, and on the webhook rails the status-guarded credit takes no lock on a confirmed or completed proposal) and carries its own guards: a confirmed, completed, archived or cancelled proposal is never relabelled, nothing owed means nothing to relabel, and money the proposal already held before this payment (which never reached the Deposit invoice) makes it decline so the link's overflow email says so. It is then guarded to an unlocked, unpaid, `sent` Deposit, so an invoice with money on it, locked, void, or differently labelled is never touched and null comes back. Lines regenerate only when the generated lines sum to the contract (an override'd or legacy-snapshot proposal keeps its old lines, and the breadcrumb records `generated_sum_mismatch`). Both webhook rails wrap the call in a `SAVEPOINT deposit_upgrade`, so a deterministic failure inside it degrades to the old link path plus a Sentry warning, never to a payment that was not recorded. `refreshUnlockedInvoices` writes `amount_due` only where the row is still unlocked and still the label it decided on, so the admin re-price (the one caller that runs it outside the proposals lock) cannot put the deposit figure back onto a just-upgraded, locked Full Payment invoice. All three payment entrances call it on a full payment: `payment_intent.succeeded` and `checkout.session.completed` under the same archived/conflict guard as the Balance mint (a stale full intent on a cancelled event or a non-chosen option never relabels and reprices an invoice), and admin record-payment on its fully-paid gate (a balance record on deposit terms leaves `payment_type` alone, and the helper returns null on the paid, locked Deposit row). The `checkout.session.completed` rail now mints a Balance invoice only on a deposit link (commit `a0b8c1f0`), in parity with `payment_intent.succeeded`, so a full link that captures short never has a Balance minted beside the upgraded Full Payment invoice that already carries the whole remainder: a full-link capture is no longer double-billed. The lookup-and-link itself is one shared function, `linkOpenContractInvoice` (`invoiceLinking.js`): oldest open invoice, off-ledger labels excluded, `id` tiebreak, then `linkPaymentToInvoice`, returning the overflow payload. An overflow that still occurs is no longer silent, and neither is a refused credit (a `not_found`, `not_payable` or `no_remaining_due` refusal carries the whole payment as overflow): each entrance emails the `payment_failure` lane from its POST-COMMIT tail via `notifyLinkOverflow`, post-commit because `notifyAdminCategory` takes its own pooled connection. Rows already stranded in the old shape are repaired by the one-off `server/scripts/backfillFullPaymentInvoices.js`.
...
*Off-ledger invoice labels:* `OFF_LEDGER_INVOICE_LABELS` (`server/utils/proposalMoneyShared.js`) holds labels whose invoice money lives entirely OUTSIDE `proposals.total_price` (genuinely additive, invoice-only upsells). Non-empty again since 2026-07-26: exactly `['Service Extension']`. (History: `Enhancement Lab` left the set on 2026-07-20 when lab additions switched to folding into `total_price`; the consumer machinery then sat wired as a no-op until the extension label joined.) Membership flips four behaviors: the Stripe webhook skips the `amount_paid` roll-up for a payment linked to such an invoice (`paymentIntentSucceeded.js`), `refreshUnlockedInvoices` and the Balance `lockedTotal` never count it (`invoiceLifecycle.js`), refund reconciliation leaves the contract alone (`refundHelpers.js`; extension payments are also excluded from the refund planner's candidate list by an anti-join, so the admin refund cannot land on them), and the open-invoice lookup that picks which invoice a contract payment credits excludes it at all three payment entrances. That fourth one matters because the lookup is otherwise label-blind (oldest `sent`/`partially_paid` row wins), so an open `Service Extension` invoice would absorb a contract payment and leave the contract invoice unpaid: each of the three now carries `AND NOT (label = ANY($2::text[]))` against this set, in `stripeWebhookHandlers/paymentIntentSucceeded.js` (since 2026-08-03), `routes/proposals/actions.js` (admin record-payment), and `stripeWebhookHandlers/checkoutSessionCompleted.js` (the payment-link rail), the last two brought to parity 2026-08-28. The label is reserved: admin invoice create and edit refuse to mint an invoice into it or rename one out of it (`routes/invoices.js`). Extension money is deliberately NOT in `CONTRACT_LABELS`: it never enters `total_price` or `amount_paid`, so treating it as contract money would either forgive the contract by the extension amount or count side money as contract revenue.

### Service Extensions (on-site added bar time)

**service_extensions** — One row per staff request for more bar time (spec 2026-07-25). Money is SIDE money: the paid extension lives as its own invoice + payment under the off-ledger `Service Extension` label (see above), and `proposals.total_price` / `amount_paid` / `pricing_snapshot` never move; the only contract mutation is `proposals.event_duration_hours` plus the single linked shift's denormalized duration and end_time. Owned by `server/utils/serviceExtension{Pricing,Settle,Payroll,Notify,Sweep}.js` and the routes in `server/routes/serviceExtensions/`.
- `proposal_id` FK → proposals (ON DELETE CASCADE); `shift_id` FK → shifts, `requested_by_user_id` FK → users, `invoice_id` FK → invoices (all ON DELETE SET NULL)
- `contracted_end_time` / `requested_end_time` VARCHAR(20): display strings ("8:00 PM"); `contracted_duration_hours` / `requested_duration_hours` NUMERIC(4,1)
- `amount_cents`, `gratuity_cents`: INTEGER CENTS; CHECK `service_extensions_money_nonnegative_check` keeps both >= 0 (the pricing module already refuses a negative delta; this is the DB backstop). The gratuity delta joins the payroll pool: accrual adds `SUM(gratuity_cents)` over the proposal's `paid` rows (`payrollAccrual.js`)
- `hosted_product_confirmed` BOOLEAN (hosted per-guest packages only, else NULL); `terms_version` TEXT looks up the versioned client-facing copy in `server/data/extensionTermsCopy.js`, which THROWS on an unknown version rather than showing text the client never agreed to
- `client_accepted_at` TIMESTAMPTZ, `client_accept_ip` VARCHAR(64), `client_accept_ua` TEXT: the acceptance artifact, stamped once by the public accept route (COALESCE keeps the FIRST acceptance; a double-tap cannot rewrite it)
- `status` CHECK: pending | paid | expired | cancelled | overridden. Every exit from `pending` goes through the claim UPDATE in `serviceExtensionSettle.js` (`... WHERE status = 'pending'`), so a settle/expire race or a replayed webhook resolves to exactly one winner
- `finalized_at` TIMESTAMPTZ: stamped only after the post-settle side effects (payroll hours + staff greenlight) have run. A `paid`/`overridden` row with `finalized_at` NULL is a crash casualty: the settle committed but payroll still holds the old hours and no bartender was told; the sweep's heal finds and finishes it
- `override_by_user_id` FK → users (SET NULL), `override_reason` TEXT, `expires_at` TIMESTAMPTZ NOT NULL, `created_at`, `updated_at`
- Indexes: partial UNIQUE `idx_service_extensions_one_pending (proposal_id) WHERE status = 'pending'` (one live request per event; this is what turns a concurrent second request into a clean 409 instead of a double charge). Partial `idx_service_extensions_pending_expiry (expires_at) WHERE status = 'pending'` (sweep driver); `idx_service_extensions_invoice (invoice_id)` (the webhook's extension discriminator); `idx_service_extensions_proposal_status (proposal_id, status)` (payroll gratuity addend); partial `idx_service_extensions_unfinalized (updated_at) WHERE finalized_at IS NULL AND status IN ('paid','overridden')` (crash-recovery heal driver)

*Expiry sweep + heal:* `server/utils/serviceExtensionSweep.js` registers the `service_extension_sweep` scheduler (every 60 seconds, gated by `RUN_SERVICE_EXTENSION_SWEEP_SCHEDULER`, schedulerHealth heartbeat, in-process reentrancy skip so a slow tick never overlaps). Each tick first expires pending rows past `expires_at` (claim via `closeExtension`, void the invoice and cancel open intents, staff decline notice carrying the insurance warning), then `healUnfinalizedExtensions` finishes settled-but-unfinalized rows. A pending row whose invoice is already PAID is never expired: that shape means the client paid but the settle tail died, so the sweep leaves it pending and alerts admins to settle or refund by hand (throttled to once per 24h per extension). Load-bearing, not housekeeping: nothing else enforces the hard service stop at the contracted end.
...
Weekly Tue–Mon payroll: `pay_periods` (status open → processing → paid, plus `reopened`: the admin "I fucked up" state reached only from `processing`, editable like `open` for pending payouts but refused by every money writer because they all test `status = 'open'`; re-processing returns it to `processing`) → `payouts` (one per contractor per period; status `pending` | `no_draw` | `paid` — `no_draw` is the owner shape (spec 2026-08-07): born via `ensurePayout` when `contractor_profiles.takes_draw = false`, tracked and line-editable like pending, but excluded from owed rollups, period-close blocking, mark-paid, paystubs, and 1099 totals; the staff portal presents it as-if-paid ("Not drawn" + blended total); ALL payout creation goes through `ensurePayout` in `payrollProcessing.js` — an inline `INSERT INTO payouts` anywhere else is a defect; `total_cents` = clamped sum of its event lines PLUS payable duty lines, written ONLY by `recomputePayoutTotal(s)` in `server/utils/payrollProcessing.js` — the single-writer rule, spec 2026-08-06 §3.3: the clamp wraps the WHOLE sum so clawback debt nets against duty pay; any other `UPDATE payouts SET total_cents` is a defect; `payment_reference` TEXT holds the mark-paid trace: Zelle conf #, Venmo note, check number; admin-only, never in staff projections) → `payout_events` (per-event line items) + `payout_duty_lines` (typed duty pay: bar_rental / parking / equipment_supplies / hosted_supplies / menu_print / out_of_area / review_bounty / review_contest; derive-never-increment in accrual, removed rows kept forever so derivation cannot resurrect an admin removal, `held_state` mirrors the payout_events hold chip, review kinds may carry NULL `shift_id`; `duty_attributions` stages who-gets-it for the attributed kinds, and the period Process endpoint 409s while any attributed duty is missing or stale). Owned by `server/utils/payrollAccrual.js` (accrual + roster sweeps) + `server/utils/dutyLines.js` (triggers/reconcile/materializers) and `server/routes/admin/payroll.js` / `server/routes/admin/payrollDuty.js` (admin worklist + edits). All money integer cents; DB backstops `payout_duty_lines_amount_cap_check` (±$1,000) and `shifts_out_of_area_bonus_cap_check` (0..$250). Ship/period-open backfill: `scripts/backfill-duty-lines.js`.

Duty-pay satellite tables and columns (spec 2026-08-06): `staff_reviews` (review_date, stars 1..5, source google|thumbtack, `tt_review_id` UNIQUE bridging the existing `thumbtack_reviews` webhook ingest, status pending|confirmed|dismissed) + `staff_review_credits` (which staffers a review names; UNIQUE(review, user)) feed the $10 `review_bounty` / quarterly `review_contest` materializers. `shifts.out_of_area_bonus_cents/_attached_by/_attached_at/_locked_at/_locked_user_id` carry the discretionary Out-of-Area Bonus (locked once an approved staffer holds the shift; re-arms on drop). `proposals.remote_fee_prompted_at` (the Send popup asks once) and `proposals.venue_lat/venue_lng` (on-demand geocode cache for the remote-staffing check) support the client-side Remote Staffing Fee surfaces built in the out-of-area lane.
...
- `notes`
- `communication_preferences` JSONB — `{sms_enabled, marketing_enabled}` (defaults true). Drives the Automated Communication system's send gating: `sms_enabled` is the SMS kill switch (STOP/START), `marketing_enabled` is the campaign unsubscribe. There is no email kill switch — email suppression comes from `email_status = 'bad'` alone. Rows also carry an inert `email_enabled` key (no writer ever set it; nothing reads it as of 2026-08-14; deliberately left in place rather than rewritten out). NOTE the column DEFAULT in `schema.sql` still includes it, so NEW rows get it too; dropping it from the default and backfilling existing rows is a separate migration decision.
...
- Written by TWO callers, both via `recordSmsConsent` (`server/utils/smsConsent.js`) inside their own transaction: `POST /api/proposals/public/submit` when the quote wizard's consent checkbox is present in the body (`source_form='quote_wizard'`), and `POST /api/sms/opt-in`, the standalone `/sms` page (`source_form='sms_page'`). Keep this list current — it is the only place the writers are enumerated. `communication_preferences.sms_enabled` + an `sms_opt_in_at` / `sms_opt_out_at` stamp are written in the same call, reusing the exact jsonb shape `smsInbound.js` writes on STOP/START.
...
- **`email_sends` joined in lane mkt-g**, which added its `client_id`; the leg still sits behind a once-per-process `information_schema` probe so the module works before and after. `idx_email_sends_client_sent_partial (client_id, sent_at DESC) WHERE client_id IS NOT NULL` backs it, because the table grows by one row per recipient per blast.
- **Scope limit, phase 1:** a contact with no proposal has nothing in *either* current source. `logClientMessage` returns early without a `proposal_id`, and every client-directed `scheduled_messages` row is proposal-anchored too, so the ~254 proposal-less clients have an empty history until they are actually sent a campaign, which the mkt-g leg then shows.
...
*Scheduled-message dispatcher:* `server/utils/scheduledMessageDispatcher.js` registers the `message_dispatcher` scheduler, which runs every 5 minutes. Each tick drains due `pending` rows from `scheduled_messages`, applies the shared suppression rules (archive / comm-prefs / bad-contact), and dispatches each row to its registered per-message-type handler (the money-path balance reminders, plus the marketing / BEO types below). Rows whose `channel` is `push` skip the handler model and fan out (via `server/utils/pushDispatch.js`) to the recipient's stored Web Push subscriptions through `pushSender.js` (see Web Push below). The push sends run with NO database transaction held; any `410`/`404`-dead subscription is then pruned in a separate short `SELECT FOR UPDATE` transaction. (The earlier design held that transaction open across every push round-trip, so a slow endpoint pinned the pooled connection idle-in-transaction until Neon killed it and the unhandled pool error crashed the process, which was Sentry SERVER-17.) Rows are wired in by `server/utils/messageScheduling.js` (`scheduleMessage`, idempotent insert). Gated by `RUN_MESSAGE_DISPATCHER_SCHEDULER` (suppressed by the global `RUN_SCHEDULERS=false`).
...
*CC-import wrap-up handler:* `server/utils/ccWrapUpHandler.js` registers a single message type `post_event_wrap_up_email` (anchor `event_date`, category `operational`, priority 3, `offsetFromEventDate: null`, `cooldownExempt: true`, `multiChannel: false`) formerly enqueued by the `/admin/cc-import/wrap-up` endpoint (DELETED in the 2026-07-07 cc-demolition); the handler stays boot-registered so the dispatcher can drain any still-scheduled rows, and no new enqueues are possible. Body lives in `server/utils/ccWrapUpEmailTemplate.js`. Registration is boot-wired in `server/index.js` (around line 343) so the dispatcher's first tick can resolve the type. The `category: 'operational'` registration deliberately bypasses the marketing-enabled gate, and `cooldownExempt: true` keeps the wrap-up email separate from the daily-overlap dedupe so the operator can sequence batches without false collisions.
...
**presence_log** — Interval log for the two-person presence tracker (admin time clock; spec `docs/superpowers/specs/2026-07-02-presence-tracker-design.md`). One row per state/toggle interval; exactly one open row (`ended_at IS NULL`) per user, enforced by partial unique index `idx_presence_log_one_open`. Tracked users carry presence columns on `users`: `presence_state` (desk/available/away, CHECK), `presence_since`, `presence_taking_leads`, `presence_lead_rank` (NULL = untracked; partial-unique; the max-rank fallback owner online-and-taking has dibs and wins outright, else the lowest online-and-taking rank owns leads, else the fallback owner), `presence_last_seen_at` (throttled sign-of-life stamp from the auth middleware + webhook splices), `presence_nudge_channel` (sms/telegram, CHECK), `presence_nudge_phone` (E.164 nudge destination + inbound match key; deliberately not `contractor_profiles.phone`). Separately, `users.timezone` (TEXT NOT NULL DEFAULT `'America/Chicago'`, IANA) is a general user attribute introduced for this strip: it carries each tracked user's real zone so the strip can render the OTHER person's 24h local wall-clock time (a browser-derived zone can only report your own). Backfilled to `Asia/Manila` for `zul@drbartender.com`, guarded on the default value. Rendered client-side via `Intl.DateTimeFormat` on the strip's existing 60s tick, wrapped in try/catch because `PresenceStrip` sits in `AdminLayout` on every admin page and an Intl throw on a malformed zone would white-screen the admin app.
...
Dibs (spec `docs/superpowers/specs/2026-07-06-presence-dibs-design.md`): the fallback owner (max `presence_lead_rank`) online-and-taking beats the chain in `derivePointer`, and his online default for `taking_leads` is false (chain users stay default-on). When his toggle, transition, or auto-flip moves the pointer, `presenceNotify.js` pings the displaced/receiving user via their nudge channel (grab: "called dibs on leads", release: "released leads. You're up"); captures around the mutation are failure-isolated, gated skips are silent, genuine send failures warn + Sentry, confirmed sends log one audit line.

**consults** — Scheduled phone consults booked via Cal.com. The table shipped ahead of the integration so downstream code (and `scheduled_messages.entity_type = 'consult'`) could reference it; the webhook has filled it since 2026-05-27, and since 2026-08-25 each row is also what the consult call bridge rings. Drink-plan notes themselves continue to live on `drink_plans.consult_selections`, not here.
- `id` SERIAL PK
...
- `calcom_event_id` TEXT — external Cal.com event identifier, and UNIQUE (see below). Nullable in the column definition because the table predates the integration; in practice the webhook has set it on every row it writes since 2026-05-27. Renamed from `calendly_event_id` to match the Cal.com vendor cutover
...
- `booker_phone` TEXT — the number the booker typed into Cal.com, stored as typed apart from a trim and a 50-character cap, never format-normalized on the way in. The consult call bridge dials this and nothing else, and because the booking page is public it is attacker-supplied: every dial goes through `toUsE164` first, at chain open and again at press-1, and a number that fails files a terminal `skipped_invalid_phone` row instead. A reschedule writes it `COALESCE`d, so a payload with no number keeps the original.
...
- pending_call / call_audit / telegram_update / voicemail_delivery — Zul VA calling
  (Telegram-triggered Twilio callback bridge) plus 224-inbound voicemail.
  pending_call = one in-flight confirm/dial per Telegram
  user (claim-then-call state machine); call_audit = append-only spend/abuse ledger
  backing the DB-backed daily cap; telegram_update = update_id de-dupe;
  voicemail_delivery = one row per MISSED inbound call, doing three jobs at once
  (PK on call_sid is the missed-call ping's dedup claim, rows in the last 24h are
  the VM_DAILY_CAP spend window, and status/delivered_at are the delivery ledger
  that outlives the Twilio recording, which is deleted after a confirmed upload
  on the ZUL line; the PRIMARY line retains it until 1b's R2 copy). Phase 1a
  columns: line TEXT NOT NULL DEFAULT 'zul' CHECK ('primary','zul') — which
  number the call arrived on; the DEFAULT IS the backfill (every pre-2026-07-26
  row was a 0082 call) and drives greeting, escalation target, and delivery
  channel. escalated_at = the press-1 claim AND its rolling spend window
  (VM_ESCALATION_DAILY_CAP counts it, partial-indexed); escalation_accepted_at =
  set only by a human keypress on the whisper (DialCallStatus cannot distinguish
  "they talked" from "a machine answered and was screened out");
  escalation_outcome = observability-only CHECK'd enum, first-writer-wins.
  listen_token = UUID NOT NULL DEFAULT gen_random_uuid() (the DEFAULT IS the
  backfill; nothing generates one in application code), UNIQUE because it is a
  public lookup key. It is the auth for GET /api/voice/vm/:token, which streams
  the retained primary recording.
  Pruned by vaCallingScheduler. The voicemail_delivery predicate is NOT
  "terminal statuses": it is "no recording left in Twilio". Pruned are
  'delivered' and 'empty' (recording deleted) plus 'missed' (the caller hung up
  during the greeting, so no recording was ever made). RETAINED past retention
  are 'recorded', 'failed', and 'skipped', because each means audio is still
  sitting in the console and the row is the only pointer to it. KNOWN 1a GAP:
  a delivered PRIMARY row breaks the "recording deleted" premise (its audio is
  retained until 1b's R2 copy), so the 30-day prune orphans that audio in the
  Twilio console; volume is small and 1b's purge closes it — do not "fix" by
  deleting primary recordings, which would destroy the only copy of the
  message content. Do not "tidy"
  'skipped' back into the pruned list: that deleted the pointer to every
  recording retained by a bootstrap-mode or SEND_NOTIFICATIONS=false instance. Spec: docs/superpowers/specs/2026-07-01-zul-va-calling-design.md
  Partial unique index `uq_call_audit_dead_leg` on `(call_sid, status)` WHERE
  `call_sid IS NOT NULL AND status IN ('no-answer','busy','failed','canceled')`
  makes the dead-leg Telegram alert atomic (claim-before-send via
...
- **`proposal_resend`** (`actions/proposalResend.js`): re-sends the proposalSent email + initial-proposal SMS with no status change (a validate-only RESENDABLE-status guard, no state side effect). Ex-`POST /api/proposals/:id/resend`.
- **`proposal_send`** (`actions/proposalSend.js`): the compose-first INITIAL send. `ensureSideEffects` does the row-locked draft→sent flip plus invoice in one transaction, then enrolls the unsigned-proposal drip post-commit. It shares `resolveRecipient` / `buildMessages` / `dispatch` with `proposal_resend` by delegation (identical proposalSent email + initial-proposal SMS, same suppression). Backs the draft-first create cockpit; the `POST /api/proposals` `send_now` path stays only for API compatibility.
- **`proposal_send_group`** (`actions/proposalSendGroup.js`): EMAIL ONLY, the single `proposalOptionsSent` compare email. `ensureSideEffects` runs the AB-BA-safe `FOR UPDATE` group flip (every draft option to `sent`, no per-option invoice or comms), idempotently. Ex-`POST /api/proposals/:id/send-group`.
- **`portal_invite`** (`actions/portalInvite.js`): emails the client their OTP portal link (nothing minted, no token in the link); email default, SMS off by default; no state side effect. Ex-`POST /api/proposals/:id/portal-invite`.
- **`payment_reminder`** (`actions/paymentReminder.js`): a balance-due reminder (balance = `total_price - amount_paid`, computed exactly as the legacy route, no money written); `ensureSideEffects` only enforces the NO_BALANCE_DUE guard. Ex-`POST /api/proposals/:id/send-reminder`.
- **`drink_plan_nudge_reenroll`** (`actions/drinkPlanNudgeReenroll.js`): `ensureSideEffects` clears the durable `nudge_suppressed` flag and re-schedules the T-21 nudges, and `dispatch` adds an immediate drink-plan nudge (the planner CTA uses the drink-plan token, not the proposal token). The deprecated `POST /admin/proposals/:id/reenroll-drink-plan-nudge` route runs the schedule-only side effects and does not auto-dispatch.
...
- **`server/utils/shoppingListGen.js`** — Shared helpers used by both the planner auto-gen path and the consult routes: `resolveCocktailIds()`, `buildPlannerGeneratorInput()`, `buildConsultGeneratorInput()`, and `autoGenerateShoppingList()` (with its strict no-overwrite guard). Planner v2 additions: `buildDerivationForPlan()` attaches an admin-only `_derivation` metadata block (drinkers/pours/split/buffers/per-category math) to generated lists when the plan answered the crowd question, and `applyAdminSetHolds()` preserves admin-set quantities across a regenerate. Absent-safe: with no `selections.crowd`, no derivation is attached and the generated list is byte-identical to pre-v2 output. Derivation is metadata only; it never alters the par-scaled purchase quantities. The `_derivation` key rides the underscore-diagnostic path (stripped from the public token response and from the admin save PUT).
...
- **`ShoppingListModal.jsx`** — Full-screen editable modal: add/remove/rename/reorder items (drag-and-drop), edit quantities, change guest count with recalculate prompt, share client link, Download PDF, Approve & Send to Client. Planner v2 instruments: an Editor / Client-view toggle (the Client view previews the exact plain-language copy the client reads), the `DerivationStrip`, an `admin_set` marker on hand-set quantities (persisted with the list so a regenerate holds them, and surfaced by a held-on-regenerate notice), and an approve consequence line naming the Enhancement Lab window close.
- **`DerivationStrip.jsx`** — Planner v2 sibling holding the admin "how we got here" strip (expected demand, category split, buffer chips, expandable per-category math, read from the `_derivation` metadata; renders nothing for legacy/consult plans) and `ClientPreview` (the parchment Client-view render). Both use the plain-language formatter exported by `ClientShoppingList.js`, so the admin preview is 1:1 on copy with the public page.
...
- **Public client view**: `client/src/pages/public/ClientShoppingList.js` — token-gated read-only display rendered at `/shopping-list/:token`. Renders plain-language quantity lines ("2 × 1.75L bottles, Tito's Vodka"), the verbatim padding sentence ("Quantities are rounded up so you never run out. Unopened bottles can be returned."), and short find-it guidance on specialty (syrup/craft) items. Exports the pure `describeClientLine()` / `specialtyNote()` / `PADDING_SENTENCE` formatter reused by the admin modal's Client-view preview.
...
- `id` SERIAL PK, `campaign_id` FK, `sequence_step_id` FK
- **Recipient is EITHER a lead or a client, never both and never neither.** `lead_id` FK (nullable since lane mkt-g) and `client_id` FK (added by mkt-g, `ON DELETE CASCADE`), constrained by `email_sends_recipient_check`: exactly one is set. Not "at least one" — a row naming both would be double-counted by the analytics aggregate and would make the webhook's bounce suppression ambiguous about whose status to flip. Any query joining a recipient must LEFT JOIN both and COALESCE; an INNER JOIN on `lead_id` silently drops every campaign send to a client.
- `idx_email_sends_campaign_client_once` UNIQUE on `(campaign_id, client_id) WHERE campaign_id IS NOT NULL AND client_id IS NOT NULL` — **the send-once guard.** `marketingSend.js` claims a row here BEFORE calling Resend, so the database arbitrates a double-click rather than a flag read earlier. The claim is reclaimable while `status = 'failed'`, or `status = 'queued'` AND older than 10 minutes. The staleness bound is load-bearing: `'queued'` is also the state of an IN-FLIGHT claim, so an unbounded reclaim let two concurrent runs mail everyone twice. **The residual, stated because it is the reachable half:** a process death AFTER Resend accepted the message but BEFORE the status flip leaves the row `'queued'`, and ten minutes later the reclaim will send that person a second time. Given a five-minute send and routine deploys, that is a real window, and it is the accepted cost of not locking a stranded recipient out of the campaign permanently. Listed in `CRITICAL_INDEXES` (`server/db/index.js`): a failed build raises 23505, which initDb's idempotency swallow would otherwise treat as success.
- **Two deliberate reversals from the spec, recorded here because the spec still says otherwise.** (1) The spec called for `ON DELETE SET NULL` on both recipient FKs; that is impossible alongside an exactly-one CHECK, so `client_id` is `ON DELETE CASCADE`. Consequence: deleting a client destroys their campaign send history, including bounce and complaint evidence, and moves the analytics denominators. (2) The spec called for the send to be backgrounded with a durable record. It ships SYNCHRONOUS: the durable per-recipient record exists, but a 500-recipient run holds one HTTP request open for roughly five minutes, and a proxy timeout will show the operator a failure while the send continues. Backgrounding it is its own lane. A third consequence of that same window: `status = 'archived'` is checked once at the start of a run, and archiving a campaign mid-send does not stop it, so the remaining recipients are still mailed. The database ends correct (the release no-ops against an archived row); the mailboxes do not.
- `idx_email_sends_client_sent_partial` on `(client_id, sent_at DESC) WHERE client_id IS NOT NULL` — backs the contact drawer's campaign leg; the table grows by one row per recipient per blast.
...
- **An open moment has THREE states, not two** (2026-08-19). `isLive` = open, undismissed, and somebody to reach. `needsSetup` = open and undismissed but the audience resolves to NOBODY *and* `emptyAudience: 'configure'`, meaning the audience is gated on a human classification. Everything else open is *waiting*: empty because nobody qualifies yet, which only time fixes. Both predicates live in `server/utils/marketingMoments.js` and the Overview tab mirrors them; `GET /api/marketing/overview` returns `open_moment_count` (sendable) and `moments_needing_setup` as SEPARATE numbers, because the header, the badge and the optimistic dismiss all read the first one as "sendable" and widening it in place would have moved all three silently.
- *Why three:* the second state used to be filtered out entirely, so "no moment this month" and "a moment worth thousands whose audience nobody configured" rendered identically — `holiday-corporate` sat open, undismissed and invisible on every surface while `client_tags` was empty in prod, through most of an annual revenue window. The third state is what stops the fix becoming a nag: `one-year-on` and `cold-quotes` are open PERMANENTLY and their audiences are temporal, so an empty one means nobody QUALIFIES yet and only time fixes it — nagging daily about that is what teaches an operator to ignore the surface. (An earlier version of this note justified the clause by claiming `one-year-on` was empty for seven months on a young book. That was WRONG: it counted native `proposals` only, while the audience reads `GREATEST(agg.last_finished, cc.last_event)`, and `legacy_cc_proposals` carries 1,230 rows back to 2024-12-05 — 210 of them inside the 11-13 month window. The clause is right; that example was not.) Each moment declares `emptyAudience: 'configure' | 'wait'` so the difference is authored, not inferred, and moments carry `audience_name` / `audience_rule` so an empty one can say *"nobody matches Past clients · corporate (Paid us · tagged Corporate) yet"* rather than naming an internal id.
...
**consult_call_attempts** — Consult call bridge: one row per (consult, SLOT), the ring-chain state machine AND the call log (spec 2026-08-25)
- `id` BIGSERIAL PK, `consult_id` INTEGER FK → consults (`ON DELETE CASCADE`), `scheduled_at` TIMESTAMPTZ NOT NULL, `UNIQUE (consult_id, scheduled_at)` — the slot is part of the key, not just the payload: a reschedule moves the consult, so the new slot opens a FRESH chain while the old slot's row survives as history. That is also the at-most-once guard under Cal.com's webhook retries and Twilio's redelivered callbacks.
- **`scheduled_at` never round-trips through JavaScript** (ruling R12). node-pg hands a TIMESTAMPTZ back as a millisecond-precision `Date`, so a microsecond slot read into JS and written back is a different value that matches neither the UNIQUE above nor the sweep's `a.scheduled_at = c.scheduled_at` anti-join, and the sweep would open a new chain every 60 seconds until the cap tripped. Every writer takes `consultId` only and derives the slot inside its own SQL; two slots are only ever compared in SQL.
- `status`: pending | calling_admin | calling_va | connected | missed | failed | skipped_cancelled | skipped_invalid_phone | skipped_unconfigured | skipped_disabled | skipped_cap | skipped_missed_window. Twelve values, and the CHECK is defined TWICE (the `CREATE TABLE` body for a fresh database, an idempotent `DO` block for an existing one), so it is contracted in `CONSTRAINT_CONTRACT`: a narrowed list would raise 23514 on the cap-trip insert inside the sweep and the feature would stop with nothing saying why. The cap-trip marker is `skipped_cap`, NOT `failed` (ruling R15): the cap counts `status NOT LIKE 'skipped%'`, so a marker outside that prefix stops a burst of junk bookings on the PUBLIC booking page from sliding the rolling window forward and holding the feature down for 24 hours past the last one.
- `admin_ring` SMALLINT NOT NULL DEFAULT 0 — the row re-enters `calling_admin` up to three times, so status alone cannot tell ring 1's callback from ring 2's. Every admin-leg claim carries `AND admin_ring = $ring`, and a callback whose ring does not match the row's current one returns before writing ANY column, telemetry included (ruling R5): a redelivered ring-1 callback landing while ring 2 is live would otherwise re-arm the chain and speak the apology to a Dallas who just picked up.
- `next_ring_at` TIMESTAMPTZ — when the sweep should place the next ring, written from `RING_OFFSETS_SEC` in SQL rather than a literal interval (ring 1 at the slot minus 90s, ring 2 at plus 60s, ring 3 at plus 180s) and cleared on every terminal transition. Each admin ring is given `ADMIN_RING_SECONDS` 20 to be answered, NOT the 25 the Zul leg gets: `ADMIN_PHONE` is a Google Voice number whose voicemail picks up at about 25 seconds, and a ring that loses that race is ANSWERED by voicemail, which bills the leg and speaks the booker's name and event details into a transcript. Harmonizing the two constants would be silently expensive. A late callback simply writes a `next_ring_at` already in the past and the sweep fires it on its next tick.
- `answered_by` (admin | va), `admin_call_sid` / `va_call_sid`, `admin_call_status` / `va_call_status` (raw Twilio per-leg dispositions; the admin pair always describes the CURRENT ring)
- `bridge_started_at`, `bridge_duration_sec` (client-leg `CallDuration`, digits-only parsed and `COALESCE`d so an at-least-once redelivery carrying a missing value cannot erase a good one)
- `client_no_answer_at` TIMESTAMPTZ — the `/dialend` latch: the bridge connected but the client never picked up. Its own column rather than a string in `detail` (ruling R14), because `placeLeg`'s catch writes a Twilio error code into `detail` earlier in the same chain and would have defeated a detail-based latch, silently dropping the one text.
- `detail` TEXT — purely diagnostic, never a latch: a Twilio error code, `invalid_phone` / `no_phone`, `cap_tripped`, `too_late`, `rescheduled_unresolved`, `stale_reaped`, `rescheduled` (the slot moved out from under a live chain), or the consult's OWN status when the still-scheduled guard fails (ruling R17: a consult flips to `completed` when Dallas saves the drink plan, which is the normal end of a call, so reporting that as `cancelled` would mislead). `guardStillScheduled` can return a fourth value, `missing`, but it cannot land in the column: it means the attempt row itself is gone, and the claim that would write it therefore matches nothing
- `updated_at` is maintained by a BEFORE UPDATE trigger, created if absent rather than DROP-then-CREATE (that shape re-runs on every boot replay and leaves a window with no trigger). Writers may still set it explicitly and several do; the trigger makes the ones that forget correct anyway.
- Indexes: `idx_consult_call_attempts_due` (status, next_ring_at) serves the sweep's fire step; `idx_consult_call_attempts_status_created` (status, created_at) serves the rolling-24h cap count
- **The 60-second sweep** (`consultCallSweep.js`, `RUN_CONSULT_CALL_SWEEP_SCHEDULER`, schedulerHealth name `consult_call_sweep`) is the only clock. One tick, three steps: OPEN consults still `scheduled` whose slot is inside (NOW minus `OPEN_BEHIND_MINUTES` 3, NOW plus `OPEN_AHEAD_MINUTES` 5] with no chain at that slot, validating the number first so an unusable one files the undialable skip instead of opening a chain nobody could dial; FILE MISSED WINDOW for slots inside (NOW minus `MISSED_WINDOW_MINUTES` 30, NOW minus 3], one row and one email per row (ruling R7, never a collapsed digest, because each is a person who waited for a call that never came); FIRE the pending chains whose `next_ring_at` is due, oldest ring first. The two windows are contiguous and disjoint, sharing one bound read from the same exported constants, so a slot belongs to exactly one and nothing falls between them. Every step and every row is separately caught and the last error is rethrown after all three have run, so one malformed consult cannot strand the rest and a failed tick is recorded rather than reported green. `windowConstantsFault` refuses to WIRE the sweep at boot if a window constant is missing or nonsense, because a NULL-bound predicate returns zero rows forever while writing a healthy heartbeat every minute, and this feature's failure mode is silence.
- **The reaper**, riding the hourly VA-calling pass (`vaCallingScheduler.js`), has two modes. Anchored on `scheduled_at`, not `created_at` like its lead-call sibling: the whole ring plan finishes within twelve minutes of the slot, so a slot half an hour gone is unambiguously stranded while a chain opened early for a future slot is not late at all. With the bridge ENABLED, any row still `pending` / `calling_admin` / `calling_va` past `STALE_MINUTES` (30) becomes `failed` + `stale_reaped` and emails once per reaped row (the UPDATE is the claim, so every returned id is a row this pass transitioned). With `CONSULT_CALL_ENABLED=false`, the same rows become `skipped_disabled` and NOTHING is emailed, which is the point of the branch: a switch flipped during an incident must not report every parked chain to Dallas as a system failure. `connected` is never reaped, a live bridge runs to its own `timeLimit`.
- Writers: `consultCallChain.js` (tail skip rows, atomic cap-plus-open, the claim-then-call driver, `sendChainEmail` / `sendMissedText`), `consultCallSweep.js` (which ids to open, file and fire), `voiceConsultCall.js` (press-1 claim, the `/dialend` latch, client-leg duration, terminal hand-off), `vaCallingScheduler.js` (stale reap)
- Alerts: only FAULTS email, through the shared `lead_call` notification category (undialable number, missed window, cap tripped, chain failed, stranded and reaped, and an unresolved reschedule that stopped MORE than one of a booker's upcoming consults; a single stopped sibling is the ordinary moved-slot case and stays a log line). A chain that rang and connected nobody TEXTS Dallas instead, from `TWILIO_PHONE_NUMBER` to `VM_TEXT_DESTINATION` falling back to `ADMIN_PHONE`, with `skipLog` so an internal ops alert never enters the client message ledger; if that text cannot go out the missed transition sends the email instead, so a fully missed consult is never invisible.
...
REPURPOSED 2026-07-07 (cc-import phase-2 reboot): the v1 importer and its Review pages are retired; these tables are now the frozen **CC-era ledger**, loaded once by `scripts/cc-ledger-import.js` from the final 2026-07-06 CheckCherry exports (archived off-repo) and consumed read-only by the metrics layer. Loader uses replace semantics (`--replace` truncates and reloads; the export files are the source of truth) with verification gates that tie row counts and money sums to CC's own P&L exports to the penny, plus a double-count guard that refuses to load when native tables carry v1-promoted CC rows (`proposals.cc_id` / `proposal_payments.legacy_charge_id` — blended metrics would count that money twice; `--allow-promoted` bypasses on contaminated dev only). `raw_import_id` is now nullable on all three ledger tables (the raw layer is the archived files, not staging rows). PHASE 3 (2026-07-07): future CC events transferred to native proposals by `scripts/cc-transfer-events.js` live in exactly ONE era — the loader skips ledger events whose `cc_id` appears in `proposals.transferred_from_cc_id` on any reload (skip count reported, post-apply expectations adjust); their CC-era deposit PAYMENTS stay in the ledger (cash-basis truth).

Metrics consumption (`server/utils/metricsQueries.js` + `/api/proposals/dashboard-stats` and `/financials`): the dashboard `include_cc` tri-state means **all** = native + CC-era ledger (additive scalar legs), **exclude** = native only, **only** = ledger only. Money legs ride `legacy_cc_payments.payment_applied_cents` (signed) over `paid_on`; funnel legs ride `legacy_cc_proposals` (`cc_created_at` = quote axis, `booked_at` = conversion axis, `total_cost_cents` = value). Era-closed by design: outstanding, pipeline, pending, time-to-accept, lost-value, and paid-status counts never read the ledger.
...
4. **Add-ons**: Calculated per billing type (per_guest, per_guest_timed, per_hour, flat). `per_guest_timed` is the one billing type whose `rate` is NOT the whole per-guest price: it is the FOUR-HOUR price, and every hour past four adds `extra_hour_rate` per guest (`calculateAddonCost`). Four catalog rows carry it today (the-foundation, the-formula, the-full-compound, mocktail-bar) and more will follow, so no display code may key on those slugs. Every surface that prints such a rate, client-facing or admin, renders it through `client/src/utils/addonRateLabel.js` (`timedPerGuestRateLabel`), which produces the qualified string "$8/guest (4hr) + $2/guest/hr after" and degrades to the "(4hr)" floor when a column is too narrow or `extra_hour_rate` is absent. A bare per-guest-timed rate on a client surface is a quote the client can hold us to; do not hand-roll the qualifier.
5. **Syrups**: Three-pack bundles ($30) + singles ($12), optimized for best price
6. **Gratuity** (client-elected at sign-and-pay and persisted only at payment, §8.3 — OR admin-mandated pre-payment, spec 2026-08-10): `gratuity_rate × staffCount × hours` where staffCount = bartenders + additional-bartender addon (NOT barbacks/servers). Added on top of the total as staff pass-through money (never diluted by discounts/override). Layered on the forced `"Shared Gratuity"` over-ratio surcharge. Helpers: `getStaffNoun`, `computeGratuityBasis`, `gratuityLineAmount`, `deriveGratuityRate` (dollars-total → stored rate with floor/sanity validation, re-asserting the floor on the derived ROUNDED rate), `recomputeSnapshotGratuity` (drift-free checkout recompute). Labels live in the shared `server/utils/gratuityLabels.js` (client mirror `client/src/utils/gratuityLabels.js`). The admin proposal editor and `PATCH /:id` have exactly ONE gratuity write path — the mandate field `gratuity_mandate_total` (spec 2026-08-10, resolved in `server/utils/gratuityMandate.js`: dollars > 0 sets `gratuity_rate` + `gratuity_floor_rate` together, null clears a real mandate, absent carries forward, locked once signed or paid); the election keys `tip_jar`/`gratuity_total` stay refused, the stored rate/jar otherwise carry forward untouched (rescaled at the same rate on a staffing change, `origin='staffing'`), and post-payment removal goes through cancel-line-item (which also clears the mandate). A mandate floors the client's checkout election on BOTH jar answers, replacing the $50 no-jar floor. `POST /calculate` previews the STORED rate, or a DRAFT mandate when `gratuity_mandate_total` is sent.
...
The result is stored as a `pricing_snapshot` JSONB on the proposal for historical accuracy. Payroll pools both gratuity labels (`payrollMath.extractGratuityCents`) and gates the gratuity portion of accrual on the proposal being funded (`amount_paid >= total_price`); the Stripe webhook records the amount actually charged (additive), never a flat `= total_price`, so a mid-flight total change can't misstate `amount_paid`. On first delivery that same webhook also APPLIES the client's tip-jar election from the intent metadata, before the credit and before `createBalanceInvoice` (so the Balance invoice sees the gratuity-inclusive total); the write is metadata-validated against the DB CHECK and bracketed in a `SAVEPOINT`, so a failed gratuity apply never rolls back the payment record. Payment-status demotion + overpayment detection is centralized in `server/utils/proposalStatus.js` (`reconcileProposalPaymentStatus`).
...
- **`server/utils/proposalRules.js`**: the CJS server twin. Exposes TWO gates, and the split matters. `validateProposalRules` is the authoritative INTERACTION gate: bundle mutex, mixer mutex, hosted-package guest minimum, glassware-dependent add-ons, parent-addon requirements, guest-count ceilings. It does NOT enforce the VISIBILITY rules that decide which add-ons a client may pick at all (`applies_to` lane gating, the hidden parking fee, the >100-guest glassware cap) — those lived only in the browser until `visibleAddonsFor` was added here as the CJS twin of the client's `filterAddons`. Any server path that prices or persists a client-chosen add-on set must gate on `visibleAddonsFor` FIRST and then `validateProposalRules`; the client copies are convenience, never the boundary. Together they stop a stale browser tab or a scripted POST from bypassing either class of rule. It throws `ValidationError` on a bad selection. The rule gate and `stripIncludedAddons` run on every proposal-write path: admin `POST /proposals`, admin `PATCH /:id` (edit), and the public `POST /api/proposals/public/submit`.
...
- **`server/utils/paymentInFlight.js`**: bank debit in flight (spec 2026-09-14). The ONE definition of "a payment on this proposal is still settling": a `stripe_sessions` row in `processing` less than 14 days old. `findInFlightPayments` feeds the public invoice and proposal payloads, the admin invoices payload and the balance reminder ladder (which defers a day instead of nagging); `IN_FLIGHT_LATERAL_SQL` rides the polled payment-state route in one round trip; `assertNoPaymentInFlight` is what the three checkout rails call before minting: a processing row refuses from the DB, then the newest recent pending or failed intents are read from Stripe (one retry, ten-second timeout) and refused when processing, succeeded, or awaiting microdeposit verification. Unknown fails closed (502). `findStaleProcessingPayments` feeds the hourly monitor's Sentry warning for a row older than 8 days.
- **`server/utils/bankPaymentProcessingNotify.js`**: post-`payment_intent.processing` helper. One client email ("We received your bank payment", email only, gated like the receipt) and one admin email (urgent_booking for a deposit or full payment, routine_finance otherwise), fired after the handler releases its connection. Best-effort, never throws.
...
`publicReadLimiter` (same file, 100 requests/15min per IP) carries the read-only public pricing endpoints — `GET /api/proposals/public/packages`, `POST /api/proposals/public/calculate` (a POST in verb but a pure pricing preview, no write), and `GET /api/proposals/group/:token` — because they share a visitor's IP budget with proposal loads and the quote wizard; the 20/15min `publicLimiter` stays on the public write endpoints (capture-lead, submit, and `POST /api/sms/opt-in`).
...
- **"Is the client overpaid?" is not `amount_paid - total_price`.** Syrup-only `Drink Plan Extras` and free-text manual invoices (Damage Fee, Travel) roll into `proposals.amount_paid` and never into `total_price`, so the raw difference counts money the client still owes for goods DRB is still buying. `invoiceExtras.sumOffContractPaidCents()` is the one answer: it nets collected money whose label is outside `IN_TOTAL_PRICE_LABELS` (Deposit / Balance / Full Payment / **Additional Services** / **Enhancement Lab** — the last two ARE contract money, minted from or folded into `total_price`). `Drink Plan Extras` cannot be classified by label at all, so it is decided by persisted line items via `extrasLinesAreFolded()`: any add-on or bar-rental line means the submit transaction folded the whole invoice into `total_price`; syrup-only means the fast path folded nothing. Consumed by the cancel-line overpayment computation. Do NOT substitute `CONTRACT_LABELS` here — that is a refund-scope classification, and using it under-refunded a client by $300 in review.
- **Balance-invoice monitor**: `server/utils/balanceInvoiceMonitor.js` watches one invariant hourly and is **alert-only** — it reads `proposals`/`invoices`, writes only `proposal_activity_log`, and never creates, edits, or voids an invoice. It reports both directions: a client billed MORE than `total_price − amount_paid` across their open invoices (never legitimate), and a client owing a balance with no payable invoice (the 2026-07-20 incident, where eight clients were dunned toward a page with no pay control). One Sentry warning plus one `proposal_activity_log` row per offending proposal, and one batched admin email per run, throttled to once per 24h per proposal via `balanceScheduler`'s marker pattern. Gated by `RUN_BALANCE_INVOICE_MONITOR`.

  **The invoice DERIVATION is deliberately unchanged.** `refreshUnlockedInvoices` still derives a Balance from `total_price − external_paid − Σ(locked invoice amount_due)` and still skips every label outside Deposit / Balance / Full Payment. Both are known defects — they over-billed four clients in July 2026 — and a rewrite was built, reviewed, and PULLED on 2026-07-28 after two review rounds found new money bugs in it. The blocker is architectural: `invoices.amount_due` has four writers that do not agree (`refreshUnlockedInvoices`, `invoiceExtras.findOrRefreshExtrasInvoice`, `lineItemCancel.reconcileOpenDeltaInvoices`, and `routes/drinkPlans/lab.js`, which runs its own copy of the discredited locked-sum formula and overwrites the refresh in the same request), and `Drink Plan Extras` carries two different money semantics under one label depending on submit path. Read `docs/superpowers/specs/2026-07-28-invoice-derivation-and-monitor-design.md` before attempting it again. Until then this monitor is the safety net.

- **Autopay**: When enrolled, Stripe saves the payment method via `setup_future_usage: 'off_session'`. A Stripe Customer is created for the client. Balance is auto-charged on the due date (default: 14 days before event) by the hourly scheduler in `server/utils/balanceScheduler.js`. A durable charge record (`server/utils/autopayDurableCharge.js`) persists each balance PaymentIntent to `stripe_sessions` at charge time and guards a stale-claim re-charge against Stripe's actual intent status, so a >24h webhook outage can't drive a second off-session charge (F1).
- **Off-session charges**: Admin can manually trigger via `POST /api/stripe/charge-balance/:id` or the scheduler runs hourly.
- **Alternative**: Admin generates a reusable Payment Link via `POST /api/stripe/payment-link/:id`. Amount follows the same booking-window policy as the client flow — full total inside the 14-day window, otherwise the $100 deposit — and tags the link with `metadata.payment_type` so the webhook settles it correctly. It carries NO gratuity election metadata, so a link-paid proposal never collects a prepaid gratuity (deliberate consequence of election-at-payment: the election belongs to the client's sign-and-pay moment).
- **Webhook events**: `payment_intent.succeeded` (deposit, full, balance, drink-plan-extras), `checkout.session.completed` (deposit OR full via Payment Link, read from `metadata.payment_type`, AND bartender tips, branched on `metadata.kind`); `payment_intent.processing` (a bank debit confirmed, money not yet moved: `stripe_sessions.status = 'processing'`, and every checkout rail, the balance reminder ladder, the autopay guard and the public payloads read it through `utils/paymentInFlight.js`); `payment_intent.payment_failed` (a decline, or a bank debit that bounced days later: releases the processing state); `refund.created` (THE refund reconciler: a refund issued in the Stripe dashboard lands in the ledger, and an in-app one is backstopped) and `charge.refunded` (the payroll tip clawback only, which reads the version-proof `amount_refunded`). The live endpoint must subscribe to all of these; the processing and failed events were added 2026-09-14, the two refund events 2026-09-15
...
- **Partial refunds**: Admin-issued via `POST /api/stripe/refund/:id`, which takes a `total_scope` of `'contract'` (default) or `'overpayment'`. `planRefund` (in `server/utils/refundHelpers.js`) auto-targets the largest refundable charge (no spanning); under overpayment scope it prefers the charge carrying the most UNCREDITED headroom, but only among charges that can cover the amount, so the no-spanning rejection still names the true maximum. The refund ORCHESTRATION (pending row → `stripe.refunds.create` → `applyRefundReconciliation` in a tx → redundant-pending-row cleanup) lives in `server/utils/refundExecute.js`, shared by this route and the cancel-event refund endpoint so no caller ever calls `stripe.refunds.create` directly. `applyRefundReconciliation` (in `refundHelpers.js`) holds a row-lock on the proposals row, applies Approach-A label-conditional `total_price` correction (Deposit/Balance/Full Payment invoices adjust the contract total; extra-scope invoices leave it intact), nets the aggregate invoice reversal, and writes to the activity log. The synchronous route does the Stripe API call then reconciliation; an idempotent `refund.created` webhook handler is the backstop that self-heals a failed post-Stripe write AND records out-of-band Stripe-dashboard refunds. The partial unique index on `proposal_refunds.stripe_refund_id` (WHERE NOT NULL) is the shared idempotency anchor.

  **Overpayment scope (spec 2026-09-15).** A refund that returns money held beyond the contract must not lower `total_price`, or the proposal stays overpaid by the same figure and the contract shrinks on every attempt. The admin chooses on the payment panel; the default and the cap come from `overpaymentCents` (`amount_paid - total_price - sumOffContractPaidCents`, the same netting cancel-line uses, exposed on `GET /api/proposals/:id` as `overpayment_cents`). The cap is asserted inside the transaction that writes the pending row, holding the proposals row lock, so two concurrent submits cannot both spend it. Reconciliation consumes the payment's UNCREDITED headroom first (`linkPaymentToInvoice` caps an invoice credit at its remaining due while the webhook rolls the whole intent into `amount_paid`, so an overpaying payment is part-credited): those cents are on no invoice, so refunding them reverses no invoice. On the dashboard path the scope is `'overpayment'` when Stripe's reason is `duplicate` OR the netted excess covers the refund, never from the reason string alone.
- **Cancellation refunds** (P6/fix #7): `POST /api/proposals/:id/cancel/refund` loops `refundExecute` per refundable charge largest-first (a booked event has deposit + balance charges, which `planRefund`'s single-charge model can't span), until the `cancellationMath` target is met. Each refund row is stamped with its `gratuity_cents` attribution. Refunded money is not income — the standard reconciliation nets it out of collected/income; the original contract total is preserved in the cancellation audit note. The money-out target is capped at a lifetime ceiling (the cancel-time `refund_owed_cents` activity-log snapshot plus post-cancel succeeded-payment headroom) so a retry after a mid-loop failure cannot over-refund via the retainer feedback loop. The retainer itself (`assembleContext` in `cancel.js`) is the paid amount on a `Deposit` invoice, plus, for a Deposit that `upgradeDepositInvoiceToFull` or the backfill relabelled `Full Payment`, the deposit amount recorded in that row's `invoice_upgraded_to_full` / `invoice_backfilled_to_full` breadcrumb capped at the row's `amount_paid`; a native Full Payment invoice carries no retainer. Decided 2026-08-28: a deposit-terms client who pays in full still forfeits the retainer (agreement 2.1 defines it by the amount the proposal stated).
- **Cancel line item** (2026-07-24): `POST /api/proposals/:id/cancel-line` removes one client-visible priced line and settles the money in a single act. `server/utils/lineItemCancel.js` holds the per-kind target registry (add-on / bar / syrup / extra-bartender / adjustment / gratuity; the package line hands off to the cancel-event flow), mutates, then reprices through `foldExtrasIntoProposal` so a negotiated `total_price_override` moves by the catalog delta. The removal COMMITS first; refunds fire post-commit, sequentially per charge, with `total_scope='overpayment'` so reconciliation never re-lowers the already-folded total. `POST .../cancel-line/preview` runs the identical core inside a rolled-back transaction, so the confirm button restates exactly what execute will do, guarded by an `updated_at`/`amount_paid` fingerprint (409 `STALE_PREVIEW`). Money paid outside Stripe surfaces as `manual_return_cents` (returned by hand, never auto-refunded). Two notice modules ride the flow: `server/utils/gratuityStaffNotice.js` sends the MANDATORY staff "you can set out a tip jar" email whenever a gratuity is removed or lowered under the $50 no-jar floor (approved non-dropped roster, email only), and `server/utils/lineItemRemovedNotify.js` sends the notify-gated client notice when a removal owed no refund.
- **Service extensions (on-site added bar time, 2026-08-03)**: a staff request mints a `Service Extension` invoice (off-ledger label, see `OFF_LEDGER_INVOICE_LABELS` under Database Schema > Invoices) and the client pays it through the normal InvoicePage rails, gated on a server-recorded terms acceptance in `create-intent-for-invoice`. The webhook discriminates an extension payment by the `service_extensions.invoice_id` lookup, skips the `amount_paid` roll-up, and runs the settle tail post-commit (`settleExtension` moves `event_duration_hours` and the shift end time, payroll hours re-seed, staff get the greenlight, `finalizeExtension` stamps completion). Extension payments are excluded from the refund planner's candidate list, so the admin refund cannot land on them; refunding one is a manual Stripe-dashboard act by design (procedure: `docs/ops-runbook.md` > "Service Extension refunds"), with the `refund.created` webhook adopting the dashboard refund without touching contract money (the off-ledger label zeroes both the contract portion and the `amount_paid` drop, whichever scope it lands in). The 60-second expiry sweep (`RUN_SERVICE_EXTENSION_SWEEP_SCHEDULER`, `serviceExtensionSweep.js`) enforces the hard stop and heals crash-stranded settles. Full route table under API Routes > Service Extensions; schema under Database Schema > Service Extensions.
- **Stale-pending refund recovery**: a `proposal_refunds` row is written `pending` before `stripe.refunds.create`; ambiguous Stripe errors (connection/API) now leave it `pending` rather than `failed` (a false `failed` re-opens refund headroom and enables a double-refund). `refundSweepScheduler.js` (`RUN_REFUND_PENDING_SWEEP_SCHEDULER`, every 15 min) reconciles rows stuck `pending` >30 min against `stripe.refunds.list` — adopting the real refund via `applyRefundReconciliation` (matched by the `proposal_refund_row_id` metadata anchor, else a unique amount) or marking it `failed` only when Stripe confirms no refund exists.
- **Refund-to-zero reaping** (M-1): a full refund that zeroes `amount_paid` demotes the proposal to `accepted`, which is still active everywhere. The admin refund UI therefore prompts to archive; `POST /:id/archive` now reaps like the cancel flow (via the shared `shiftReap.js`): soft-cancels shifts, denies open shift_requests, deletes pending proposal-level scheduled_messages, and voids unpaid invoices, so a refunded booking no longer lingers on the staff open-shifts feed or the dunning ladder. Partial refunds never auto-kill. The public invoice and drink-plan intent routes reject payment on an archived proposal (409 `EVENT_CANCELLED`), and the settlement webhooks alert on any payment that still lands on an archived proposal.
...
- **Marketing**: `server/routes/emailMarketing/` — leads, campaigns, sequences, conversations, analytics, behind a composition router (`index.js`). Split from a single 987-line file in lane mkt-e; paths, middleware, and mount order are unchanged. Mount order in `index.js` IS behavior: `designer.js` and `sequences.js` register paths more specific than `campaigns.js`'s `/campaigns/:id`, and Express matches in registration order. Verify any change with `node scripts/route-inventory.js server/routes/emailMarketing` before and after, and diff.
...
- **Greetings**: `server/utils/firstName.js` — `firstNameOf()` is the single source for the name in every "Hi ..." line across all 10 template files (client- and staff-facing). Title-aware (shares the `TITLES` set with `staffDisplayName.js`, so "Dr. Monica Donnely" greets as "Monica") and idempotent, so it is safe at a site whose input may already be a first name. Apply it AT the greeting, never at the `const name =` above it: several of those variables also feed admin-facing copy that must keep the full name. **Couple-aware**: a name joined by `&`, `+`, or `and` with whitespace on both sides is split, narrowed per side, and rejoined with the joiner the data used, so "Aubrey Smith & Dominic Jones" greets as "Aubrey & Dominic" instead of dropping the partner. Nine client rows are stored this way and nearly all are weddings; requiring whitespace on both sides of the joiner is what keeps "Alexander" and "Andrea" intact.
...
- **Inbound flow (two lines since Phase 1a, spec 2026-07-26)**: the phone system is TWO-LINE. The 0082 is Zul's: `POST /api/voice/inbound` forwards to `VA_CELL` and stamps `?line=zul` on its `<Dial action>`. The 1922 is the primary business number: `POST /api/voice/inbound/primary` dials `VM_PRIMARY_DIAL_TARGET` (Dallas's 312 GV, env-only strict E.164) with a deliberately short `VM_PRIMARY_RING_SEC` so a re-enabled GV voicemail can never intercept, stamping `?line=primary`. One SHARED missed handler serves both, keyed on the HMAC-covered `line`: it picks the greeting, the escalation target, and the delivery channel (zul → Telegram audio; primary → SMS to `VM_TEXT_DESTINATION` carrying a listen link, recording retained until 1b). Carrier voicemail is OFF on `VA_CELL` (owner-confirmed 2026-07-22), which is what makes the miss detectable: the dial rings out, ends `no-answer`, and Twilio requests `POST /api/voice/inbound/missed`. VERIFIED 2026-07-24 by a live test call: a CALLER mid-ring hang-up (well before the 20s timeout) DOES cause Twilio to request the action URL with a `MISSED_STATUSES` value, so it inserts a `voicemail_delivery` row and pings Zul, the same as a full ring-out. Twilio's docs never documented this; the live call settled it. `VOICEMAIL_ENABLED` was flipped on in Render on 2026-07-24 and the full flow (ping → recorded → delivered → Twilio recording deleted) was confirmed against prod. An instant answer on the primary line is logged by the interception canary — LOG-ONLY since 2026-08-14, no Sentry page (the detector reads connected time, not time-to-answer, so it fired on fast humans and stayed silent for real interceptions; check the `[voice/missed] ... dialSec` console line, not Sentry).
- **Press-1 escalation (spec 2026-07-26 section 3, ships dark behind `VM_ESCALATION_ENABLED`)**: the missed DAY greeting is wrapped in a `<Gather>` whenever a digit could arrive — escalation on, or the greeting already saying "press one" (`dayGreetingOffersPress1`, 2026-08-20). A caller who presses 1 reaches `/api/voice/escalate`, which tries the OTHER person before voicemail, or goes straight to the `escalate_failed` recording when the switch is off. Never inviting a digit nobody is listening for is the invariant; the switch's job is the DIAL, and it still stops every one of them. The billed leg (international when the target is `VA_CELL`) is guarded four ways: the kill switch, a single-use claim (`escalated_at IS NULL` on the row's PK), a HARD rolling daily cap (`claimEscalationUnderCap` serializes cap-check+claim behind `pg_advisory_xact_lock(74209,0)` with a 3s lock_timeout, pinned to READ COMMITTED), and a per-target quiet window. The dialed leg gets a `<Number url>` whisper requiring a KEYPRESS to bridge, so an unattended carrier voicemail is screened out; `escalate/done` branches on the recorded acceptance state, never on `DialCallStatus`. No keypress at the greeting still falls through to `<Record>` with no extra route.
- **Caller communication (spec 2026-08-19)**: the flow narrates itself, because escalation shipped with two silent gaps. Pressing 1 now plays an acknowledgment (`escalate_ack`) BEFORE the `<Dial>`; previously the route answered the `<Gather>` with a bare `<Dial>`, so the caller tapped a key and got dead air for up to 20 seconds and often pressed again into a document that had already left the `<Gather>`. Every return to voicemail plays `escalate_failed` before the beep, which covers seven of the eight branches of `voiceEscalate.js` with one edit (the rate-limiter handler carries the same message inline, because no `line` binding exists that early); previously that was a bare `<Record playBeep>`, and an unexplained tone after 20 seconds of ringing reads as a dropped call rather than an invitation. Inside `VM_NIGHT_WINDOW` (default `21:00-09:00` `America/Chicago`, the literal `none` disables night entirely, evaluated by `isNight()` in `voicemailLine.js`) the missed document drops the `<Gather>` entirely and the greeting promises the morning. That is deliberately NOT a business-hours rule: Zul keeps Chicago-aligned hours, so Chicago night is her off time too and a night press-1 rings a sleeping phone in the Philippines for a predictable failure. No copy claims we are open or closed, so nothing here can contradict the published client-facing hours. `isNight` shares the window machinery with the quiet hours (`parseWindow`, `minutesInZone`, and one `inWindow` half-open comparison, so a wrap-around fix lands in one place) and fails OPEN to day, so a config typo produces a rung phone rather than a caller silently denied a human. Four message slots per line (`greeting_day`, `greeting_night`, `escalate_ack`, `escalate_failed`) resolve through `messageVerb()` on the `VM_GREETING_URL` contract: unset takes the default, `say` forces synthetic (the kill switch for a bad recording), a URL is `<Play>`ed. Both lines default to bundled recordings served by `voiceAssets.js`: Dallas's four and Zul's four, all recorded 2026-08-19. Her 2026-07-24 mp3 is still served at its original `/api/voice/greeting.mp3` path for anything referencing that URL directly, but it is no longer any slot's default. The press-1 offer now lives INSIDE the day greeting, so `needsAppendedOffer()` trusts only the KNOWN defaults, and since 2026-08-19 both lines' defaults say the offer themselves, so nothing is appended by default. An override URL has UNKNOWN content, so it gets the offer appended on EITHER line. That direction is deliberate: a doubled offer is an obvious stutter, while a missing one loses the feature invisibly behind a silent four-second `<Gather>`. Set `VM_ESCALATION_PROMPT=none` to suppress the append after wiring a recording that already says it. The old global `VM_ESCALATION_PROMPT` could not be set correctly once the lines diverged: off hid the option from her callers, on made his line say it twice.
- **Public phone constants (client)**: voice and text are two DIFFERENT numbers since Phase 1a: `COMPANY_PHONE`/`COMPANY_PHONE_TEL` = the 1922 for calls, `COMPANY_TEXT_PHONE`/`COMPANY_TEXT_PHONE_TEL` = the 888 for texts (the 224 numbers have no approved A2P SMS campaign until Phase 2, when the pairs reunite on the 1922). Do not collapse them: repointing texting at the 1922 sends client texts to a number that silently does not deliver.
- **Voicemail (spec `docs/superpowers/specs/2026-07-22-voicemail-224-telegram-design.md`)**: `/inbound/missed` checks `VM_DAILY_CAP` FIRST (a rejected call must not write a row that extends the window), claims a `voicemail_delivery` row (the ping dedup, stamping `line`), returns the per-line greeting + `<Record>`, and only THEN pings Zul on Telegram — zul-line misses only; a primary miss shows natively on the GV dial target, so it pings nobody (response first, so a hung Bot API can never leave a live caller in dead air). The ping is two messages when the caller is NANP: prose, then the bare number alone, because the bot's own trigger runs `toUsE164` over the whole message text. `<Record>` carries no `action` attribute on purpose (Twilio skips it when the caller hangs up, which is the normal case); delivery hangs off `recordingStatusCallback` → `POST /api/voice/inbound/voicemail`, which claims delivery, fetches the mp3 from a URL CONSTRUCTED from the account SID + a `^RE[0-9a-f]{32}$`-validated `RecordingSid` (the body's `RecordingUrl` is never read), uploads it to the same chat, and deletes the Twilio recording **only** on an affirmative `ok === true`. A gated `skipped` send and a `failed` send both keep the recording. Both voicemail webhooks fail CLOSED on signature in every environment. Master switch `VOICEMAIL_ENABLED`, default off. **Listen link (spec 2026-08-10):** every row carries a `listen_token` UUID, and the primary line's alert SMS ends with `<API_URL>/api/voice/vm/<token>`, which streams the retained recording so the message can be heard from the text instead of the Twilio console. The token is the only auth, acceptable ONLY because the alert goes to `VM_TEXT_DESTINATION` (the operator, who can already hear every voicemail in the console) — explicitly NOT the proposal/invoice precedent, since the audio belongs to a third party. Two-sided kill switch `VM_LISTEN_LINK_ENABLED` (default on) closes the route AND drops the line from the text so no dead URL is ever sent; revoke a single link with `UPDATE voicemail_delivery SET listen_token = gen_random_uuid() WHERE call_sid = $1`. Links expire at 30 days, enforced in the route's own SQL rather than borrowed from the prune's status allowlist.
- **Status feedback**: `POST /api/voice/status`; a failed/unanswered leg messages Zul via Telegram so she always learns the outcome.
- **Toll-fraud guards** (this webhook dials billed international calls on an auto-refill account from external input): (1) Telegram `secret_token` header + unguessable secret URL path, (2) numeric `user_id` allowlist layered on top, (3) US-only NANP validation in code, (4) confirm-before-dial with a TTL'd pending record (a new target replaces it), (5) claim-then-call idempotency + `telegram_update` `update_id` dedupe, (6) DB-backed spend caps (5 triggers/min, 40 calls/day counted from `call_audit`) + a 1800s per-call `timeLimit` on both legs, (7) shared `xmlEscape` on every TwiML interpolation, (8) `notificationsEnabled()` gate so a dev server never dials the live account, (9) a daily webhook heartbeat (`getWebhookInfo` → re-`setWebhook` + admin email on failure), (10) last-4 log redaction and a retention purge of the PII-bearing `call_audit`, (11) `VM_DAILY_CAP` on the voicemail path (the inbound analog of `VA_CALL_DAILY_CAP`: a missed call used to cost nothing after ring timeout and now costs greeting + up to `VM_MAX_LENGTH_SEC` of billed recording) plus a per-`CallSid` limiter and fail-closed signature checks on both voicemail webhooks, (12) the press-1 escalation's own stack: `VM_ESCALATION_ENABLED` kill switch (default off), the single-use `escalated_at` claim, the HARD `VM_ESCALATION_DAILY_CAP` (cap-check+claim serialized on an advisory lock so concurrency cannot overshoot it), the quiet windows, and the keypress whisper screen; every escalation dial target is env-only strict E.164, (13) signature-BEFORE-limiter ordering on both forward routes so an unsigned flood can never drain the constant-key buckets and busy-out real callers.
- **Helper modules**: `server/utils/telegram.js` (Bot API, raw fetch, no SDK), `server/utils/pendingCall.js` (claim-then-call DB helpers + audit + prune), `server/utils/usPhone.js` (`toUsE164`/`isUsE164`), `server/utils/xmlEscape.js` (shared TwiML escape), `server/utils/vaCallingScheduler.js` (prune + line-aware undelivered-voicemail redelivery sweep + webhook heartbeat), `server/utils/voicemail.js` (voicemail ledger, per-line delivery, line-aware `alertOperator`, constructed-URL Twilio media fetch/delete), `server/utils/voicemailLine.js` (per-line routing policy: `resolveLine`, `E164_RE`, escalation targets, quiet windows, `isNight` for the caller-facing night window, ring clamp, interception canary), `server/utils/voicemailTwiml.js` (shared TwiML fragments so the two emitting routers cannot drift; owns the four message slots and `messageVerb`), `server/utils/voicemailEscalation.js` (escalation claim/cap/acceptance/outcome ledger), `server/utils/sms.js#placeBridgedCall` (Twilio `calls.create`, gated identically to `sendSMS`).
- **Tables**: `pending_call` (confirm-before-dial state, one row per user), `call_audit` (spend/abuse audit + daily-cap source of truth), `telegram_update` (`update_id` retry dedupe), `voicemail_delivery` (one row per missed inbound call: ping dedup claim + `VM_DAILY_CAP` window + delivery ledger). Pruned by the `RUN_VA_CALLING_SCHEDULER` job, which also runs the undelivered-voicemail redelivery sweep.
...
- **Sender**: `server/utils/pushSender.js` — `sendPush()` configures VAPID once at module load inside a try/catch, so a missing or malformed key fails closed (`vapid_unset`) and never crashes boot. Maps `410`/`404` → `{ gone: true }` so the dispatcher prunes dead subscriptions; all other errors are transient. Sole caller is `pushDispatch.dispatchPushRow` (invoked by the dispatcher's push-channel branch).
- **Subscriptions**: stored per-user in `users.staff_notification_preferences.push_subscriptions[]` (JSONB array, 10-cap, evict-oldest), written via `POST` / `DELETE /api/me/push-subscriptions`.
- **Client**: service worker `client/public/staff-sw.js` (`push` → `showNotification`, `notificationclick` → focus/open), registered by `client/src/utils/pushSubscribe.js` (`permissionState` / `subscribePush` / `unsubscribePush`). iOS only delivers push to a home-screen-installed PWA, so a staff-scoped manifest (`client/public/staff-manifest.json`) + Apple standalone meta tags are injected on the `staff.` host by `client/src/utils/installStaffPwaMeta.js`. The Notifications settings drive the flow (`PushPermissionBanner` permission states + `IOSCoachmark` install walkthrough).
- **Channel routing**: push is one of three channels (push / sms / email) selectable per notification category; `notificationChannelResolver.js` keeps an email fallback for critical categories so a push-only preference never silently drops a critical message.
- **Public opt-in form**: `POST /api/sms/opt-in` (`server/routes/smsOptIn.js`, no auth, `publicLimiter`) is the submit for the standalone `/sms` page, built after two Twilio A2P 10DLC rejections (30909, then 30896) whose cause was that the quote wizard's consent checkbox sits on step 2 of `/quote` where a carrier reviewer never sees it. **The checkbox is OPTIONAL, and the number is required only when it is ticked** — a third rejection ("Forced Consent Violation", 2026-08-03) landed because requiring both meant a consumer could not decline texts and still use the form. An un-ticked box is therefore a valid submit, and it takes a **completely separate path that never touches `clients`**: it upserts `email_leads` (the real marketing list) and returns. Any number supplied is ignored and stored nowhere. It never stamps `sms_opt_out_at` either, because not ticking a box is not a STOP and that field blocks every later opt-in as `prior_opt_out`; the no-consent path never calls `recordSmsConsent` at all. **Both halves of that design are load-bearing and were paid for.** The first version created a `clients` row with `sms_enabled` forced false, and the review fleet found two defects in it: a client row here is a row the submit "owns", so a LATER ticked submit from the same address resolved as `existing_client`, recorded nothing and still showed a success screen (declining once meant never being able to opt in through this form again); and because that branch skipped `recordSmsConsent` it bypassed the phone-scoped STOP guard, so posting a stranger's stopped number under a different name committed a second row carrying it, which `smsInbound`'s newest-row-wins `lookupSender` would then resolve STOP/START against instead of their real row. Writing to `email_leads` removes both by construction: no client row to own, no column to hold a number. `status` is deliberately excluded from the upsert's `DO UPDATE` list so a public form cannot resurrect an unsubscribed lead. Consent semantics on the ticked path are entirely `utils/smsConsent.js` — the ownership gate (`subjectIsNew`), the row- and phone-scoped STOP guards, and the `sms_consent_log` audit row (`source_form='sms_page'`). Where the wizard has to keep a client row whose consent did not record and force `sms_enabled=false`, this route's **ticked** path rolls the whole submit back instead, because nothing depends on the row. Every consent outcome returns the same generic 200 so the form cannot be used to probe which emails are clients or which numbers have opted out.
...
### Admin PWA (Mobile Admin, spec 2026-08-13)
- **Surface**: the admin host is installable (spec section 7). `client/public/admin-manifest.json` ("DrB OS", `start_url /events`, standalone) + placeholder icon set (`client/public/admin-icon-*.png`, regenerated by `scripts/make-admin-icon.js`; the design fit-back lane replaces the art, filenames are stable). Metas injected at runtime by `client/src/utils/installAdminPwaMeta.js` behind the admin host gate (`admin.*` / bare localhost, mirroring `getSiteContext`); it also mutates the existing `theme-color` meta in place and amends the viewport with `viewport-fit=cover`. The staff injector/manifest/SW are untouched siblings.
- **Service worker**: `client/public/admin-sw.js` (registered by `client/src/utils/adminSw.js` from AdminLayout). Push display + notification-click deep links (staff-sw pattern). Offline: at install the SW fetches `/asset-manifest.json` and precaches ONLY its `entrypoints` (Chrome's memory cache sits above the SW, so the entry bundle can bypass the fetch handler on a controlled reload; lazy chunks runtime-cache organically on cold-session navigations, and a full-manifest sweep was rejected in review as 22.6MB per SW update); network-first navigations with cached `index.html` fallback (only an `ok` document may become the shell); cache-first ONLY for content-hashed `/static/` assets (the dev server's stable bundle names are deliberately excluded so a dev box can never pin a frozen bundle); allowlisted phone-surface GET `/api/` reads cached per-user (`admin-api-<ver>-u<id>`), served ONLY on transport failure or a 4s timeout race, stamped `x-sw-cached-at` (surfaced as `res.staleAt` by `utils/api.js` `markStaleFromHeaders`, rendered via `utils/staleTime.js`). Non-GET is never intercepted; writes never queue. A server-answered non-2xx is never answered from cache. `AuthContext.logout` posts a purge (`purgeMobileAdminState`) clearing all admin caches + the phone's localStorage. Rollback: a self-unregistering stub recipe is documented at the top of the SW file; the kill path is exercised in the lane's browser pass.
- **Install row**: `client/src/utils/installPrompt.js` captures `beforeinstallprompt` at module import; More renders an explicit "Install app" row when available and not already standalone.
- **Passkey lock (spec section 8)**: `MobileLockScreen` has TWO mounts and they cover different failures. A **gate** inside `ProtectedRoute` (`client/src/App.js`) handles the expired-JWT cold launch: no user, but a phone with an enrolled passkey and a token in storage gets the lock instead of a redirect to `/login`, so the URL survives re-entry. An **overlay** inside `AdminLayout` arms the 30-minute background lock and claims phone 401s. Both render null unless armed, where armed means a passkey enrolled on THIS device AND a phone viewport (`phoneUnlockArmed()`), so desktop admin and the staff portal keep today's behavior untouched. `POST /api/auth/webauthn/assert-verify` (see "WebAuthn phone unlock" in the route table) is the ONLY site that mints the 12h device JWT. Revoking a credential (`DELETE /api/auth/webauthn/credentials/:id`) bumps `users.token_version`, which is a deliberate GLOBAL logout: it is the lost-phone kill switch, not a per-device unenroll. The RP ID is pinned to the admin host (`WEBAUTHN_RP_ID`, defaulting to `admin.drbartender.com` in prod) because one bundle serves four hosts, and a bare-domain RP ID would let an admin passkey assert from the public, hiring, and staff origins. Rows live in `webauthn_credentials` + `webauthn_challenges` (Database Schema above); both are `CREATE TABLE IF NOT EXISTS` in `schema.sql`, which `server/db/index.js` re-executes on every boot, so the DDL is self-applying and there is no separate migration step.
- **Lock-model utils**: `client/src/utils/mobileLock.js` owns the decision model, deliberately kept as pure functions plus a tiny registry so the UI holds no policy: the 30-minute threshold (`LOCK_AFTER_MS`, measuring BACKGROUNDED time, stamped on background and cleared in the foreground, never inactivity), the expired-JWT cold-launch decision (`shouldLock` reading the token's own `exp`), the `adminPasskeyEnrolled` / `adminPasskeyNudgeDismissed` phone-local flags, the one `REVOCATION_CODES` list plus the 401-claim handler registry that `SessionExpiryHandler` consults before logging a phone out, and the key set `purgeMobileAdminState` (`utils/adminSw.js`) wipes on logout. `client/src/utils/webauthnClient.js` is the thin `@simplewebauthn/browser` wrapper: `registerPasskey` / `unlockWithPasskey` / `isPasskeySupported`. Every URL it calls sits under `/auth/`, inside `utils/api.js`'s 401-exclusion prefix, so a failed unlock can never fire the session-expired dispatch; `registerPasskey` treats an already-registered authenticator as success, because logout purges the device flag while the server row and the OS resident passkey both survive.

### WebAuthn (Phone admin passkey unlock)
- **Libraries**: `@simplewebauthn/server` (server-side option generation + attestation/assertion verification) and `@simplewebauthn/browser` (client). There is no third-party service here: the "provider" is the phone's own platform authenticator, so there are no keys to rotate and no vendor outage mode.
- **Config**: `WEBAUTHN_RP_ID` and `WEBAUTHN_ORIGIN`, both optional with prod/dev defaults in `server/routes/webauthn.js`. Both are deliberately host-scoped to the admin origin, never the bare domain (see Admin PWA above).
- **Routes**: `server/routes/webauthn.js`, mounted at `/api/auth/webauthn` (full table under "WebAuthn phone unlock"). Six routes, all on `webauthnLimiter`. Challenges are server-issued, DB-stored, single-use, 5-minute TTL.
- **Storage**: `webauthn_credentials` (one row per enrolled device: credential id, public key, signature counter, label, `last_used_at`) and `webauthn_challenges`. No secret material of ours is stored; the private key never leaves the phone.
- **Failure mode**: a passkey that cannot assert is not a lockout. Password login is untouched and still mints the 7d token, so the escape from a broken or lost authenticator is to log in normally and revoke the credential from Settings > Security.
...
- **Side effects**: the booking-filing path sends NO admin SMS on any booking event, and no admin email either. Cal.com itself owns booking notification (it emails the organizer and syncs the event into the organizer's Google Calendar), so drb-os silently files the booking into the `consults` table for status tracking, suppression queries (drink-plan nudge), and audit. The one exception is the consult call bridge below, which is a FAULT path rather than a booking path: its tail can send a single admin email when the booker's number is undialable, bounded to one per rolling 24 hours. It never sends SMS, because `notifyAdminCategory` texts only when a caller passes `smsBody` and `sendChainEmail` never does.
- **Booker phone capture** (spec 2026-08-25): `extractPhone` reads the booker's typed number from the four places Cal.com puts it (`attendees[0].phoneNumber` / `.phone`, `responses.phone`, `customInputs.phone`, unwrapping the `{ value, label }` form-field shape), `normalizeBooker` trims it and caps it at `MAX_PHONE_LEN` 50, and `BOOKING_CREATED` stores that in `consults.booker_phone` without any format normalization. It is the ONLY number the consult call bridge ever dials, and it is attacker-supplied, because the booking page is public: nothing dials it until `toUsE164` accepts it, at chain open and again at press-1. `BOOKING_RESCHEDULED` writes it `COALESCE($6, booker_phone)`, so a reschedule payload that carries no number keeps what the booker gave originally, and the tail then reads every field off the RETURNING row rather than the payload.
- **Release before the tail**: `handleCreated` holds a pooled client for its transaction and calls `client.release()` immediately after `COMMIT`, before `consultCallTail` runs. The tail uses bare `pool.query`, so holding the client across it would be one request occupying a connection while waiting for a second one, the pool-deadlock pattern that has bitten this codebase twice. A `released` flag keeps the `catch` from issuing a `ROLLBACK` on a client the pool may already have handed to someone else, and the tail runs inside a swallow-everything wrapper so the webhook's 200/503 semantics never change. Only a WON insert (`rowCount === 1`) reaches the tail: losing the `ON CONFLICT` race means the winner's own tail covers that consult.
- **What the tail files**: `consultCallTail` deliberately opens NO chain (a consult booked three weeks out would leave a pending row sitting for three weeks; the sweep opens it five minutes before the slot). It does only the three things that only the webhook knows. When a reschedule named an old booking uid we could not resolve, it marks the booker's OTHER future scheduled consults `skipped_cancelled` / `rescheduled_unresolved`, and that runs BEFORE the phone check (ruling R3) so a moved slot is stopped even when the new number turns out to be undialable. When `toUsE164` refuses the number it files the terminal `skipped_invalid_phone` row (`invalid_phone` vs `no_phone`), emailing at most once per rolling 24h. And on a reschedule it DELETEs any `skipped%` row standing at the consult's CURRENT slot, derived in SQL, so a re-taken slot is not blocked by a stale skip; rows at the old slot carry a different `scheduled_at` and survive as history.
...
- **Two validators, deliberately different widths** (`server/utils/fileValidation.js`):
  - `isValidUpload` — PDF, JPEG, PNG, WebP. The narrow default, used by the W-9 (`payment.js`), blog images, staff-portal uploads, and BOTH headshot fields (a headshot must stay a renderable image).
  - `isValidOnboardingDocument` — additionally accepts HEIC/HEIF and DOC/DOCX. Used ONLY for the resume and alcohol certification on the two onboarding forms. These files are stored in R2 and opened by an admin through a signed URL, never rendered publicly, so an iPhone photo or a Word doc is legitimate; rejecting them stranded a recruit (2026-07-23). Container formats (DOC/DOCX) are accepted only when the magic bytes AND the filename extension agree, so PK magic alone never admits an arbitrary `.zip`.
  - The client mirrors these two sets as `UPLOAD_KINDS.narrow` / `UPLOAD_KINDS.document` in `client/src/utils/uploadLimits.js`, checked at pick time. The client must never accept more than its destination validator, or the user waits through an upload and is rejected at the end.
- **Size limit**: `MAX_FILE_SIZE` (default 10MB) enforced by `express-fileupload` in `server/index.js` with `abortOnLimit: true`, plus a `limitHandler` that reports `upload_limit_exceeded` to Sentry. Installing `limitHandler` means express-fileupload answers through it and its own `closeConnection` early-returns on `res.headersSent`, so the socket is not reset and the JSON 413 arrives readable (the stock config reset the stream and the browser reported a bare "network error"). The remaining body drains rather than being cut off, which matches what `express.json` has always done for an over-limit body and is bounded by Node's 300s `requestTimeout`. The primary defence is still the client-side check, which rejects before a byte is sent; images over ~1.5MB are downscaled in the browser first (`client/src/utils/downscaleImage.js`) and rarely reach the limit at all.
...
- Client-only dependency used by `client/src/pages/staff/tipCard/SignLayout.jsx`, `BizCardLayout.jsx` and `PhoneSignLayout.jsx` to render the bartender's tip-page URL as a QR code on the downloadable bar sign, the hand-out card and the full-screen phone display, and by `TipCardPage.js` for the on-screen preview. These use `QRCodeCanvas`, not `QRCodeSVG`, deliberately: html2canvas is far more reliable capturing a canvas, and the print renders become 300-DPI files a bartender prints. Three settings are load-bearing: `level="Q"` (signage-grade error correction for an object that gets handled and set down in wet places), `marginSize={0}`, and a 2x `size` displayed at 1x via `style` (qrcode.react sizes the canvas backing store to `size * devicePixelRatio`, so a 1x QR on a 1x display is a bitmap html2canvas then upscales, putting 150 DPI of real information inside a 300 DPI file). No server side.
- **The quiet zone is the plate band, not `marginSize`.** qrcode.react defaults `marginSize` to ZERO, so the redesign moved the clear space out of the SVG and into the cream plate that surrounds the code: `SIGN_QR` / `CARD_QR` in `sizes.js` size the `plate`/`pad` pair, and `pad` must never fall below 4 modules. Setting a non-zero `marginSize` on top of that does not add safety, it doubles the quiet zone and shrinks the code inside a fixed plate. The two numbers in each `*_QR` constant are a pair and move together; `sizes.js` carries the arithmetic for the current 37x37-module tip URL.
...
- **Hook**: `.husky/pre-commit` runs five steps in order:
  1. `bash scripts/check-docs-drift.sh`: warns (never blocks) when a structural file was added/removed/renamed (or `schema.sql` changed) without a matching doc update. Its watched paths are `server/routes/`, `server/utils/`, `client/src/components/`, `client/src/pages/`, `client/src/context/`, plus any change to `schema.sql`. New files under `client/src/utils/`, `client/src/hooks/`, `server/middleware/` and `scripts/` are OUTSIDE it and drift silently.
  ...
  3. `node scripts/check-css-palette-scope.js --staged`: the CSS palette-scope checker, a no-op unless `client/src/index.css` is staged. **Warn-only on purpose** (`|| true`, 2026-08-14) while the parser burns in: it is a brand-new 600-line CSS parser and `index.css` is the largest file in the repo, so a false positive here would cost the author a `--no-verify` that ALSO skips the file-size ratchet, lint-staged, and `guard-os-main`. It prints, it does not veto. The same script exits non-zero under `npm run check:css-scope`, so CI can gate on it and the hook can be promoted to blocking later.
  4. `npx lint-staged`: ESLint on staged `server/**/*.js` only (fast, not the full codebase); blocks on errors, allows warnings.
  5. `bash scripts/guard-os-main.sh`: the os-stays-on-main guard; blocks any commit from the primary `os` worktree while it is off `main`, and any `docs/superpowers/specs|plans/` doc committed off `main`.
- **Workflow tooling** (think-on-main / build-in-lanes, not all hook-invoked): `scripts/guard-os-main.sh` (above), `scripts/merge-lane.sh` (flock'd squash-merge wrapper, run only in `os`), `scripts/lane-status.js` (open-lane + stale-lane detection, `npm run lane:status`), `scripts/board-write.sh` (atomic `docs/build-board.md` writer with a PII/Stripe-id denylist; its push is **guarded** and skipped entirely unless the board commit is the only unpushed commit on `main`, because `git push origin main` is a branch push and would otherwise turn a board line into an uncued production deploy of unrelated work — it nearly shipped 49 commits that way on 2026-08-14), `scripts/sensitive-paths.txt` + `scripts/sensitive-match.js` (the one sensitive-path list and its matcher, the single trigger for review-scaling, conflict-escalation, and auto-pull disqualification), and `scripts/check-claudemd-invariants.sh` + `scripts/claudemd-invariants.txt` (paired keyword/regex coverage check over `CLAUDE.md`). Most carry a co-located `*.test.js` run by `npm test` (`node --test`).
...
Twelve custom agents live in `.claude/agents/`, all on `model: opus`. They split into two rosters by WHEN they run.

**Code review (six)**, pointed at a diff and scaled by `scripts/sensitive-paths.txt`:
- `code-review`: code quality and error handling (dead code, duplication, missing error handling, React anti-patterns, API consistency)
- `consistency-check`: cross-file drift between schema, routes, and frontend
- `database-review`: schema, query, and migration review (index coverage, transaction safety, idempotency)
- `performance-review`: rendering, API response times, bundle size, data transfer, public pages first
- `security-review`: OWASP Top 10:2025 pass
- `ui-ux-review`: Playwright-driven UI + accessibility. Explicit-only, needs `npm run dev` running, never auto-runs on push. On a surface with a design artifact, adherence to that artifact is the primary benchmark

**Design stage (six)**, run BEFORE any code exists, explicit-only and report-only:
- `/review-spec` runs `spec-grounding`, `spec-gaps`, `spec-risk` in parallel on a spec doc
- `/review-plan` runs `plan-fidelity`, `plan-decomposition`, `plan-feasibility` in parallel on an implementation plan

See `CLAUDE.md` > Review model (when the fleet runs, the iron rule, chunk-and-retry) and `CLAUDE.md` > Design-stage review fleet.
