...
- Client-only dependency used by `client/src/pages/staff/tipCard/SignLayout.jsx` and `BizCardLayout.jsx` to render the bartender's tip-page URL as a QR code on the downloadable bar sign and hand-out card, and by `TipCardPage.js` for the on-screen preview. The sign and card use `QRCodeCanvas`, not `QRCodeSVG`, deliberately: html2canvas is far more reliable capturing a canvas, and these renders become 300-DPI files a bartender prints. Three settings there are load-bearing: `level="Q"` (signage-grade error correction for an object that gets handled and set down in wet places), `marginSize={4}` (the quiet zone; qrcode.react defaults it to ZERO, so without it the plate padding is the only clear space), and a 2x `size` displayed at 1x via `style` (qrcode.react sizes the canvas backing store to `size * devicePixelRatio`, so a 1x QR on a 1x display is a bitmap html2canvas then upscales, putting 150 DPI of real information inside a 300 DPI file). No server side.
