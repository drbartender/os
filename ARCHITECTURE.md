...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/inbound` | Twilio signature | Client dials the 224 → returns TwiML `<Dial timeout="20" action="…/inbound/missed" callerId="<caller>"><Number>VA_CELL</Number></Dial>` forwarding to Zul's cell (caller = `req.body.From`, xml-escaped). Prod: 403 on bad/missing signature. |
| POST | `/inbound/missed` | Twilio signature, **fail-closed in every env** | The `<Dial>` action callback. On a `DialCallStatus` of `no-answer`/`busy`/`failed`/`canceled` it checks `VM_DAILY_CAP`, claims a `voicemail_delivery` row (the ping dedup), returns the recorded greeting (`<Play>` of the public `GET /greeting.mp3`, or `VM_GREETING_URL`) + `<Record>`, then pings Zul on Telegram AFTER the response. Any other status, or `VOICEMAIL_ENABLED` off, returns a bare `<Hangup/>`. |
| POST | `/inbound/voicemail` | Twilio signature, **fail-closed in every env** | The `<Record>` `recordingStatusCallback` (not `action`: Twilio skips that when the caller hangs up). Claims delivery, then answers 204 and works detached: fetches the mp3 from a URL constructed from the account SID + a `^RE[0-9a-f]{32}$` `RecordingSid` (the body's `RecordingUrl` is never read), uploads to Telegram, and deletes the Twilio recording **only** on an affirmative `ok === true`. |
...
- pending_call / call_audit / telegram_update / voicemail_delivery — Zul VA calling
  ...
  sitting in the console and the row is the only pointer to it. Do not "tidy"
