...
- Excess derivation (2026-07-26): **REVERTED before shipping, never live.** The idea was that the portion of a refund exceeding the contract (`amount_paid - total_price`) would not lower `total_price`. It was dropped in `29316b10` because that difference is not overpayment in this schema: paid `Drink Plan Extras` and manual-label invoices legitimately roll into `amount_paid` and never into `total_price`, so the derivation counted them as excess and then subtracted them twice, swallowing genuine contract refunds. `refundHelpers.js` carries the full rationale and the netting formula a correct version would need. Only `total_scope='overpayment'` spares `total_price` today.
- `total_scope` (TEXT NOT NULL DEFAULT 'contract', CHECK contract|overpayment) — the total_price rule for THIS refund. 'contract' = Approach A (refund lowers the total); 'overpayment' (cancel-line) = the fold already corrected total_price before the refund fired, so reconciliation leaves total_price alone. Durable on the row because webhook/sweeper adoption runs later with no caller memory; `refundExecute` adopts its own row BY ID so a stranded same-amount pending row can never rewrite the rule. Invoice `amount_due` is NO LONGER a separate question: as of 2026-07-28 every refund drops `amount_due` and `amount_paid` together, for every label, locked or not. The old exception spared unlocked Balance / Full Payment invoices (`TOTAL_TRACKING_INVOICE_LABELS`, now deleted) on the premise that `refreshUnlockedInvoices` had already rebuilt their demand from the new total inside the cancel transaction. The derivation rewrite killed that premise: a remainder invoice's demand is now `amount_paid + allocation` FLOORED at `amount_paid`, and in overpayment scope the allocation is always 0, so the refresh writes `amount_due = amount_paid` and cannot pre-absorb anything. Keeping paid-only stranded a client-visible `partially_paid` phantom balance equal to the entire refund on a live pay link. Dropping both always lands on `due == paid`.
...
- **Invoice invariant + monitor**: every open invoice is a derived view of its proposal, never an independent authority for what a client owes. `refreshUnlockedInvoices` (`server/utils/invoiceLifecycle.js`) enforces `Σ(open DERIVABLE invoice remaining) ≤ owed` where `owed = total_price − amount_paid`. The label sets are an **allow-list** (`proposalMoneyShared.DERIVABLE_INVOICE_LABELS`): `PARTIAL_BILL_LABELS` (Deposit alone) is capped and never raised, `REMAINDER_BILL_LABELS` (Balance > Full Payment > Additional Services, in priority order) takes what the partials leave, and `amount_due` floors at `amount_paid` so a settled invoice stays settled. Anything OUTSIDE the allow-list is left strictly alone and is neither derived nor netted out, because its money may be additive rather than inside `total_price` — a syrup-only `Drink Plan Extras` invoice never folds into the contract (`routes/drinkPlans/submit.js`), and `POST /api/invoices/proposal/:id` accepts free-text labels. Deriving those either destroyed the invoice or made it cannibalise contract money. `createAdditionalInvoiceIfNeeded` keys its absorbing check on the same `REMAINDER_BILL_LABELS` constant, or the two double-bill a price increase. `server/utils/balanceInvoiceMonitor.js` watches the same invariant hourly in both directions and is **alert-only**: it reads `proposals`/`invoices`, writes only `proposal_activity_log`, and never creates, edits, or voids an invoice (a bug there would be a wrong number in front of a client; a bug here is a spurious email). Gated by `RUN_BALANCE_INVOICE_MONITOR`.
