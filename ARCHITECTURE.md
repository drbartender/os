...
**`shifts.status = 'cancelled'` means the EVENT was cancelled — closure must never write it.** The value is overloaded: `client/src/components/adminos/shifts.js` `isCancelledEvent` renders a Cancelled chip off it on the admin `GET /shifts` feed, and `server/routes/calendar.js` turns it into `STATUS:CANCELLED` with a bumped `SEQUENCE`, which strikes the event off the owner's subscribed Google Calendar. Its intended producers are the genuine event-cancel paths (P6 cancel, `shiftReap.js`, `cancel-or-unassign` mode='cancel'). There is a **fourth, unintended writer**: `PUT /shifts/:id` (`server/routes/shifts.handlers.js`) destructures `status` off `req.body` with **no allowlist** — unlike `equipment_required` / `supply_run` validated a few lines below — and writes `status = COALESCE($9, status)`, so any admin/manager PUT can stamp an arbitrary status on a delivered event's shift. Recorded here; closing it belongs to a lane that owns that route.
