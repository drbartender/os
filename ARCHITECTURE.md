...
| POST | `/first-reply-failed` | Agent secret | Definitive reply failure (`template_not_found`/`lead_not_found`/`quick_reply_unavailable`/`send_unverified`/`ambiguous_lead`/`already_replied`/`response_cta_not_found`/`ai_draft_clear_failed`): guarded `pending`→`failed`; reason logged + Sentry-tagged (except `already_replied`, a benign manual-reply race), not stored; no email. |
