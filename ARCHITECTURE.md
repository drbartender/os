...
| POST | `/:id/portal-invite` | Admin | Email the client their portal link (plain invite; the portal sits behind the OTP login, so no token rides in the email). Requires a client email; logs `portal_invite_sent`. Rate-limited via `adminWriteLimiter`. |
| PATCH | `/:id/notes` | Admin | Update admin notes |
...
| POST | `/:id/send-group` | Admin/Manager | Send the whole comparison as ONE `proposalOptionsSent` email (one `/compare/:token` link), transitioning every draft option to `sent`. Defers per-option invoicing (to the winner's settle) and suppresses the per-option `proposalSent` email/SMS. Idempotent: a resend with no new draft options sends nothing. `PATCH /:id/status → sent` on a grouped option is blocked (409 `USE_GROUP_SEND`) so the solo path can't double-invoice/email it. |
