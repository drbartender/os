...
- **Checkout gratuity** — gratuity is stored as a per-staff-per-hour RATE (`gratuity_rate`); the dollar line is always computed (`rate × staffCount × hours`, staff = bartenders + additional-bartender addon, NOT barbacks/servers) and layered on top of the forced `"Shared Gratuity"` surcharge. Added on top of `total_price` (never diluted by a discount/override), pooled with the forced surcharge in payroll (both labels via `gratuityLabels.GRATUITY_PAYROLL_LABELS`), and gated on funded before accrual. Applies to all packages via `staff_noun`. Labels come from the one shared constant module (`gratuityLabels.js`, server + client mirror). The no-jar floor (rate ≥ 50) is enforced at the route (`deriveGratuityRate`), in the engine, and by a DB CHECK; an admin mandate (below) REPLACES that floor on both jar answers (the CHECK's third disjunct), never stacks on it. The Stripe webhook records the amount actually charged (additive), never `= total_price`. Aside from an admin mandate, the election persists ONLY at payment: `create-intent` computes it in memory and stamps `tip_jar`/`gratuity_rate` into PaymentIntent metadata; the `payment_intent.succeeded` webhook applies it (before the Balance invoice, `FOR UPDATE` on the proposal row, under the PAYABLE status guard `sent`/`viewed`/`accepted` — deliberately narrower than the credit's lifecycle guard, so a stale intent settling post-conversion or post-refund cannot rewrite `total_price` back up from a stale snapshot). The payment is never hostage to that write: the metadata is validated against the DB CHECK before the row is touched and the UPDATE runs inside a `SAVEPOINT`, so a failed gratuity apply degrades to "not applied, alerted" and never rolls back the payment record. Unpaid proposals never carry a client-elected gratuity; the ONLY pre-payment gratuity is an admin mandate (`gratuity_floor_rate > 0`, spec 2026-08-10), which floors the client's election on BOTH jar answers and rides the same rate machinery, with EVERY persisting snapshot writer carrying `gratuity.floor_rate` (crud, extras fold, webhook spread — a writer that drops it strips the checkout floor client-side while the server still enforces). The admin PATCH accepts exactly one gratuity field, `gratuity_mandate_total` (dollars > 0 via `gratuityMandate.js`; null clears only a real mandate and forces the jar on; absent carries forward and rescales at the canonical rate; rejected once signed or paid), and still never accepts `tip_jar`/`gratuity_total` (admin removal goes through cancel-line-item only, which also clears the mandate column AND the in-memory row before its fold). Payments that carry no metadata — balance, invoice, drink-plan, admin-issued Stripe payment links — never touch the gratuity FIELDS; a mandated gratuity rides inside `total_price` and is collected by any payment. Grep `gratuityLineAmount` / `GRATUITY_LABEL` before touching gratuity.
...
- **The cue IS the confirmation.** An explicit push call ("push", "deploy", "ship it", "send it", "let's get this pushed") is the yes — Claude does NOT ask again. It announces the batch in one line as a STATEMENT, not a question (*"N commits / M files pending — running review + push."*), naming anything the user might not know is riding along (other windows' commits, unpushed quick fixes), then proceeds. If the user says *defer / wait / one more thing / hold on*, Claude stands down silently and re-asks nothing; the next cue restarts it. **Claude NEVER volunteers a "ready to push?" prompt.** After a commit, silence is correct. *Why:* the gate exists so a push is never Claude's initiative and never a surprise batch — an explicit cue plus a visible inventory satisfies both, and asking "are you sure?" on top of a direct order is ceremony that trains everyone to stop reading prompts (same reasoning as the merged-lane `branch -D` pre-approval). Re-confirm ONLY when the inventory contains something the cue plainly did not intend.
- The push-time sweep and the sensitive-path re-review run after that yes. **The cue gates the PUSH, never the review.** The Review model above (per-lane before merge, risk-scaled quick-fix review) runs on Claude's own initiative, without a cue and without asking; review is standing policy, not a permission request. What Claude does not do is run the push-time sweep as prep for a push nobody has called. *Why:* the old wording here ("never pre-run review agents ... the confirmation prompt is the single entry point to the fleet") contradicted the Review model in the same doc, and on 2026-08-25 and again on 2026-08-28 Claude read it as a standing "no agents until told", merged lanes unreviewed, marked review as owed pending the user's ask, and attributed that rule to the user. He never said it: *"Since when do I have to ASK for review."* / *"I haven't said not to use agents."*
...
> The same `.husky/pre-push` also runs the money-path smoke gate (`node scripts/testdb-smoke.js`) when a push changes `server/` — money suites against an isolated Neon `ci-smoke` branch — first (fails faster than the client build). **LIVE and fail-closed since ~2026-08-11**: `NEON_API_KEY` is configured and the gate ran all 42 money suites at both the 2026-08-11 and 2026-08-13 pushes (the old "SKIP banner until the key exists" transitional state is over). See README > Test gate. Same `--no-verify` escape.

> **Run `npm run gate` BEFORE the push (2026-08-14).** Both gates now live in `scripts/push-gate.js`; the hook calls it and skips instantly when a receipt covers the exact bytes on disk. *Why this exists:* `git push` opens the SSH connection and pulls the ref advertisement BEFORE running the hook, so once the money smoke went live (~8 min hook) the connection sat idle until GitHub closed it and the push failed writing the packfile AFTER passing every gate — silently, looking like a network flake. Keepalives do not fix it (the remote closes an idle `git-receive-pack` session). The gate is not weakened, the DUPLICATION is removed: the review pass already runs these checks, so the hook re-running them is what broke the push. The receipt is keyed on a fingerprint of HEAD **plus the sha256 CONTENT of every modified and untracked file**, because both gates test the WORKING TREE and this is a shared checkout: any edit from any window moves the hash and the hook runs the full gate itself. It is also only valid when the sha being pushed IS HEAD — the gates describe your checkout, so they cannot vouch for `git push origin <other-sha>:main`. Receipts expire at 12h. **Known limits, deliberately not overstated:** gitignored files (`.env`, `client/.env*`, `~/.secrets/*`) are outside the fingerprint and can change what the gates do, and the receipt is a plain local file rather than a privilege boundary (anything that can forge it can already run `--no-verify`). A money gate that SKIPPED for want of `NEON_API_KEY` is never recorded as passed. The 2026-08-14 review broke the first draft of this on five paths, so treat changes here as money-path changes: `scripts/push-gate.js` is sensitive-listed.
...
| `RUN_STALE_PROPOSAL_SWEEP_SCHEDULER` | **Opt-in, not opt-out — the only scheduler flag that works this way.** Must be exactly `true`. Runs the hourly stale-proposal sweep (archives past-dated, never-booked proposals as `event_passed`, voids their unpaid invoices, reaps shifts, deletes pending messages). Unset keeps it dark even with `RUN_SCHEDULERS=true`, because its first run voids ~105 invoices and makes ~220 LIVE Stripe calls, and this box talks to live Stripe by design. |
| `STALE_PROPOSAL_SWEEP_DRY_RUN` | Set to `true` to make that sweep log its candidate list and write nothing. Use before the first real run. |
...
| `RUN_SHIFT_CLOSURE_SWEEP_SCHEDULER` | Optional. Set to `false` to disable the hourly shift-closure sweep (`shiftClosureSweep.js`): closes a shift to `completed` once the SHIFT's end instant has passed AND its proposal is `completed`. Never writes `cancelled` (that value is an EVENT-cancelled signal read by the Events dashboard and the iCal feed). Default on. Honored only when `RUN_SCHEDULERS` is not `false`. |
| `RUN_BALANCE_INVOICE_MONITOR` | Optional. Set to `false` to disable the hourly balance-invoice monitor (`balanceInvoiceMonitor.js`): alerts when a client is billed MORE than they owe, or owes a balance with no payable invoice. ALERT ONLY, it never creates/edits/voids an invoice. Default on. Honored only when `RUN_SCHEDULERS` is not `false`. |
...
| `ADMIN_PHONE` | Optional. E.164 admin hotline. Used by the lead-call bridge ring order (`ADMIN_PHONE` → `VA_CELL`), inside-T14 change-request SMS, dispatcher dead-letter alerts, and urgent inbound-SMS alerts. It is also the consult call bridge's first ring: unset there, the chain skips Dallas entirely and opens on Zul's leg with the briefing that never says he missed a call. Unset → those SMS legs are skipped. NOTE it does **not** drive the ≤72h last-minute booking alert, which fans out via `notifyAdminCategory('urgent_booking')` to subscribed admins/managers and never reads this var. |
...
| `WEBAUTHN_RP_ID` | Optional. WebAuthn RP ID for phone-admin passkey unlock (`routes/webauthn.js`). Defaults `admin.drbartender.com` in prod, `localhost` in dev. Pinned to the admin host on purpose: a bare-domain RP ID would let an admin passkey assert from the public/hiring/staff origins. |
| `WEBAUTHN_ORIGIN` | Optional. WebAuthn expected origin. Defaults `https://admin.drbartender.com` in prod; `http://localhost:3000` + `http://admin.localhost:3000` in dev. |
...
| `VOICE_CALLER_ID` | The 0082 US voice line in strict E.164 (`+12242220082`). Caller ID on Zul's outbound calls and her inbound line; since Phase 1a the number ADVERTISED to clients for calls is the 1922 (`COMPANY_PHONE`). |
...
| `RUN_VA_CALLING_SCHEDULER` | Optional. Set to `false` to disable the VA-calling scheduler (hourly prune of `pending_call`/`call_audit`/`telegram_update`/`voicemail_delivery`, the undelivered-voicemail redelivery sweep, the lead-call and **consult-call stale reapers**, and the Telegram webhook heartbeat). Default on. Honored only when `RUN_SCHEDULERS` is not `false`. The consult-call reaper is the ONLY thing that rescues a consult ring chain stranded mid-flight, and it rides `pruneVaCallingRows`, so it is controlled by THIS flag, not by `RUN_CONSULT_CALL_SWEEP_SCHEDULER`. |
...
| `VA_CALL_TIME_LIMIT_SEC` / `PENDING_CALL_TTL_SEC` | Per-call hard `timeLimit` on both call legs (default 1800 = 30 min; both the lead-call and consult-call bridges reuse the same value, on their agent legs and on their press-1 bridges) and confirm-before-dial pending-record TTL (default 120s). |
...
| `LEAD_CALL_DAILY_CAP` | Max lead-call attempt chains opened per rolling 24h (default 25). Toll-fraud backstop for the lead call bridge; normal volume is ~2-3 leads/day. `0` is treated as unset (NaN-guard) and falls back to 25; the only off switch is `LEAD_CALL_ENABLED=false`. |
| `CONSULT_CALL_ENABLED` | Consult call bridge kill switch (spec 2026-08-25): only the literal `false` disables it, and it silences the Cal.com webhook tail, the 60-second sweep, and the callback hops that would dial Zul or text. Default on: set `false` in Render BEFORE the feature's first deploy, and flip it after the launch call test passes. |
| `CONSULT_CALL_DAILY_CAP` | Max consult ring chains opened per rolling 24h (default 10). Toll-fraud backstop; `0` is treated as unset (NaN-guard) and falls back to 10, so the only off switch is `CONSULT_CALL_ENABLED=false`. The Cal.com booking page is PUBLIC, so this value sets TWO ceilings and you cannot raise one without raising the other. (a) Chains opened: the count excludes `status LIKE 'skipped%'` (ruling R15), so a chain ending `skipped_cancelled` frees its slot again -- deliberate, because counting skipped rows would let a burst of junk bookings on the public page hold the whole feature down for 24 hours. That bounds the Manila legs and texts at this value. (b) Rings placed at the 312: `dialCap()` = this value x `MAX_ADMIN_RINGS` (3), so 30 by default, counted over admin_ring on EVERY row in the window, skipped ones INCLUDED, and checked in `advanceChain`. Guard (b) exists because R15 left the book, ring, cancel, rebook cycle unbounded on the admin leg; it is what makes the documented worst case a ceiling rather than a description. Raising this widens a public surface on both, so treat any edit as a security change. |
| `CONSULT_CALLER_ID` | Strict E.164, the 1922: the caller ID the client sees when Dallas pressed 1, so their callback rings the company line. FORMAT-checked, not presence-checked, at boot and again per call, because a typo would make every press-1 `<Dial>` fail at Twilio while the row already reads `connected`, which is terminal and never reaped. Unset or malformed falls back to `VOICE_CALLER_ID` (the 0082, Zul's line) with a boot warning; Zul's own press-1 always shows the 0082. |
| `RUN_CONSULT_CALL_SWEEP_SCHEDULER` | Optional. Set to `false` to disable the 60-second consult call sweep (opens a chain per upcoming Cal.com slot, files the slots that passed unrung, places each due ring). Default on. Honored only when `RUN_SCHEDULERS` is not `false`. |
...
| `VM_GREETING_URL` | Optional. Source of Zul's DAY voicemail greeting. Unset (default) → her 2026-08-19 re-recording, `server/assets/zul-greeting-day.mp3`, served at `/api/voice/audio/zul-greeting-day.mp3`. A full https URL `<Play>`s a different recording without a redeploy; the literal `say` falls back to the `Polly.Joanna-Neural` synthetic voice (known-good kill switch), whose text mirrors the recording word for word and therefore also offers press 1. The older 2026-07-24 mp3 (`voicemail-greeting.mp3`) is still served at `GET /api/voice/greeting.mp3` for anything referencing that URL directly, but is no longer any slot's default. |
| `VM_PRIMARY_DIAL_TARGET` | What the primary line (+12242221922) dials to reach Dallas, strict E.164 (normally the 312 Google Voice number). Unset → the primary line apologizes and hangs up rather than emitting an empty `<Dial>`, and pages Sentry (throttled). |
| `VM_PRIMARY_RING_SEC` | Ring seconds on the primary line before Twilio calls it a miss (default 18, clamped 5..30). Deliberately shorter than a carrier/GV voicemail pickup (~25s) so their voicemail can never intercept ours. |
| `VM_TEXT_DESTINATION` | Where a primary-line voicemail is texted (the 312), strict E.164, sent from `TWILIO_PHONE_NUMBER` with `skipLog` (internal ops alert, never a client-thread entry). The consult call bridge reuses it for its missed-consult and client-did-not-answer texts, falling back to `ADMIN_PHONE`; when neither is strict E.164 the missed chain sends the admin email instead, so the consult is never invisible. Unset/malformed SILENTLY disables primary delivery: rows stay retryable, but only a startup warning + per-call log says why. |
| `VM_LISTEN_LINK_ENABLED` | Listen-link kill switch, **default ON** (only `'false'` disables). TWO-SIDED: off → `GET /api/voice/vm/:token` 404s AND the primary alert SMS omits the link line (one `listenLinkEnabled()` in `utils/voicemail.js` feeds both, so they cannot drift and strand a dead URL in an alert). The token IS the auth, and the link is only ever sent to `VM_TEXT_DESTINATION` (the operator, who can already hear every voicemail in the Twilio console) — that, NOT the proposal/invoice precedent, is what makes a bare UUID acceptable on third-party audio. Revoke one link with `UPDATE voicemail_delivery SET listen_token = gen_random_uuid() WHERE call_sid = $1`. Links expire at 30 days, enforced in the route's own SQL. |
| `VM_GREETING_URL_PRIMARY` | Dallas's DAY greeting, same contract as `VM_GREETING_URL` but for the primary line. Unset → his BUNDLED recording (`primary-greeting-day.mp3`, 2026-08-19); `say` → the synthetic Dallas text; a full https URL → `<Play>` it. Never falls back to Zul's recording. |
| `VM_ESCALATION_ENABLED` | Press-1 escalation master switch, **default OFF** (only `'true'` enables). Off guarantees no claim and no billed dial (`voiceEscalate.js` tests it itself). It does NOT remove the offer from the caller experience: since 2026-08-19 both day greetings say "press one" inside the recording. Off therefore still emits the `<Gather>` in the DAY (fix 2026-08-20 — dropping it meant callers pressed 1 into a document that was not listening); a pressed digit reaches the `escalate_failed` recording. Night emits no `<Gather>` either way. The old "byte-identical to pre-feature production" claim died with the 2026-08-19 re-recording; the golden pins now assert the CURRENT documents. |
| `VM_ESCALATION_DAILY_CAP` | Max escalation legs per rolling 24h (default 25), a HARD bound (cap-check + claim serialize on an advisory lock). A press-1 is a billed outbound leg, international on the primary line. |
| `VM_ESCALATION_QUIET_ZUL` / `VM_ESCALATION_QUIET_PRIMARY` | Quiet hours for the person being escalated TO, local to them, STRICT `HH:MM-HH:MM` (leading zeros required; `24:00`/en-dash are invalid → warned, treated as no window). Inside the window the caller goes straight to voicemail. May wrap midnight. |
| `VM_ESCALATION_TZ_ZUL` / `VM_ESCALATION_TZ_PRIMARY` | Optional IANA zone overrides for the quiet windows (defaults Asia/Manila, America/Chicago). A bad zone name silently disables the window (fail-open on a live call; per-call warning only). |
| `VM_ESCALATION_PROMPT` | Manual override that suppresses the APPENDED press-1 offer everywhere. Since 2026-08-19 the day greetings say the offer themselves. `needsAppendedOffer()` trusts only the KNOWN defaults. Since BOTH lines now default to 2026-08-19 recordings that say the offer themselves, nothing is appended by default and this switch should not be needed. An override URL has UNKNOWN content and DOES get the offer appended, on either line; use this to suppress that when the override already says it. |
| `VM_NIGHT_WINDOW` | Caller-facing night window, strict `HH:MM-HH:MM`, default `21:00-09:00`, may wrap midnight; the literal `none` disables night entirely. Inside it the greeting drops the press-1 offer and promises the morning. NOT `VM_ESCALATION_QUIET_*` (those protect the person being DIALED, in THEIR zone, and suppress only the dial). Unparseable fails OPEN to day. |
| `VM_NIGHT_TZ` | IANA zone for `VM_NIGHT_WINDOW`, default `America/Chicago`. A bad zone name fails open to day. |
| `VM_NIGHT_GREETING_URL_PRIMARY` / `VM_NIGHT_GREETING_URL` | Night greeting per line. Same contract as `VM_GREETING_URL`. Primary defaults to the bundled `primary-greeting-night.mp3`. |
| `VM_ESCALATE_ACK_URL_PRIMARY` / `VM_ESCALATE_ACK_URL` | Played the instant a caller presses 1, before any ringing. Primary defaults to the bundled `primary-escalate-ack.mp3`. |
| `VM_ESCALATE_FAILED_URL_PRIMARY` / `VM_ESCALATE_FAILED_URL` | Played whenever escalation returns the caller to voicemail (seven of the eight branches; the rate-limiter handler carries the same message inline). Primary defaults to the bundled `primary-escalate-failed.mp3`. |
| `VA_INBOUND_PER_MIN_CAP` | Global inbound-call flood cap per minute (default 30 per line) on `POST /api/voice/inbound` AND `POST /api/voice/inbound/primary`, separate buckets per line; on trip it returns busy TwiML and never dials. Runs BEHIND the signature gate so unsigned floods cannot consume it. |
...
| `FIRST_REPLY_CALL_DELAY_SECONDS` | Breathing room between the confirmed/failed first reply and the promised lead call (default 60; 0 = immediate; clamped 0-600). In-process timer; sweep Arm A is the restart backstop, defers flipped rows by delay+60s (never preempts the room), and the `lead_id`-UNIQUE attempt open absorbs double-fires. A delay that would cross the 23:00 clamp fires immediately instead. |
| `FIRST_REPLY_NIGHT_JITTER_START_HOUR` / `FIRST_REPLY_NIGHT_JITTER_END_HOUR` | Chicago dead-hours window `[start, end)` in which night-template leads get the 2-14 min reply jitter (defaults 2 and 8; may wrap midnight, e.g. 23/6; start == end disables). Outside it night replies offer immediately — "night leads are fast until 2am" (2026-08-03). |
