...
| `VM_ESCALATION_PROMPT` | Manual override that suppresses the APPENDED press-1 offer everywhere. Since 2026-08-19 the day greetings say the offer themselves. `needsAppendedOffer()` trusts only the KNOWN defaults, so it appends for Zul's pre-escalation mp3 and not for Dallas's recording; an override URL has UNKNOWN content and therefore gets the offer appended on EITHER line. Use this to suppress that after wiring a recording that already says it. |
