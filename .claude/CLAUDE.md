...
| `FIRST_REPLY_CALL_DELAY_SECONDS` | Breathing room between the confirmed/failed first reply and the promised lead call (default 60; 0 = immediate). In-process timer; the 60s sweep's Arm A is the restart backstop and the `lead_id`-UNIQUE attempt open absorbs double-fires. |
