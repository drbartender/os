...
| `VM_GREETING_URL_PRIMARY` | Dallas's greeting, same contract as `VM_GREETING_URL` but for the primary line. Unset → the synthetic Dallas text; a full https URL → `<Play>` it. Never falls back to Zul's recording. |
...
| `VM_ESCALATION_PROMPT` | Set to `none` once a greeting recording itself announces the press-1 option; default speaks a short synthetic prompt after the greeting. |
