...
| `LABRAT_SEED_ENABLED` | Optional. Prod kill-switch for the Lab Rat seed endpoint (`POST /api/qa/seed`, which mints staff test users + returns plaintext creds). Default CLOSED in production — the endpoint 404s unless this is `true`. Set `true` during a tester campaign. Ignored outside production (NODE_ENV gate, same philosophy as `RUN_SCHEDULERS`/`SEND_NOTIFICATIONS`). |
