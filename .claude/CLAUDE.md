...
| `VM_TEXT_DESTINATION` | Where a primary-line voicemail is texted (the 312), strict E.164, sent from `TWILIO_PHONE_NUMBER` with `skipLog` (internal ops alert, never a client-thread entry). Unset/malformed SILENTLY disables primary delivery: rows stay retryable, but only a startup warning + per-call log says why. |
