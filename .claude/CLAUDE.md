...
| `CONSULT_CALL_DAILY_CAP` | Max consult ring chains opened per rolling 24h (default 10). Toll-fraud backstop; `0` is treated as unset (NaN-guard) and falls back to 10, so the only off switch is `CONSULT_CALL_ENABLED=false`. The Cal.com booking page is PUBLIC, and this bounds ONLY the legs whose chains reach a counted status: 10 Manila legs and 10 texts per rolling 24h. It does NOT bound the admin rings to the 312. The cap counts `status NOT LIKE 'skipped%'` (ruling R15), so a chain ending `skipped_cancelled` frees its slot again and a book, ring, cancel, rebook cycle is bounded by the supply of bookable Cal.com slots plus `MAX_ADMIN_RINGS` (3), never by this value. That is deliberate: counting skipped rows would let a burst of junk bookings on the public page hold the whole feature down for 24 hours. Never tune this believing it holds the ring count down; raising it widens a public surface on the counted legs. |
