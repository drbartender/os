...
| `LABRAT_SCREENSHOT_ALLOWED_HOSTS` | Optional. Comma-separated hostname allowlist for tester-submitted `screenshotUrl` on `/api/test-feedback`. Unset → any http(s) host passes; set → only listed hosts pass (open-redirect defense for the admin viewer). |
