...
| `VM_GREETING_URL` | Optional. Source of the voicemail greeting the caller hears. Unset (default) → Zul's recorded mp3, served by the public `GET /api/voice/greeting.mp3` (bundled at `server/assets/voicemail-greeting.mp3`). A full https URL `<Play>`s a different recording without a redeploy; the literal `say` falls back to the `Polly.Joanna-Neural` synthetic voice (known-good kill switch). |
...
| `VM_ESCALATION_ENABLED` | Press-1 escalation master switch, **default OFF** (only `'true'` enables). Off → no `<Gather>` on either line; the zul document is byte-identical to pre-feature production (golden-pinned by test). |
