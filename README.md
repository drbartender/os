...
| `CONSULT_CALL_DAILY_CAP` | No | Max consult ring chains opened per rolling 24h (default 10; toll-fraud backstop). `0` is treated as unset and falls back to 10, so the only off switch is `CONSULT_CALL_ENABLED=false`. The Cal.com booking page is public, and this bounds ONLY the legs whose chains reach a counted status: 10 Manila legs and 10 texts per rolling 24h. It does NOT bound the admin rings to the 312, because the cap counts `status NOT LIKE 'skipped%'`, so a chain ending `skipped_cancelled` frees its slot again; what bounds the rings is the supply of bookable Cal.com slots plus `MAX_ADMIN_RINGS` (3), never this value. Deliberate per spec section 6, so never tune this believing it holds the ring count down. |
