...
| `RUN_STALE_PROPOSAL_SWEEP_SCHEDULER` | No | **Opt-in, not opt-out.** Must be exactly `true` to run the hourly stale-proposal sweep, which archives past-dated proposals that were never booked (`archive_reason = 'event_passed'`), voids their unpaid invoices, reaps shifts, and deletes their pending scheduled messages. Unset keeps it dark even when `RUN_SCHEDULERS=true`, because its first run voids ~105 invoices and makes ~220 live Stripe calls. |
| `STALE_PROPOSAL_SWEEP_DRY_RUN` | No | Set to `true` to make that sweep log its candidate list and the past-dated `accepted` proposals it would leave for an admin, while writing nothing, sending nothing, and calling no Stripe. Use before the first real run. |
...
| `STRIPE_TEST_MODE_UNTIL` | Optional | ISO 8601 cutoff date. While set and in the future, every Stripe call uses the `*_TEST` credentials; after the cutoff, the next request automatically reverts to the live credentials with no redeploy. **The date must be in the FUTURE.** A cutoff already past — or malformed, or empty — silently means LIVE (real cards), because `isTestMode()` tests `Date.now() < cutoff` and fails open. A stale sample date from this table was pasted into a real `.env` on 2026-08-19 and read as "configured for test" while charging live. Use e.g. `2099-12-31T23:59:59Z` and verify `isTestMode()` rather than trusting the file. Note the dev box runs against LIVE Stripe deliberately; leave this unset there. |
...
| `RUN_VA_CALLING_SCHEDULER` | No | `false` disables the VA-calling prune, the undelivered-voicemail redelivery sweep, the lead-call and **consult-call stale reapers**, and the Telegram webhook-heartbeat scheduler. Default on. Honored only when `RUN_SCHEDULERS` is not `false`. Note the sweep is the ONLY retry for a voicemail stranded between claim and upload, and the consult-call reaper is the ONLY thing that rescues a consult ring chain stranded mid-flight: it rides `pruneVaCallingRows`, so this flag controls it, NOT `RUN_CONSULT_CALL_SWEEP_SCHEDULER`. |
...
| `VA_CALL_TIME_LIMIT_SEC` | No | Per-call hard `timeLimit` on both legs (default 1800 = 30 min). Both the lead-call and consult-call bridges reuse the same value, on their agent legs and on their press-1 bridges. |
...
| `CONSULT_CALL_ENABLED` | No | Consult call bridge kill switch: only the literal `false` disables it, silencing the Cal.com webhook tail, the 60-second sweep, and the callback hops that would dial Zul or text. Default on. Set `false` in Render before the first deploy, flip after the launch test. |
| `CONSULT_CALL_DAILY_CAP` | No | Max consult ring chains opened per rolling 24h (default 10; toll-fraud backstop). `0` is treated as unset and falls back to 10, so the only off switch is `CONSULT_CALL_ENABLED=false`. The Cal.com booking page is public, and this value sets TWO ceilings at once: the chains that may open (counting only `status NOT LIKE 'skipped%'`, so a `skipped_cancelled` chain frees its slot again -- ruling R15), and, multiplied by `MAX_ADMIN_RINGS` (3), the rings that may be placed at the 312 in the same window (`dialCap()`, counted over every row including skipped ones). The second ceiling exists because the first one alone left cancel-and-rebook unbounded on the admin leg. Raising this widens a public surface on both. |
| `CONSULT_CALLER_ID` | No | Strict E.164 caller ID the client sees when Dallas pressed 1 (the 1922, the company line printed on proposals). Unset or malformed falls back to `VOICE_CALLER_ID` (the 0082, Zul's line) and warns at boot. Zul's own press-1 always shows the 0082. |
| `RUN_CONSULT_CALL_SWEEP_SCHEDULER` | No | `false` disables the 60-second consult call sweep (opens chains, files missed windows, places the due ring). Default on. Honored only when `RUN_SCHEDULERS` is not `false`. |
...
| `VM_TEXT_DESTINATION` | Yes* | Where a primary-line voicemail is texted (the 312), strict E.164. Unset/malformed silently disables primary delivery (rows stay retryable; startup warning + per-call log only). Also the consult call bridge's text destination, falling back to `ADMIN_PHONE`; with neither valid, a missed consult sends the admin email instead. |
...
| `VM_GREETING_URL_PRIMARY` | No | Dallas's DAY greeting. Unset → his bundled recording; `say` → synthetic text; a URL → `<Play>`. Never falls back to Zul's recording. |
...
| `VM_ESCALATE_ACK_URL_PRIMARY` / `VM_ESCALATE_ACK_URL` | No | Press-1 acknowledgment per line; primary defaults to a bundled recording. |
| `VM_ESCALATE_FAILED_URL_PRIMARY` / `VM_ESCALATE_FAILED_URL` | No | Escalation-failed message per line; primary defaults to a bundled recording. |
...
│   ├── assets/                 # Static bundled audio served to Twilio <Play>: voicemail-greeting.mp3 (Zul, 2026-07-24) + primary-greeting-day/-night, primary-escalate-ack/-failed (Dallas, 2026-08-19; 8kHz mono, the rate a phone line carries)
│   ├── data/
...
│   │   ├── voiceAssets.js      # Static voicemail audio for Twilio <Play> (GET /api/voice/greeting.mp3 + /api/voice/audio/:name). Split from voice.js at its 700-line cap. PUBLIC by necessity (Twilio sends no credentials); :name is a KEY into a frozen allowlist, never a path, so it cannot traverse
│   │   ├── voiceConsultCall.js # Consult call bridge Twilio webhooks (/api/voice/consult): /answer (Gather-wrapped spoken briefing), /digit (press-1 → Dial the booker from the 1922, press-9 replay, max 3 plays), /dialend (Dial action: latches client_no_answer_at once and texts Dallas the number), /status (terminal-status allowlist, then onLegTerminal). Signature FAIL-CLOSED in every env
...
│   │   ├── consultCallBriefing.js # Spoken copy for the consult call legs: buildConsultBriefing (booker name, slot time, ring wording, proposal event date + guest count), spokenDateOnly / clockTimeWithMinutes, formatUsPhoneForText. Pure, no DB, Chicago-pinned
│   │   ├── consultCallChain.js # Consult call bridge chain driver. OPEN side: consultCallTail (Cal.com post-commit tail, never throws, bare pool.query), openChain (atomic cap-plus-open), fileUndialable / fileMissedWindow (terminal skip rows), sendChainEmail. FIRE side: advanceChain (claim-then-call, ring ladder ADMIN_PHONE → VA_CELL), onLegTerminal (terminal-status allowlist + admin_ring guard, owner of every non-initial transition), placeLeg, sendMissedText. scheduled_at is derived in SQL and never round-tripped through JS
│   │   ├── consultCallSweep.js # The 60-second clock (RUN_CONSULT_CALL_SWEEP_SCHEDULER): opens chains for slots inside the ahead/behind window, files missed windows for slots that passed unrung, hands due rings to advanceChain. Per-row and per-step catches so one bad consult cannot stop the tick; windowConstantsFault stops a NULL-bound query from writing green heartbeats over a dead bridge
...
│   │   ├── staleProposalSweep.js # Hourly OPT-IN sweep archiving past-dated proposals that were never booked -> archived + archive_reason 'event_passed', reusing the manual Archive button's helpers so the two doors cannot drift. 'accepted' is exempt and emailed to an admin instead: the demote ladder parks refunded-to-zero bookings there. Dry-run mode, a runaway bound, a re-entrancy guard, and a failure-marker-keyed Stripe heal (RUN_STALE_PROPOSAL_SWEEP_SCHEDULER)
...
│   │   ├── voicemailLine.js    # Per-line routing policy (pure): resolveLine enum + E164_RE, escalationTargetFor (env-only), quiet windows (strict HH:MM-HH:MM, IANA tz), primaryRingSec clamp, interception canary, isNight (the caller-facing 21:00-09:00 night window; shares parseWindow/inWindow with the quiet hours)
│   │   ├── voicemailTwiml.js   # Shared TwiML fragments (one owner for voice.js + voiceEscalate.js): the four message slots (greeting_day/greeting_night/escalate_ack/escalate_failed) resolved per line by messageVerb, needsAppendedOffer, <Record> verb, vmMaxLengthSec clamp, VM_ESCALATION_ENABLED switch
...
│   │   │                       # PendingPaymentCard ("Your bank payment is processing.", shared by the invoice page, the proposal page and, through plan/components/PaymentReturnNotice, the drink-plan celebrations, spec 2026-09-14),
...
│   │   │   │                   # StatusChip, StaffingCell (events-list staffing column: confirmed/needed with a red shortfall, plus a requests-vs-waitlist chip), StaffHoverCard (hover cards under that cell: the ratio lists who is confirmed, the requests chip lists who applied and for what; portaled and inert), RainbowDefs, Toolbar, Icon, KebabMenu, SortableTh (clickable sort headers), AddressLink,
...
│   │   │   ├── admin/          # Dashboard sub-pages (PotionsPage bar-program home at /potions with potions/ RecipesTab + PantryParsTab + PackagesTab/PackageDetail/MakeabilityPanel (planner-v2 package model: contents, slots, live margin, makeability) + PlansDrawer siblings, proposals (ProposalCreate cockpit split into proposalCreate/ with ClientSection + EventSection + PackageSection + AddonSection + shared helpers.js Lbl), clients, events, EventDetailPage, proposalEditor/ (shared proposal/event editor mounted by Proposal Detail AND Event Detail: ProposalEditorForm + PackageSection + formState seed/recovery helpers + single patchBody payload builder + repriceSummary/RepriceConfirmModal booked-event reprice confirmation; replaced ProposalDetailEditForm + EventEditForm), shifts, staff, menus, hiring, blog, email marketing, Messages admin SMS conversation/thread page, userDetail/tabs/TipPageTab admin tip-page controls + userDetail/tabs/FeedbackCard.js, the admin-only guest-feedback card at the bottom of that tab (rating 1-3, comment, submitter email, Mark reviewed; the only client reader of GET /admin/tip-feedback since the standalone /tips queue was retired), applicationDetail/, NotificationSettings per-user notification-subscription toggles, SecuritySettings passkey list + revoke on the /settings Security tab (revoke = global logout by design), PendingPaymentsList Processing lines above the invoice list in the Proposal Detail payment panel (one per bank debit still settling, spec 2026-09-14), ProposalChangeRequestCard client-portal change-request review card on Proposal Detail (diff, preview, apply-in-editor, decline), AlternativesPanel option-group manager on Proposal Detail (add/remove alternatives, Send options, copy compare link), ChangeRequestsDashboard admin pending-requests queue at /change-requests, eventDetail/MessageLogCard newest-first client message log (email + SMS, sent/failed) on EventDetailPage, payroll/PayRunView open-period pay-run queue (stat strip + period cards with process/reopen) + payroll/PayPanel per-payout generate-gated method-aware pay panel (replaced PayQRModal + MarkPaidAction + PayrollHeader, all three deleted), payroll/DeferredTipsPanel admin list + Retry button for tips/clawbacks that deferred while the open pay period was frozen, StripePayoutsTab Stripe payout reconciliation tab on the Overview Payouts tab, payroll/TaxTotalsTab 1099 calendar-year totals tab with per-person include/exclude toggle + CSV export, payroll/CurrentWeekCard.js the date-derived current-week pay-run card (no pay_periods row behind it, so nothing to process or expand) + payroll/TipsLedger.js the Payroll Tips tab ledger (From/To filter, cursor pagination, net of refunds with the gross struck beside it) + payroll/tipStatus.js its row-derived Status column, userDetail/tabs/PayoutsTab imported payment-history section + blended all-time total, CancelEventDialog 3-step cancel-booked-event modal on Proposal Detail + Event Detail: mode radio → server-computed preview → typed-last-name arm + suppress toggles, then a distinct Issue-refund action, CancelLineDialog per-line-item removal + refund modal on Proposal Detail + Event Detail: server targets enumeration → rolled-back-preview → two-step confirm with notify-client chaining; package row hands off to CancelEventDialog, payroll/DutyLineRow + payroll/AttributionModal duty-pay editing + run-payroll attribution on PayRunView + payroll/dutyKinds.js manual-add duty-kind catalog (labels + per-kind default cents; EVENT kinds minus out_of_area, review kinds never manual), OutOfAreaKnob Out-of-Area Bonus control on the Event Detail staffing card (dollars in / cents out, server-supplied suggestion + venue distance, locked badge, unlocked-money warning; the $250 cap and the bands stay server-side), RemoteStaffingFeePrompt pre-send Remote Staffing Fee popup shared by ProposalCreate's first send and ProposalDetail's send + resend: self-checks and auto-proceeds when there is nothing to ask, otherwise offers add-suggested / custom / send-without and writes the surcharge through the existing editor PATCH builder)
...
│   │   │   │   └── staffHub/    # The Staff hub at /staffing (spec 2026-08-19-admin-staff-hub-design.md). StaffHubLayout.js: the hub route shell, owns ONE summary fetch (GET /admin/staff-hub/summary) and shares { summary, refresh, setActions } with its four children through Outlet context, drawing the header-fused .hub-tabs strip (Roster lands at the index; Hiring, Payroll and Reviews are adminStrict children, so a manager bounces at the route instead of landing on a page whose every fetch 403s). reviews/ holds the Reviews child that replaced the retired standalone /reviews page: ReviewsPage.js (the child shell), PendingReviewCard.js (one pending review as a workbench card, with the third state a confirm enters when no pay period is open), LogReviewForm.js, ResolvedTable.js (confirmed and dismissed rows, Dismiss still reachable on a confirmed row), ContestRail.js (the quarterly contest beside the workbench, quarter selector included), AwardDialog.js and suggestNames.js (excerpt-derived, removable name chips that only become credits on confirm)
│   │   │   │                    # hubSubtitle.js: the pure live-subtitle builder (active count, the date-derived pay-run window and payday, reviews waiting to confirm), manager-safe because the admin-only summary fields come back null
...
│   │   │   ├── plan/               # PlannerRouter (planner_version switch) -> legacy PotionPlanningLab (v1 wizard, steps/ components/ data/ intact for in-flight drafts; components/PaymentReturnNotice.js is the post-Stripe return box both v1 and v2 celebrations share: received on a card, processing on a bank debit, spec 2026-09-14) OR v2/ (planner v2: PlannerV2 orchestrator, queue.js, steps/ x11 — no payment UI, hosted three-shape picker, crowd questions); lab/ (Enhancement Lab page at /plan/:token/lab: EnhancementLab orchestrator + LabShelves + LabLedger, invoice-only additions)
│   │   │   ├── invoice/        # InvoicePage — public token-gated invoice view + payment
│   │   │   ├── proposal/       # ProposalView (public client-facing) — split into proposalView/ folder (parent + ProposalHeader + ProposalPricingBreakdown + SignAndPaySection + PaymentForm + SwitchBanner post-switch banner with undo + AgreementText markdown-lite renderer + helpers + styles + paidState.js (row truth for the paid card and terms box; readRedirect) + settlePoll.js (bounded post-redirect poll) + useSettle.js (ref-latched settle phase) + checkoutVisibility.js (pure checkout gating: settling / pending / pay controls) + PaidCard.js + PaymentTermsBox.js + intentQuote.js (merges a create-intent quote into the SNAPSHOT only; the projection never touches proposal.total_price) + gratuityFloor.js) + compare/ (ProposalCompare thin wrapper for the option-group page at /compare/:token + PackageMatrix aligned live-priced compare grid; its ExplorePackagesSection export is superseded by otherOptions/ and no longer referenced) + otherOptions/ (the options LADDER drawer: OtherOptionsPanel shell + Rung presentation + ladder.js assembly + ExtrasPanel anchor-scoped strip + switchApi.js the one write + popularExtras.js curated chips; pin-to-compare and CompareTable are retired) + catalogBuckets.js (shared package-contents bucketing)
...
| `npm run palette:contrast` | Dev-only two-skin WCAG contrast sweep of 13 admin surfaces in BOTH skins (`scripts/palette-contrast.js`). Reads `getComputedStyle` off real painted text, resolves the effective background by walking ancestors (House Lights sets surfaces transparent, so an element's own background is usually meaningless), composites text alpha, and applies the real large-text rule. Writes `palette-contrast-report.json`. `--only <surfaces>` / `--skin light\|dark` to narrow. Same dev-branch gate as `mobile:check`. This is the automated half of the admin palette sweep: every contrast figure in that work was token arithmetic and no browser had ever been run |
...
- After tipping, guests land on a thank-you page that nudges a Google review and offers an optional bartender feedback form (`tip_page_feedback` table). Admins read and clear that queue on the staffer profile's Tip Page tab, in the `FeedbackCard` that the 1-3 star alert email links straight to
...
- Admins audit tip pages and regenerate Stripe links from the `TipPageTab` on the user detail page, whose `FeedbackCard` also carries that bartender's guest feedback; cross-staff tip activity lives in the Payroll child's Tips ledger at `/staffing/payroll?tab=tips`
