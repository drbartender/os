...
**Setup time is back-of-house only.** `proposals.setup_minutes_before` (admin override; NULL ⇒ derive 90 hosted / 60 else), the synced `shifts.setup_minutes_before`, and the derived `setup_time_display` are crew arrival/prep timing — surfaced ONLY on admin event/proposal pages, the staff portal (Shifts tab / ShiftDetail), and the staff hire-confirmation email (`shiftRequestApproved`). The public token route (`server/routes/proposals/publicToken.js`) uses an explicit column allowlist that **deliberately omits** `setup_minutes_before` and adds no derived setup key to its `res.json` — clients/leads never see it.

**No client-facing surface renders a derived arrival time, on ANY channel — email, SMS, token-gated page, or marketing copy.** Clients get the published range only: **"30 to 90 minutes"** (prose) / **"30–90 minutes"** (web). The channels that state arrival are the event-eve SMS (`smsTemplates.eventEveSms`), the signed-and-paid orientation email timeline (`stripePaymentNotifications.js`), the FAQ (`FaqPage.js`), and the Method page (`MethodPage.js`); all four are hardcoded to the range and must stay that way. Staff surfaces are the opposite and stay exact: the crew needs a real clock time.

*Why:* until 2026-07-19 the event-eve SMS and the orientation email both rendered `effectiveSetupMinutes()` verbatim ("arrives 60 minutes before"), which read as a promise and made a normal 30-minute arrival look late to the client. The earlier version of this rule only enumerated email and token-gated templates, so SMS walked straight through the gap. It is now a channel-agnostic rule. Regression guard: `smsTemplates.test.js` > *"never renders a derived setup-minutes value"*.
