...
- **Dashboard / Financials redesign (Money Board)** — BUILD UNDERWAY 2026-07-09: spec + plan committed (`docs/superpowers/specs/2026-07-09-money-board-design.md`, `docs/superpowers/plans/2026-07-09-money-board.md`). Lanes a (list filters) + b1 (Overview shell at `/dashboard`, `/financials` redirect) + b2 (analysis band: MoneyTiles / FunnelCard / LeadSpendCard / RangeTables, History-control restyle, old `Dashboard.js` + `FinancialsDashboard.js` deleted) merged/merging. Remaining serial lanes: b3 (rainbow revenue chart) → c (payroll card) → d (payouts focus) → e (prep queue). Prep-queue Potions gate is CLEARED (Potions shipped live 2026-07-09).
