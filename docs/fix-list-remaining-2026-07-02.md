...
- **The par baseline is not bar-type aware, so a mocktail bar stocks tonic and bitters** (filed 2026-08-20, Dallas: "nobody cares... its an NA package", i.e. do NOT advertise the absence, just stop stocking them). `PARS_100` is built as "the `in_full_bar` rows" in `potionCatalog.js`, and `par_items` has no package or `bar_type` scoping, so `angostura-bitters` (`in_full_bar = true`, `paired_spirits = {bourbon,whiskey}`) and `Tonic Water` land on the list for The Clear Reaction, whose whole premise is a zero-proof bar. Tonic is merely pointless there (it is a gin mixer with no gin); bitters is ~44% ABV on a package sold to religious, cultural, and sober-curious crowds. Dallas also wants commercial ginger beer stocked for that bar; house-made stays a paid upgrade, so this is a par change ONLY and `covered_addon_slugs` must stay `{}` for `the-clear-reaction` or the $2.50/guest add-on stops being sellable. Fix is either a bar-type exclusion on those par rows or teaching `PARS_100` the package's `bar_type`. Internal prep-list correctness: no client-facing surface until someone reads a par sheet. Deliberately NOT bundled into the options-ladder lanes (outside their footprint).
