...
- **Later phases (order Dallas's call):** master CC report — load payments/payouts/event summaries into the existing empty `legacy_cc_*` sidecar tables (schema already fits the exports) and blend CC-era numbers into financials/dashboard (invisible blending, era labels only where a number would otherwise look wrong, NEVER vendor banners; CC payouts stay out of payroll math, 1099 reference line only). Then v1 importer/UI code deletion as housekeeping.
