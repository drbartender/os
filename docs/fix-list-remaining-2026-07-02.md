...
- **Phase 3 MERGED to main 2026-07-07 (477344b, not yet pushed): the event transfer.** CC dies 7/21 (Dallas's call). scripts/cc-transfer-events.js moves 13 future events (Luva 7/18 stays in CC per Dallas) to native born-confirmed proposals: total_price_override = CC contract, external_paid folded into amount_paid with ZERO payment rows, drink plans born nudge_suppressed (durable flag, cleared by the reenroll button), balance/pre-event reminders enrolled with past rungs purged, ledger rows deleted + loader skip registry. Fleet caught and fixed: invoice-refresh re-billing external_paid (Balance/Full Payment netting + DB test), the nudge-resurrection-on-PATCH blocker (durable suppression), ProposalView's hardcoded paid-card balance. Dev integration 13/13. PROD RUNBOOK (after push): dispatcher env OFF for the apply window -> dry-run -> Dallas go -> --apply -> checks -> dispatcher on -> Dallas turns off CC client notifications, sends intro notes, re-enrolls nudges per client, sends balance invoices (paid-card has no pay affordance for confirmed; invoice links are the pay path). Sid (due 7/9) / Cody (7/17) may pay via CC first: bump via documented UPDATE (external_paid + amount_paid), never payment rows.
- Contracts bulk-download before cancellation still owed; 2026 YTD P&L export recommended (2024+2025 archived).
