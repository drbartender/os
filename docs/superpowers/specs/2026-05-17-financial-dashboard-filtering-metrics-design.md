...
**State lives in the URL** (`?from=&to=&basis=`, plus a `range` UI sentinel for the two states explicit dates can't represent) — refresh-safe, shareable, symmetric with the backend params. React Router 6 is already in place. Encoding: a **named preset** writes explicit `from`+`to` (no `range`); **All time** writes `?range=all` (no from/to → backend omits the date predicate); **Custom** writes `?range=custom` + the chosen `from`/`to`. No params at all = the default (Last 12 months), kept deliberately **DISTINCT** from All time (the earlier "All time = params omitted" idea collided with the default and was dropped). The hook owns all encoding/decoding; the bar is purely presentational. Only `from`/`to`/`basis` reach the API — `range` is UI state, never sent.
