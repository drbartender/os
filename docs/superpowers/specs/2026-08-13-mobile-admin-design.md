...
- **Reads:** network-first with cache fallback, under these rules:
  ...
  - **Allowlist:** the SW caches only the phone surfaces' GET paths (events/shifts feed, event detail, proposals list/detail, badge-counts, search, and the active-staff feed the assignment sheet reads), plus `/auth/me` as the one identity path (amended 2026-08-14, ma-d plan fleet: offline cold launch hydrates the user from the cached copy and lands on the restored route instead of Login; a server-answered 401 still evicts it, and the cached identity payload sits under the same remote-revoke honest limit as every other cached read). Desktop-view traffic through the escape hatch (payroll, users, paystubs) is never cached.
  ...
  - **Cache keying and purge:** the API cache name embeds the authenticated user id; on boot with a different user, old namespaces are purged (managers share these routes; one device must never show another user's cached data). Full purge on logout. Honest limit, stated as accepted risk: a *remote* credential revoke cannot reach a phone's CacheStorage; revoke kills access to fresh data and live sessions (§8), while the cached snapshot persists on-device behind the OS device lock. No remote-wipe theater.
  - **The offline session is bounded by its own token (added 2026-08-14, cross-window push review).** Because `/auth/me` is cached, an offline launch reconstructs the *identity* from a snapshot, not just the data, and offline the server is never consulted. So a cache-served `/auth/me` (the response carries `staleAt`) is honored only while the stored token is still unexpired by its own `exp`; once it has died, the client refuses to hydrate and purges the device's caches and token on that open. Without this bound a phone that never enrolled a passkey arms no lock at all (`shouldLock` returns false when unarmed), so it would render a full admin session, and every allowlisted read would serve cached client PII, indefinitely. This is a client-side bound on a client-side snapshot, not a cryptographic check: it cannot be, offline. What it buys is that the exposure window is the token's lifetime (12h for a phone session) rather than forever, and that the device self-clears on the first open after expiry.
...
- **Lock behavior:** app backgrounded > 30 minutes, or JWT expired → full-screen lock that fully occludes content (no cached data visible behind it). One biometric tap asserts and re-enters. The lock arms only once a passkey is enrolled on this device (amended 2026-08-14, ma-d plan fleet: without a credential there is nothing a biometric tap can assert); an un-enrolled phone keeps today's behavior, where a dead session logs out to password login. Enrollment is offered right after password login (dismissible) and stays one tap away under More > Security. **In-scope client changes the fleet caught:** `SessionExpiryHandler` currently force-logs-out and navigates to `/login` on the first 401 and its once-only guard never resets in a long-lived PWA document; on the phone surface a 401 routes to the lock screen instead (desktop behavior unchanged), and the guard resets on re-auth. `AuthContext` currently clears the stored token on ANY `/auth/me` rejection; the corrected law is four-way (amended 2026-08-14, ma-d plan fleet): a revocation-class 401 (`TOKEN_VERSION_MISMATCH`, `USER_NOT_FOUND`) purges caches and clears the token everywhere; any other real 401 purges and clears only when phone unlock is NOT armed (no enrolled passkey, or not a phone viewport); with unlock armed the token artifact and caches stay, occluded behind the lock, so the expired-JWT cold launch lands on the lock and one tap re-enters (the only reading under which the lock sentence above and the §9 behind-the-lock cold launch both hold); a transport failure never clears anything, otherwise an offline cold launch permanently signs the phone out, which the §7/§9 offline promises cannot survive.
