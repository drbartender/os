...
- **Reads:** network-first with cache fallback, under these rules:
  ...
  - **Allowlist:** the SW caches only the phone surfaces' GET paths (events/shifts feed, event detail, proposals list/detail, badge-counts, search, and the active-staff feed the assignment sheet reads). Desktop-view traffic through the escape hatch (payroll, users, paystubs) is never cached.
