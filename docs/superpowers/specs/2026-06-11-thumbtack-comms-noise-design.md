...
- Add `client_phone_prefill` to the response: the client's current phone, or `''` when the phone is a Thumbtack proxy or absent. Proxy check: run `findThumbtackProxyLead(client.phone)` (Component 1 helper) **only when the client's `source` is `'thumbtack'`** (the guard keeps the extra query off the common public-page path). The guard is safe, verified against `clientDedup.js`: Thumbtack lead payloads carry no email, and a post-rollout proxy is a fresh number that can never phone-match an existing row (the phone match also requires a NULL email and a name match), so a proxy phone can only ever live on a row `findOrCreateClient` created with `source = 'thumbtack'`. The signing form never shows a proxy number to the client.

**Server, `POST /api/proposals/t/:token/sign`:**
- Accept optional `client_phone`. Validation runs with the other field validations (so errors surface together): validate with the existing `validatePhone` helper from `server/utils/phone.js` (the save-time helper: returns `{ value, error }`, coerces empty to null, strips a leading 1 from an 11-digit NANP number, enforces exactly 10 digits). On error, `ValidationError` keyed `fieldErrors.client_phone`. Storage format is the returned 10-digit string, consistent with how `clients.phone` is stored everywhere else and with inbound last-10 matching. (NOT `normalizePhone` from `sms.js`, which is the send-time E.164 converter.) Empty or missing input is valid and never overwrites.
- **The phone write is gated on the signature write succeeding.** Run it after the existing sign UPDATE returns a row (the `client_signed_at IS NULL` TOCTOU gate): a replayed or stale sign POST that hits `ALREADY_ACCEPTED` performs no phone write, so a leaked token cannot mutate the phone after acceptance. The write is `UPDATE clients SET phone = $1, phone_status = 'ok' WHERE id = $2 AND phone IS DISTINCT FROM $1`, scoped to the proposal's `client_id`. Resetting `phone_status` matters: `'bad'` suppresses every automated SMS via `channelFallback.js` and is keyed to the client row, not the number, so a stale verdict earned by the old proxy (for example, delivery failures during the unregistered-number window) must not follow the client onto their fresh real number.
...
- Unit (`node:test`, run per-suite per the shared-dev-DB constraint):
  ...
  - Sign endpoint: valid phone updates (and resets `phone_status` to `'ok'`), invalid rejected with `fieldErrors.client_phone`, empty no-op, replayed sign (`ALREADY_ACCEPTED`) performs no phone write, phone-write failure does not fail the signature.
