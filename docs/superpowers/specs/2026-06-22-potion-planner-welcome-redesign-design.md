...
They already booked, so "step zero" is genuinely complete. Surface it: "Your booking's confirmed, so you're already underway." Seed the in-flow progress indicator (`.potion-rail`) so the first tick reads done on entry, with that real reason. Do not fabricate progress beyond the booking.
...
- Endowed + orient: "Your booking's confirmed, so you're already underway. All that's left is finalizing the bar: three parts, about three minutes."
...
- Reassurance (near the button): "No wrong answers. You can change anything later, and your progress saves as you go."
- Primary button label on the welcome step: **"Start"** (currently hardcoded "Next").

The de-jargoning is the key content move: "Confirm logistics" becomes "The day-of details," "Choose menu design" becomes "Design your menu card." Each part is one warm sentence that *names* what's inside it (no bullet stacks, which read as a workload and re-trigger the daunt).

### Files touched

- `client/src/pages/plan/components/WelcomeRoadmap.js` — restructure three horizontal cards into the vertical numbered procedure; new copy; keep the `mode` (byob/hosted) branch.
- `client/src/pages/plan/steps/RefinementWelcomeStep.js` — greeting + endowed/orient + reassurance copy and the layout around the procedure.
- `client/src/pages/plan/PotionPlanningLab.js` — Start label on the welcome step (`nextLabel` is currently hardcoded `'Next'`, line ~764); optional honest seed of the welcome-step progress rail.
- `client/src/index.css` (`.potion-app` scope) — replace the existing `.potion-roadmap*` rules (~13262 to 13353) with the vertical-procedure styles, and welcome-layout / Start-button prominence tweaks.

### File-size constraint (load-bearing)

`index.css` is far over the 1000-line hard cap (~16.9k lines), so the pre-commit hook blocks any commit that *grows* it. The new procedure CSS must be written by **replacing** the old `.potion-roadmap*` block in place (old block ~90 lines), keeping the file net flat or shrinking. Do not add the new rules alongside the old ones.

## Verification

- Render the welcome step in **both** byob and hosted modes (the isolated-CSS harness used for the contrast fix, or a real `/plan/:token`), screenshot, and confirm: nothing reads as clickable except the Start button; the orient line, three plain-language parts, and reassurance line are present and legible; the progress shows the seeded head start.
