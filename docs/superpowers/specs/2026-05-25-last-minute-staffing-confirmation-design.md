...
1. **Extract + rename.** Move the helper out of `server/routes/shifts.js` into a new `server/utils/lastMinuteStaffingConfirmation.js` (the same module that hosts the notify fn) and rename it to `confirmStaffingIfFullyStaffed`. Both `shifts.js` (at the two existing call sites) and `autoAssign.js` (at the new call site) import the renamed helper from `../utils/lastMinuteStaffingConfirmation`. The extraction is required, not a stylistic preference: if the helper stayed in `routes/shifts.js`, the new `autoAssign.js` import would need to reach back into `routes/`, but `shifts.js:7` already requires `../utils/autoAssign` at module-load, creating a hard circular require where `autoAssign.js`'s top-level `require('../routes/shifts').confirmStaffingIfFullyStaffed` would resolve to `undefined`. A grep across the repo confirms three code references for the OLD name: the definition in `shifts.js`, the call at `shifts.js:669`, and the call at `shifts.js:786`. The literal string in the outer catch's `console.error(...)` moves with the function and gets updated. No test file or other consumer holds a stale reference, so the rename + move is a closed change set.
2. **Atomic flip becomes the one-shot guard.** Replace today's unconditional `UPDATE proposals SET last_minute_hold = false WHERE id = $1 AND last_minute_hold = true` with the same query plus `RETURNING id`. If a row comes back, *this caller* is the unique owner of the flip and is responsible for firing the notification. If zero rows, the hold was already cleared (by an earlier concurrent fill, or the proposal was never held) and the notify path is skipped. No new column, no extra row, no race. `proposalId` is in hand from the existing `SELECT proposal_id, positions_needed FROM shifts WHERE id = $1` at `shifts.js:831-833` and passed straight into `notifyClientOfStaffingConfirmation(proposalId, shiftId)`; the `RETURNING id` is a guard, not a value source.
   ...
   **PIN:** keep the existing `JSON.parse(row.positions_needed || '[]').length` parsing (with its inner try/catch) when computing `needed`. `shifts.positions_needed` is `TEXT DEFAULT '[]'` (JSON-encoded string, `schema.sql:280`), not JSONB. An `Array.isArray(row.positions_needed)` check would silently always return false → `needed === 0` → early return → notify never fires AND the existing hold-clear regresses.
...
2. **Load bartenders for the just-filled shift only.** Filter by `sr.shift_id = $shiftId`, not by `proposal_id`:
   ...
   SELECT cp.preferred_name, cp.phone
    ...
    ORDER BY sr.id ASC
   ```
   Display name per bartender: `preferred_name || 'Your bartender'`. The `users` table has **no** `first_name` or `last_name` columns (verified against `schema.sql:12-21` plus every `ALTER TABLE users`); `contractor_profiles.preferred_name` is the only name source. The trailing fallback covers a null `preferred_name` AND a missing `contractor_profiles` row (LEFT JOIN → null). `shift_requests.user_id` has `ON DELETE CASCADE` on `users(id)` (`schema.sql:298`), so any approved `shift_requests` row's `user_id` is guaranteed to point at a live user; no `users` JOIN is needed.
...
* **Bartender missing `contractor_profiles` row OR null `preferred_name`.** Renders as `'Your bartender'` per the fallback. Still useful (and rare enough that a generic label is acceptable).
...
| File | Change |
|---|---|
| `server/routes/shifts.js` | DELETE the existing `clearHoldIfFullyStaffed` helper (lines 821-857). At the two call sites (lines 669, 786), call the extracted `confirmStaffingIfFullyStaffed` imported from `../utils/lastMinuteStaffingConfirmation`. No new Sentry import needed (`shifts.js:2` already imports `@sentry/node`); no new `module.exports` shape (router stays the only export). |
| `server/utils/autoAssign.js` | Add `await confirmStaffingIfFullyStaffed(shiftId)` between the per-candidate SMS for-loop (ends ~line 342) and the `auto_assigned_at` UPDATE at line 346. The approval itself is the batched `UPDATE shift_requests` at lines 305-310; the for-loop is per-candidate Twilio SMS, not part of approval. Wrap the new call in try/catch + Sentry (matches the sibling `scheduleStaffShiftMessages` pattern at line 353: `Sentry.captureException(err, { tags: { component: 'autoAssign', issue: 'staffing-confirmation' }, extra: { shiftId } })`). Import from `./lastMinuteStaffingConfirmation` (sibling util, no cross-layer, no circular require). |
| `server/utils/rescheduleProposal.js` | Inside `rescheduleProposalInTx(client, { proposalId, old, updated })`, after the existing balance-due / cascade work, compute `getBookingWindow({ eventDate: updated.event_date, eventStartTime: updated.event_start_time })` and `UPDATE proposals SET last_minute_hold = $1 WHERE id = $2` via the same `client` if the computed value differs from `updated.last_minute_hold`. The PATCH handler at `crud.js:617-626` is the only production caller; the test-only `rescheduleProposal(...)` wrapper inherits the behavior for free. |
| `server/utils/lastMinuteStaffingConfirmation.js` | **New.** Houses the whole Touch 2.2 feature in one module: `renderBartenderList` (pure), `_resolveDisplayName` (pure), `notifyClientOfStaffingConfirmation(proposalId, shiftId)` (side-effect), and `confirmStaffingIfFullyStaffed(shiftId)` (side-effect; the extracted/renamed helper that wraps the atomic flip + the notify call). ~200 lines. |
...
* **Bartender-list renderer unit tests.** 1 / 2 / 3 / 4 bartenders; all-phones, no-phones, mixed; null `preferred_name` AND missing `contractor_profiles` row → renders `'Your bartender'` fallback.
