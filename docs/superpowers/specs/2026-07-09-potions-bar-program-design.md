...
6. Day-one generated output byte-identical to today until Dallas edits data. Proven by test. One documented carve-out: consult "matching mixers" lists keep identical content but may reorder (pairing order derives from catalog row order); everything else is byte-exact.
...
- **SettingsDashboard** drops its `drink-menu` tab (one home, not two) AND re-points its default `activeTab` (`SettingsDashboard.js:288`, currently `useState('drink-menu')`) to the first remaining tab, or Settings opens onto a tab that no longer exists.
...
- **Plans drawer** (design 1a): compact review list, newest first: event label (via `getEventTypeLabel`, never concatenated with client name), client, date, guests, service mode via `SERVING_LABEL` (currently a module-local const in `DrinkPlansDashboard.js:24`; extract it to a shared util both consumers import), resolved drink names, status chips. Chip logic: primary chip is the plan lifecycle status, covering all five values including `exploration_saved` (the existing index forgets it); a secondary warn chip "List to review" appears when `shopping_list_status = 'pending_review'` (the badge condition). Standard loading / empty ("No plans yet") / error-with-retry states per existing async-surface conventions. Rows link to `/drink-plans/:id`; footer link "Full index" opens `/drink-plans`.
...
| `SPIRIT_MIXER_PAIRINGS` | inversion of `paired_spirits` tags (per-spirit mixer ORDER now follows catalog row order; content identical, order may differ on consult "matching" lists — the one accepted output delta) |
| `INGREDIENT_MAP` | `ingredient_aliases` (+ item/section from the row; size from the row EXCEPT in the signature-merge path, where a resolved `role='spirit'` item sized `1.75L` is added as `750mL`, preserving the legacy map's deliberate smaller sig-drink bottles) |
...
- **Public exposure trim**: the public `GET /api/cocktails` and `GET /api/mocktails` currently `SELECT c.*` / `m.*` and feed the unauthenticated planner (`PotionPlanningLab.js:124-125`). They switch to explicit column lists that EXCLUDE `recipe_review` (internal workflow state). `ingredients` stays served: recipe visibility matches the menu's existing ingredient-gist descriptions and powers bartender prep on `ShiftDetail.js:686`; flagged to Dallas as an accepted IP-exposure decision (recipes scrape-able from the public endpoint; revisit if he considers the bar book proprietary).
- **Concurrency (recipes)**: whole-recipe PUT is last-write-wins, accepted (one admin plus Zul; no optimistic-lock ceremony). Two rules make it safe in practice: the client flushes any pending debounced save before Mark-reviewed fires, and the review-state auto-transition is strictly empty→draft (editing a `reviewed` recipe never demotes it), so a late-landing recipe PUT cannot flip a just-reviewed drink back to draft.
- **Pars**: new `server/routes/potions.js` mounted at `/api/potions` (`server/index.js`, one line, beside the cocktails mount; separate prefix sidesteps the greedy `/:id` mount-order trap). Endpoints, all admin/manager: `GET /pars` (full active catalog), `POST /pars`, `PUT /pars/:id`, `DELETE /pars/:id` (soft: `is_active = false`), `POST /pars/reorder` (unnest bulk pattern verbatim from `cocktails.js:50-64`, text ids). Validation: role/section enums, qty numeric ≥ 0, slug id server-generated from item name when absent, 23505 → ConflictError. **Delete is blocked (`ConflictError`, category-delete precedent at `cocktails.js:122-136`) while any recipe resolves to the row** (alias or `override_item_id`; the used-by computation the tab already needs); the error names the referencing drinks so the admin re-points recipes first. The resolver reads only active rows; an `override_item_id` pointing at a missing or inactive row falls through to alias resolution, then to the visible unresolved state (never a silent drop, never a resurrected item). `override_item_id` is validated on write (must reference an existing active par row).
- **Preview**: `GET /api/potions/preview?guests=175&mode=full_bar|spirit_driven` (admin) runs the pure generator against the live catalog with a synthetic input; feeds the Pars tab "Preview shopping list" button and the Projected column sanity check. Read-only, saves nothing. Validation: `guests` integer clamped 1..1000 (400 outside), `mode` enum (400 otherwise).
- **Plans list enrichment** (additive, existing consumers unaffected): `GET /drink-plans` gains `shopping_list_status` (already on the row), `guest_count` (LEFT JOIN proposals, the join `GET /:id` already uses), and `drink_names` (batched: collect `selections.signatureDrinks` + `selections.mocktails` ids across returned rows, one query per table, map in JS; no N+1; skip the queries entirely when the collected id set is empty).
- **Regenerate**: `POST /drink-plans/:id/shopping-list/regenerate` (admin), body `{ guest_count_override? }` (snake_case per API convention). Picks the input builder by `shopping_list_source`; **`NULL` source (every manually generated list today) defaults to the planner builder**. Guard: if the plan has no proposal `guest_count` and no `guestCountOverride`, throw `ValidationError` ("Set a guest count to generate this list") rather than produce NaN quantities. Validate `guestCountOverride` as an integer clamped 1..1000. Returns the fresh list JSON, **does not write**. The modal's existing `PUT /:id/shopping-list` and `PATCH /:id/shopping-list/approve` remain the only writers. Note: there is no server-side manual generate path today (manual generation is client-side in `ShoppingListButton.jsx`); this endpoint becomes that path.
...
- `generateShoppingList()` stays pure ("no DB calls" header contract). New signature accepts a `catalog` argument. `server/utils/potionCatalog.js` (new, pure) owns `buildCatalogSlices(parRows)` (the derivations table above), alias resolution, and the recipe resolver. Callers load rows and pass them in: `shoppingListGen.js` (`buildPlannerGeneratorInput`, `buildConsultGeneratorInput`, `autoGenerateShoppingList`) and the new preview + regenerate endpoints.
- **Fallback discipline**: if the catalog read returns zero active rows OR throws (table absent mid-deploy, transient DB error), fall back to the legacy constants (kept in `shoppingList.js` as `LEGACY_*`, no longer consumed on the happy path) and `Sentry.captureMessage('par_catalog_empty' / 'par_catalog_read_failed')`. Never generate a bare list, and never let a catalog read failure abort a caller's transaction: in the consult path the catalog is loaded BEFORE `BEGIN` (today generation cannot fail on a DB read; that property is preserved). When generation encounters an unresolved ingredient, emit a Sentry breadcrumb (`captureMessage('unresolved_ingredient')` with drink + ingredient) so silent wrong-list regressions are visible in prod, not just in the Recipes tab UI.
...
- **Mirror kill** (two client consumers, both rewired; found by spec fleet after a `.js`-only grep missed the `.jsx` one):
  1. `ShoppingListModal.jsx` regenerate (~:78) calls the regenerate endpoint. Regenerate keeps a `window.confirm` gate (matching the existing Reset/guest-count precedent): it replaces manual edits and, on save, an approved list returns to `pending_review`; never silently.
  2. `ShoppingListButton.jsx` (`:4` import, `:39` call, rendered by `DrinkPlanDetail.js:203` and the canonical event-side `DrinkPlanCard.js:198`) owns the FIRST-generate and manual-guest-count flows; both route through the regenerate endpoint. Its `GET /drink-plans/:id/shopping-list-data` input feed (`:66`, sole consumer, verified) is retired in the same lane.

  Then delete `client/src/components/ShoppingList/generateShoppingList.js` and `shoppingListPars.js`. `client/src/data/syrups.js` stays (planner wizard UI data, not generator). The `CI=true react-scripts build` gate proves no import survives.
- **needsRecipe persistence and rendering**: the generated blob carries `needsRecipe`; the modal's save/approve persist it with the rest of the list, so the token route serves it. Renders in the modal ("Client requested: recipe needed" + Add recipe), and on `ClientShoppingList.js` + `ShoppingListPDF.jsx` with the client copy from above. These two files gain the block; they are otherwise untouched by the mirror kill. `needsRecipe` entries carry `{ name }` only (the client's own free text; no admin data). The stored blob's item rows stay purchasable-level (item/size/qty/section); recipe internals (`note`, per-serving amounts, `override_item_id`, review state) never enter the blob.
...
- **Recipes tab** (design master-detail): left list (search, per-drink chip `OK/Draft/Empty`, drink-type seg cocktails/mocktails), right detail card (recipe table: drag-order rows; cells ingredient / amount / unit(select) / resolved purchasable "→ Tito's Vodka · 1.75L" with override picker / note; add/delete row; category select; **Mark reviewed** button; "N of M reviewed" counter in the page header). Unresolved ingredient renders a warning chip linking to the Pars tab ("alias it or add an item"). Editing writes through the whole-recipe PUT via debounced autosave with a visible saved/unsaved indicator; Mark-reviewed flushes any pending save first. Client-side validation mirrors the server rules (amount numeric > 0, unit from the closed set); an invalid cell shows a `FieldError` and blocks the save with "fix amount to save" rather than 400-ing silently. Standard loading / empty ("No drinks yet") / error-with-retry states on both panes; a failed autosave surfaces a `FormBanner` with retry.
- **Pars tab**: two section cards (Liquor · Beer · Wine / Everything Else), rows drag-orderable, inline cells item / size / qty @ 100 / **Projected @ N** (computed, `max(1, ceil(qty × guests / 100))`, accent-styled), **Called on** chips (`Full bar`, `Vodka · pairs`, `Style: IPA`, `Always`, from role/flags/keys), **Used by** chip (count of recipes resolving to the row), delete (soft; blocked with the referencing drinks named while used-by > 0). "Baseline stock is set for 100 guests" explainer card with the pulsing flask icon (the one magical-realism moment), guest-count preview input, mode preview filter seg (Full Bar / Spirit-Driven), and "Preview shopping list" button (calls `/api/potions/preview`). Add-item form generates the slug server-side. Same loading / empty / error-with-retry standards as the Recipes tab.
...
- **Docs law**: README folder tree (add `PotionsPage.js` + `potions/` siblings, `server/routes/potions.js`, `server/utils/potionCatalog.js`, `server/scripts/seedRecipeDrafts.js`; REMOVE `generateShoppingList.js` + `shoppingListPars.js`), README Key Features entry for Potions, npm scripts (if any added), ARCHITECTURE route table (`/api/potions`, drink-plans additions, regenerate) + a mention of `potionCatalog.js` in the shopping-list section, schema section (par_items, new columns), this spec linked from the fix-list Bar Program entry.

## 8. Verification

1. Parity unit test (above) green before any UI work merges.
2. Generator behavior tests: planner custom matched/unmatched, consult mocktails resolve, alias exact-vs-substring, empty-catalog AND throwing-catalog fallbacks fire the Sentry path, regenerate with `shopping_list_source = NULL` defaults to planner, regenerate without guest count throws ValidationError, inactive `override_item_id` falls through to alias resolution.
