...
- D4 Under overpayment scope, reconciliation consumes the payment's **uncredited** headroom first and walks invoice links only for the remainder. The planner prefers the payment with the most uncredited headroom **among candidates that can cover the amount**, falling back to largest-remaining so the over-cap message stays true (Dallas approved, 2026-09-15). **Amended after the lane fleet, then CORRECTED at push time.** Absorption alone only fixes the overflow case when the headroom covers the refund, so the panel additionally refuses (`OVERPAYMENT_NOT_ON_A_CHARGE`) an overpayment it cannot fund. What it may be funded FROM is two things, and the first version of this rule named only one of them, which refused the commonest overpayment there is:

  1. the target charge's **uncredited headroom**, and
  2. **`contractInvoiceSlackCents`**: how much the contract invoices still demand beyond `total_price`.

  The second is load-bearing. Paying a contract in full by card credits the whole charge to a Balance invoice, which then locks; repricing down afterwards leaves that invoice demanding the old figure while the contract is lower. There is no uncredited money anywhere, yet reversing part of that credit is precisely the right correction, and the reconciler already did it correctly (`refundHelpers.scope.test.js`, the locked-invoice case). A headroom-only rule refused it, leaving the admin with a permanent overpayment or the uncapped contract path, which is the original bug. Money taken outside Stripe is the opposite: the invoices already match the contract, slack is zero, and reversing a credit would push a settled invoice BELOW it. Same headroom, opposite right answer; only the slack tells them apart. The webhook cannot refuse, so it applies the same allowance to choose `contract` instead.
...
Absorption alone is CONDITIONAL on the headroom covering the refund; the allowance rule in section 4 is what makes it complete on the panel path.

**Cancel-line DOES change**, and the earlier claim that it did not was wrong. 22 of 103 succeeded prod payments carry uncredited headroom, dominantly a `full` payment linked only to the $100 Deposit invoice, and cancel-line issues overpayment-scope refunds, so absorption fires there. The new outcome is better (the deposit really was paid, and its invoice keeps saying so, where the old walk zeroed it) and it is now pinned by its own fixture in `refundHelpers.scope.test.js`. Cancel-line does not pass through `planRefund`, so the refusal never applies to it.
...
2. Resolve the proposal from `proposal_payments` by intent, and ONLY from there. `refund.metadata.proposal_id` is externally supplied, which is what D10 hardens against elsewhere, so it never decides attribution. No payment row: a `stripe_sessions` row for that intent (written by every checkout rail when the intent is minted, and never by a tip) means the charge IS ours and its payment row has not been recorded yet, so the delivery FAILS and Stripe retries; acking there would drop the refund permanently, since a dashboard refund has no pending row for the sweeper to heal. Neither a session row nor that metadata means it is not ours: return, the dispatcher acks.
...
8. Sentry warnings, ids and cents only, never a name or an address: a contract-scope refund landing on a proposal whose netted excess is positive (the money rule may be wrong in the shrinking direction), and a refund that looked like an overpayment but whose charge holds too little uncredited money, which therefore lands `contract` instead. That last rule is D4 on the webhook side: the panel can refuse such a refund, the webhook cannot (the money has moved), so it picks the rule the charge can honor rather than reversing credited invoice money while the contract stands. The excess it compares against is the AVAILABLE one, net of pending overpayment refunds, so two dashboard refunds in the same window cannot spend it twice.
