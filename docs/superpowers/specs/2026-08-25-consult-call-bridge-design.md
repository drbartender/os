...
- `GET /api/admin/lead-call-attention` (`server/routes/admin/leadCalls.js`) gains a UNION of `consult_call_attempts` rows in `failed`/`skipped_unconfigured`/`skipped_invalid_phone`/`skipped_missed_window`/`skipped_cap` from the last 7 days whose consult is still `scheduled`. **REV 3 CORRECTION, and it reverses rev 2: there is NO `scheduled_at > NOW()` filter.** Rev 2 borrowed the lead feed's reasoning, that once the moment passes there is nothing to rescue, and that reasoning does not transfer. A missed LEAD is a lost opportunity; a failed CONSULT is a client who was promised a call and did not get one, so the slot passing CREATES the callback obligation rather than retiring it. The filter was also self-defeating. **Corrected count, because rev 3 first shipped an overcount and a reviewer caught it against the code:** TWO statuses are written after the slot by construction, `failed`/`too_late` past the 10-minute bound and `skipped_missed_window` past by definition, and a third is path-dependent, the VA-leg `failed` reached via the ring-3 hop at slot+180. `skipped_cap`/`va_leg_cap_tripped` is NOT one of them: its only writer sits in `advanceChain`'s `ADMIN_PHONE`-unset branch and runs at ring-1 time, slot minus 90 seconds, with the slot still ahead. The ring-3 hop deliberately files no cap marker when the international ceiling is spent, falling through to `finishMissed` so the missed text still goes out rather than being replaced by silence. The decision is unchanged and rests on the two arguments that were never in doubt: the callback obligation, and the fact that `skipped_unconfigured` emails NOTHING at all, so the feed was its only surface and the filter hid it within about 90 seconds. `c.status = 'scheduled'` stays and is the correct clearing mechanism: a consult that reaches `completed`, `cancelled` or `no_show` is genuinely closed, and the 7-day bound on `a.created_at` bounds the rest. Rows are shaped like the lead rows plus `kind: 'lead' | 'consult'`, aliasing `booker_name AS customer_name`. The consumer is `buildLeadCallItems` in `client/src/pages/admin/overview/queueItems.js` (fetched by `OverviewPage.js`); it keys items on `kind + id` (a bare id collides across the two tables) and titles consult rows "Consult call with Tyler Anderson call failed" / "bad number" / "call misconfigured" / "missed window". Healthy steady state stays empty.
...
- **`skipped_cap` carries THREE details, not two.** `cap_tripped` from `openChain` is the chain-open daily cap and is the one a stranger hammering the public page hits first and most often; `dial_cap_tripped` is the ceiling on rings to Dallas; `va_leg_cap_tripped` is the ceiling on international legs. Any surface that maps cap details must cover all three or the DOMINANT one lands on a fallback.
- **Toll-fraud and harassment posture (stated).** Agent legs dial only env-configured numbers; the client leg dials only `toUsE164` output; `timeLimit` caps every bridged leg. The trigger is a PUBLIC booking page, and the rolling cap (default 10) bounds the legs whose chains reach a COUNTED status: at most 10 rings to Manila, 10 texts and one bounded email per rolling 24h. **AMENDED 2026-08-25, before the launch call: it now bounds the rings to the 312 as well.** As written, it did not. The cap counts `status NOT LIKE 'skipped%'`, and a chain that ends `skipped_cancelled` frees its slot again, so a book, ring, cancel, rebook cycle could ring Dallas as many times as the Cal.com page had bookable slots to sell, at up to `MAX_ADMIN_RINGS` (3) rings per opened chain -- roughly 600 to 900 rings a day against a 15-minute, 12-hour booking page. Two reviewers found that independently. A SECOND rolling-24h counter, `dialCap()` = `CONSULT_CALL_DAILY_CAP` x `MAX_ADMIN_RINGS`, now sums `admin_ring` over EVERY row in the window, skipped ones included, and is checked in `advanceChain` before the admin claim; a trip files `skipped_cap`/`dial_cap_tripped` and emails once per window. R15 is untouched: `openChain` still ignores skipped rows, so junk bookings still cannot hold the feature down for 24 hours. The two counters read the same rows differently, on purpose. That is a deliberate trade, not an oversight (ruling R15): counting `skipped%` rows would let a burst of junk bookings on the public page hold the whole feature down for 24 hours past the last one, which is strictly worse than rings Dallas can decline. So nobody should raise or lower `CONSULT_CALL_DAILY_CAP` believing it is what holds the ring count down; raising it widens a public surface on the counted legs, and treat any such edit as a security change.
...
- `CONSULT_CALL_DAILY_CAP`: max chains opened per rolling 24h, default 10; also the public-surface ceiling (section 6), and, multiplied by `MAX_ADMIN_RINGS`, the rolling ceiling on rings placed at the 312 (`dialCap()`).
