...
- **D10. The admin Payment Link rail offers card and Link only** (review round). It settles through `checkout.session.completed`, which credits only a session Stripe reports paid; a bank debit through a link returns unpaid and its later async success event is not handled, so the money would never be credited. `PAYMENT_LINK_METHOD_TYPES` in `stripeRouteHelpers.js`.
...
- Two new indexes: the partial `idx_stripe_sessions_proposal_processing` (section 3.2) for the processing reads, and a plain `idx_stripe_sessions_proposal_id`, because the combined guard and the widened autopay scan read pending, processing and failed rows together and the two partial indexes cover one status each. The `invoice_id` foreign key is added in its own guarded DO block (idempotent on its own, not only as part of the column add).
...
Two idempotency rules (review round, corrected by the push-time second opinion 2026-09-15): the handler records the Stripe event id in the `webhook_events` ledger INSIDE its transaction and returns on a repeat, so a delivery whose transaction fails leaves no ledger row and Stripe's retry is processed rather than acknowledged and dropped; and the UPDATE moves a `pending` row, or a `failed` row whose intent Stripe reports `processing` right now (one read before the transaction, because Stripe does not promise event order and a failed event delivered before a delayed processing event must not be undone by it). Unknown at Stripe leaves the row released; any other error on that read fails the delivery before the ledger insert, so Stripe retries it rather than the event being acknowledged with the real debit invisible. A row that processed and then bounced revives the same way when the client debits again with the same intent. `succeeded` and `processing` never move, so a late processing event can never downgrade a settled payment.
...
- Known residual, accepted 2026-09-15: the guard is a read followed by a create, not a lock, so two Pay requests inside the same fraction of a second can both pass it. That window existed at every moment before this change; the guard narrows it to concurrent requests only. A per-proposal lock around check-and-create would close it if it ever matters.
...
- A fifth phase, `'blocked'` (push-time review 2026-09-15): the create-intent rail answered 409 `PAYMENT_IN_FLIGHT` and the reloaded row carries no `pending_payment`. The rail refuses for three reasons (section 5.1: a debit Stripe already sees before the webhook wrote the row, a bank account waiting on verification, a card that already settled and is being recorded) and only the first matches the processing card, so the blocked phase shows the rail's own 409 message with a Refresh button and never `PendingPaymentCard`. The pay controls stay hidden and no intent is minted until reload, exactly as for a pending payment. A reload that finds the row lands `'pending'`; the row wins over the latch.
...
- Visual pass 2026-09-14 (Playwright on dev, all four surfaces at three widths): the closing line's address is a mailto link; the card's title is a paragraph, not a heading, on every surface; in the invoice page's narrow rail the card reads left-aligned, and below the two-column breakpoint the rail moves above the document while a payment is pending; the invoice page imports the Stripe.js loader lazily so a pending visit loads no Stripe script; the admin Processing line uses the panel's warning hue at weight 500 in both skins (the `--amber*` names are teal). Noted, not built: the live region's initial content is not announced on a cold load, and the activity log renders `payment_processing` raw like every other action.
