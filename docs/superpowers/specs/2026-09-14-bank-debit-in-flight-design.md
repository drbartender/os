...
Two idempotency rules (review round): the handler records the Stripe event id in the `webhook_events` ledger first and returns on a repeat, like the failed handler; and the UPDATE moves a `pending` row or a `failed` row whose `processing_at` is NULL, never one that processed and then bounced, so a queued redelivery landing after a bounce cannot lock the proposal again or email a received notice for money that came back. `succeeded` and `processing` never move, so a late processing event can never downgrade a settled payment.
