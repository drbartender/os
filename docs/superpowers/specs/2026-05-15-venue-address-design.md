...
city + state + ZIP). Capture it **optionally** in the quote wizard (most clients
are excited about their venue and will volunteer it early). If the street address
is still missing when the client signs and pays, **require it at the sign + pay
step** — the booking cannot complete without it.

## Goals

- Capture a structured venue address: `venue_name` (optional), `venue_street`,
  `venue_city`, `venue_state`, `venue_zip`.
- Zero new friction for clients who provide it in the quote wizard.
- A hard requirement at sign + pay for clients who did not — enforced
...
abbreviation mapping is introduced.
...
| venue_street | optional     | **required**    |
| venue_zip    | optional     | **required**    |
| venue_name   | optional     | optional        |

`venue_city`/`venue_state` requirement in the wizard exactly preserves today's
behavior (they are already required there). Server-side validation mirrors the
client rules on every endpoint; `venue_state` is validated against the same
allowed-state list the dropdown uses.

## Shared Component

`client/src/components/VenueAddressFields.js` (new). One controlled component
rendering: Venue name *(optional)* · Street · City · State (the existing
5-state `<select>`, reused) · ZIP. Props expose values, an `onChange`, and a
`requireStreet` flag (false in wizard, true at the gate) so the same markup and
validation logic serve all three surfaces with no drift. This replaces the
single freeform `LocationInput` usage for venue entry. `LocationInput.js` stays
in the repo for any other callers but is no longer used for venue address.

## Surface 1 — Quote Wizard (optional capture)
...
`client/src/pages/website/quoteWizard/helpers.js` (`defaultForm` /
`steps`), `server/routes/proposals/public.js`
(`POST /api/proposals/public/submit`, INSERT ~329–342).

- `EventDetailsStep` renders `VenueAddressFields` with `requireStreet={false}`.
  City + State stay required (unchanged validation). Venue name, Street, ZIP are
  optional.
- `helpers.js` `defaultForm` gains `venue_name`, `venue_street`, `venue_zip`
  (city/state already present as `event_city`/`event_state`; reuse those values
  for `venue_city`/`venue_state`).
- `QuoteWizard.js` submit stops sending the pre-joined
  `event_location: "City, State"` and instead sends the five `venue_*` keys.
- `public.js` submit handler reads the `venue_*` keys, validates (wizard tier),
  stores them in the new columns, and sets `event_location` via the Composition
  Rule.
...
- `GET /t/:token` returns the structured `venue_*` values and a boolean
  `venue_complete` (true when `venue_street`, `venue_city`, `venue_state`,
  `venue_zip` are all present).
- `SignAndPaySection`:
  ...
  - `venue_complete === false` → render `VenueAddressFields` with
    `requireStreet={true}`. The signature submit control and the deposit/pay
    control are disabled until the gate-tier validation passes.
- `POST /t/:token/sign` accepts the `venue_*` keys. **Server-side**: if the
  ...
  is validated at the gate tier and is **required** — reject the signature with
  a `ValidationError` if absent/invalid. On success, persist the structured
  fields and recompute `event_location`.
- `POST /create-intent/:token` re-checks (defense in depth): refuse to create a
  payment intent if the proposal still lacks a complete venue address. Since the
  shift is created at `deposit_paid` (Stripe webhook → `createEventShifts`), the
...
- Replace the single `LocationInput` with `VenueAddressFields`
  (`requireStreet={false}` for admin — admin is trusted and may legitimately
  save a partial address for an early-stage proposal).
...
`[venue_name, venue_street, "<city>, <state><space><zip>"]` → e.g.
`"Citadel Banquet Hall, 123 Main St, Chicago, Illinois 60601"`. When only
city/state exist (early proposal, no street): `"Chicago, Illinois"` —
byte-identical to today's behavior, so legacy proposals and any admin-typed
city/state values are unaffected.
...
- **Wizard → only city/state, then sign + pay:** gate detects missing street,
  requires the full structured address at sign; server persists and recomposes
  `event_location`; deposit webhook composes `shifts.location`. Works.
...
- Quote wizard submits with: city/state only; full address; verify proposal row
  columns + composed `event_location`.
- Sign + pay with a proposal missing street: client controls disabled until
  valid; `POST /t/:token/sign` rejects missing/invalid address server-side;
  `POST /create-intent/:token` rejects when incomplete.
- Sign + pay with a proposal that already has a complete address: read-only
...
- Server-side validation: invalid state, malformed zip, empty required fields
  all rejected with `ValidationError`.
...
Post-deposit reminder scheduler; auto-assign hard gate; admin-confirm hard
block; street autocomplete/geocode-on-type. (Recorded here so the implementation
plan does not reintroduce them.)
