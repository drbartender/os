...
lanes:
  - id: first-reply-server
    footprint:
      ...
      - server/utils/firstReplySweep.js           # new: Arm A call fallback + Arm B hygiene
      - server/utils/firstReplySweep.test.js
  ...
  - id: first-reply-agent
    footprint:
      ...
      - README.md                                 # thumbtack-agent section row
...
- [ ] F4. **Reply endpoints in `thumbtackAgent.js`** (file goes 467 → ~620, inside the yellow zone, single agent-surface concern; new suite `thumbtackAgent.replies.test.js`). All `agentSecretOnly`; body validation mirrors `harvest-failed` (negotiation_id required/trimmed; enums 400):
  - `GET /pending-first-replies?limit=N`: `[]` unless `TT_AUTOREPLY_ENABLED === 'true'`. Offer query: `first_reply_status='pending' AND (first_reply_attempted_at IS NULL OR first_reply_attempted_at < NOW() - FIRST_REPLY_COOLDOWN_INTERVAL)`, night rows additionally withheld until `created_at + ((2 + id % 13) || ' minutes')::interval` (spec 4.3 jitter). Offering stamps `first_reply_attempted_at=NOW()` AND bumps `first_reply_attempts`; a row at `first_reply_attempts >= MAX_FIRST_REPLY_ATTEMPTS` flips to `failed` instead of offering (offer-side cap, spec 4.1). Day rows offered while `LEAD_CALL_ENABLED === 'false'` are flipped to `night` in the same UPDATE before returning. Returns `negotiation_id, customer_name, first_reply_template, created_at`.
  ...
  Tests: lease + bump + cap-to-failed, cooldown re-offer, jitter withholding (night row too fresh not offered), day-to-night downgrade, kill switch `[]`, secret 401s, sent-flip fires trigger with the constructed camelCase shape from a REAL row (the gaps-blocker regression test), duplicate sent no-op, stale sent inserts `reply_confirmed_late` and never dials, failed-reason enum.
- [ ] F5. **`firstReplySweep.js`** (new util + 60s scheduler in `server/index.js` behind `enabled('RUN_FIRST_REPLY_FALLBACK_SCHEDULER')`, registered with `wrapScheduler('first_reply_sweep', 60, ...)` + a stagger, the va-calling block pattern at index.js:495). One tick:
  ...
  - Arm B: (1) day `pending` rows PAST the freshness bound with no attempt row → insert attempt row `failed` detail `reply_stale` (at-most-once via the lead_id UNIQUE); (2) `not_needed` rows younger than 24h with no attempt row (enqueue-crash strand) → `enqueueFirstReply` with the row-constructed lead shape. Arm B runs regardless of kill switches.
...
- [ ] A4. **Lane gate.** No automated harness (harvester precedent): code-review + consistency-check agents on the lane diff; `node --check` the agent file; manual dry-run on this box with the server flag OFF (agent polls, gets `[]`, idles clean through several ticks + one harvest piggyback tick). Live selectors are exercised only in the staged rollout below.
