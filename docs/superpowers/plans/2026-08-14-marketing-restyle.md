...
- **The shipped Compose has NO block palette and NO Look panel.** `ComposeTab.js` is a plain form: subject input, raw HTML textarea, RecipientPicker, confirm-send ("Write it / Who gets it / Send it"). Spec 5.1's composer (block palette + Look panel with themes/fonts/accent/corners) was never delivered into the new Compose; mkt-g shipped the simple form instead. This is a FUNCTIONAL scope drop from the original build, discovered 2026-08-14 while writing this plan. The block-designer components DO exist client-side at `client/src/components/emailBuilder/` (688 lines: EmailBlockBuilder, CampaignBlastEditor, BlockCard, BlockSettings, EmailPreviewModal, blockCatalog, starterTemplates), consumed today by the LEGACY pages `EmailCampaignCreate.js` / `EmailCampaignDetail.js`. The artifact's Look-panel data (4 themes, 6 accents, 5 fonts, `--em-*` vars) is in the artifact's trailing script block.
...
- Compose folds to the artifact's 2 steps: the shipped Send step's content (count, send result, confirm, no-scheduling line) becomes the "Before you send" rail beside the recipients table. Endpoints and the send confirmation content are untouched. The restyle applies the artifact's FRAME to the compose that exists (the plain subject + body form styled on DS tokens in the canvas well); the artifact's block canvas + Look panel is NOT deliverable behavior-inert and is declared as lane `mkt-compose-canvas` below, UNSCHEDULED, pending Dallas's explicit go. Deferring it silently would repeat the exact failure this plan corrects, so it is named here and in spec section 10.
...
lanes:
  ...
  # Declared so the deferral is visible, NOT scheduled. Feature work, not a
  # restyle: needs Dallas's explicit go before any lane is cut.
  - id: mkt-compose-canvas
    phase: 2
    scope: >
      Deliver spec 5.1's composer into the new Compose: wire the existing
      emailBuilder block palette (client/src/components/emailBuilder/) into
      ComposeTab's Design step and build the Look panel (theme presets, font
      pickers, accent, corner style) per the artifact. Touches how a draft's
      html_body is authored; the send path, guards, and resume machinery stay
      untouched.
    footprint:
      - client/src/pages/admin/marketing/ComposeTab.js
      - client/src/pages/admin/marketing/**
      - client/src/components/emailBuilder/**
      - client/src/index.css
    depends_on: [mkt-restyle]
