...
**Architecture:** Each option is a real `proposals` row; a `proposal_groups` row bundles siblings and owns the public `/compare/:token` link. The client picks on a read-only compare page, then signs/pays that option's existing page. On the winning option's first settled payment, `commitGroupChoice` runs **inside that payment's existing settle transaction** (before COMMIT): it creates the winner's invoice (so the link step finds it), first-writer-wins-sets `chosen_proposal_id`, and archives the losing rows (+voids their unpaid invoices). The losers' best-effort marketing/change-request reaps run **post-commit** (matching today's `→archived` semantics), so a loser's reap failure can never roll back a paid winner.
...
Placement (verified): the tx is committed and the client released **before** each `createEventShifts` (stripeWebhook COMMIT:361/release:378→shift:407; COMMIT:682/release:697→shift:704; actions COMMIT:229/release:234→shift:277). So `commitGroupChoice` must run **inside** each settle tx, within the `isFirstDelivery` block, **before** the existing `amount_paid`/invoice-link step (so the winner invoice it creates is present when the link runs).

- [ ] **Step 1: Failing tests** per path: winner payment archives the sibling and the winner ends with a linked invoice (no phantom balance); a payment on an already-decided-by-other option returns `conflict`, does NOT `createEventShifts`, does NOT credit `amount_paid`, and records a refund-flag + Sentry.
- [ ] **Step 2: Implement** at each site: near the top of the `isFirstDelivery` tx block, `const { conflict, archivedLoserIds } = await commitGroupChoice(proposalId, dbClient);` If `conflict`: skip the amount_paid credit + skip conversion, `Sentry.captureMessage('option_paid_after_decided', {proposalId})`, mark for manual refund; return. Otherwise proceed with the existing credit/link, then (after COMMIT, alongside the existing post-commit work) run the best-effort reaps for `archivedLoserIds`: `for (const id of archivedLoserIds) { try { await cancelMarketingForProposal(id); await cancelPendingChangeRequestsForProposal(id); } catch (e) { Sentry.captureException(e); } }` — matching today's post-commit best-effort `→archived` semantics so a reap failure never rolls back the paid winner. Note: `actions.js` also has a second `createEventShifts` at `:46` (`POST /:id/create-shift`, requires already-paid) — deliberately excluded (it is not a first-settlement path).
...
- **Coverage:** §4.1 compare contents + **category fallback** → T12/T14; §4.3 resolver + precedence → T7/T13; §5.1 gate/cap/lock + **clone group_id write** → T2; §5.2 remove/dissolution → T3; §5.3 send + suppression (**email AND SMS**) + defer → T5/T6; §5.4 rollup + **group_id in payloads** → T4/T15; §6 choice-commit **in-tx, all 3 paths, first-writer** → T10/T11; §6 loser reaps **post-commit best-effort** → T11; §7.1 winner-invoice-before-settle **on every path (inside commitGroupChoice)** → T10/T11; §7.2 void helper (**new file**) → T9; §8 schema → T1; §9 activity-log (correct `details` column) + Sentry (conflict + post-commit reap) → T2/T6/T10/T11. All covered.
