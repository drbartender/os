...
- [x] **Google Voice on the 312: confirm forwarding is ON. Voicemail deliberately STAYS ON** (Dallas's call, 2026-08-08, overriding this plan's original "DISABLE its voicemail"). Reason: the 312 was the public company number until 8/07, so direct callers to it must keep a mailbox; disabling it to protect the 1922 path would silently strand every legacy caller. The 1922 path is protected instead by `VM_PRIMARY_RING_SEC=18`, which takes the call back before GV's ~25s pickup. **Accepted edge case:** when Dallas's phone is off, dead, or on DND, GV answers immediately and a 1922 caller lands in the GV mailbox instead of ours. The interception canary logs when that happens (LOG-ONLY since 2026-08-14: the Sentry page was removed as inverted — the detector reads connected time, so it fired on fast humans; check the `[voice/missed] ... dialSec` console line, there is no alert to wait for). Also declined: repointing `VM_PRIMARY_DIAL_TARGET` at his personal cell to remove the GV hop entirely (would have closed the edge case; rejected to keep the cell out of the system).
